...
class TestBridgeParseArgs(EmbodyTestCase):
    ...
    def test_port_and_config(self):
        with patch.object(
            sys,
            "argv",
            ["envoy_bridge.py", "--port", "9999", "--config", "/tmp/c.json"],
        ):
            port, config = bridge.parse_args()
            self.assertEqual(port, 9999)
...
class TestBridgeForwardToHttp(EmbodyTestCase):
    ...
    def test_http_error_raises_oserror(self):
        ...
        err = urllib.error.HTTPError("http://localhost:9870/mcp", 500, "Internal Server Error", {}, None)
    ...
    def test_request_content_type_header(self):
        with patch("urllib.request.urlopen", return_value=self._make_response("{}")) as mock_urlopen:
            bridge.forward_to_http("http://localhost:9870/mcp", {"id": 1})
...
class TestBridgeWaitForEnvoy(EmbodyTestCase):
    def test_server_up_immediately(self):
        ...
        with patch("urllib.request.urlopen"):
            result = bridge.wait_for_envoy("http://localhost:9870/mcp", time.monotonic() + 10)
    ...
    def test_connection_refused_retries_then_succeeds(self):
        ...
        with (
            ...
            result = bridge.wait_for_envoy("http://localhost:9870/mcp", time.monotonic() + 60)
        self.assertTrue(result)
    ...
    def test_deadline_expired_returns_false(self):
        ...
        with (
            ...
            result = bridge.wait_for_envoy("http://localhost:9870/mcp", time.monotonic() - 1)
    ...
    def test_many_retries_caps_at_last_interval(self):
        ...
        self.assertApproxEqual(tail_sleep, bridge.RETRY_INTERVALS[-1], tolerance=0.01)
...
class TestBridgeMainLoop(EmbodyTestCase):
    ...
    def _run_main(self, stdin_messages, wait_result=True, forward_side_effect=None, port_args=None):
        ...
        if forward_side_effect is None:
            fwd = MagicMock(return_value={"jsonrpc": "2.0", "id": 1, "result": "ok"})
    ...
    def test_initial_connection_timeout_sends_error(self):
        ...
        msg = {"jsonrpc": "2.0", "id": 1, "method": "resources/list"}
        responses = self._run_main([msg], wait_result=False, forward_side_effect=fail)
        self.assertLen(responses, 1)
    ...
    def test_full_mcp_handshake_when_td_down(self):
        ...
        self.assertLen(responses, 3)
        self.assertEqual(responses[0]["result"]["serverInfo"]["name"], "envoy-bridge")
        self.assertIn("launch_td", {t["name"] for t in responses[1]["result"]["tools"]})
    ...
    def test_local_ping_request_returns_empty_result(self):
        ...
        responses = self._run_main([msg], wait_result=False)
        self.assertLen(responses, 1)
        self.assertEqual(responses[0], {"jsonrpc": "2.0", "id": 42, "result": {}})
    ...
    def test_initial_timeout_then_next_message_retries_connect(self):
        ...
        responses = self._run_main(msgs, wait_result=False, forward_side_effect=fwd)
    ...
    def test_url_error_sends_error(self):
        ...
        responses = self._run_main([{"jsonrpc": "2.0", "id": 1, "method": "x"}], forward_side_effect=forward)
        self.assertDictHasKey(responses[0], "error")
    ...
    def test_multiple_disconnect_reconnect_cycles(self):
        ...
        msgs = [{"jsonrpc": "2.0", "id": i, "method": f"op{i}"} for i in range(1, 6)]
        stdin = self._make_stdin(msgs)
        stdout = io.StringIO()
        stderr = io.StringIO()
    ...
    def test_forward_returns_none_no_response_sent(self):
        ...
        msg = {"jsonrpc": "2.0", "id": 1, "method": "test"}
        responses = self._run_main([msg], forward_side_effect=lambda url, msg, **kw: None)
        self.assertLen(responses, 0)
...
class TestBridgeEntrypoint(EmbodyTestCase):
    ...
    def test_keyboard_interrupt_suppressed(self):
        ...
        with (
            patch.object(bridge, "main", side_effect=KeyboardInterrupt),
            patch.object(sys, "stderr", stderr),
        ):
...
class TestBridgeProjectJsonAndDiscovery(EmbodyTestCase):
    ...
    def test_load_project_config_missing(self):
        ...
        self.assertEqual(bridge.load_project_config("/nonexistent/.embody/envoy.json"), {})
    ...
    def test_parse_build_embedded(self):
        ...
        self.assertEqual(bridge._parse_build("TouchDesigner.2025.32660"), (2025, 32660))
...
class TestBridgeProcessManagement(EmbodyTestCase):
    ...
    def test_find_all_td_pids_filters_self_and_bridges(self, mock_run, mock_is_bridge):
        ...
        self.assertNotIn(my_pid, pids, "Own PID must be excluded")
        self.assertNotIn(67890, pids, "Bridge process PID must be excluded")
    ...
    @patch("envoy_bridge.subprocess.run")
    def test_find_all_td_pids_filters_helper_processes(self, mock_run, mock_cmdline, _mock_bridge):
        ...
        self.assertEqual(
            pids,
            [100],
            "Only the real TD process survives; Web Render helper and CEF child are filtered out",
        )
...
class TestBridgeMetaTools(EmbodyTestCase):
    ...
    def test_restart_td_quit_fails(self):
        ...
        with (
            patch.object(bridge, "find_td_pid", return_value=12345),
            patch.object(bridge, "is_process_alive", return_value=True),
            patch.object(bridge, "quit_td", return_value=(False, "Could not terminate")),
        ):
    ...
    def test_restart_td_success(self):
        ...
        with (
            patch.object(bridge, "find_td_pid", return_value=12345),
            patch.object(bridge, "is_process_alive", return_value=True),
            patch.object(bridge, "quit_td", return_value=(True, "Exited gracefully")),
            patch.object(bridge, "launch_td", return_value=(True, "Launched", 67890)),
            patch.object(bridge, "wait_for_envoy", return_value=True),
    ...
    def test_restart_td_clears_state(self):
        ...
        with (
            patch.object(bridge, "find_td_pid", return_value=12345),
            patch.object(bridge, "is_process_alive", return_value=True),
            patch.object(bridge, "quit_td", return_value=(True, "Exited")),
            patch.object(bridge, "launch_td", return_value=(True, "Launched", 67890)),
...
class TestBridgeHashTools(EmbodyTestCase):
    ...
    def test_added_tool_changes_hash(self):
        ...
        b = [self._tool("create_op", "x"), self._tool("delete_op", "y")]
        self.assertNotEqual(self.hash_tools(a), self.hash_tools(b))

    def test_removed_tool_changes_hash(self):
        a = [self._tool("create_op", "x"), self._tool("delete_op", "y")]
...
class TestBridgeReconcilerTransitions(EmbodyTestCase):
    ...
    def test_fires_on_each_transition_exactly_once(self):
        ...
        p_list = [patch.object(bridge, name, new=impl) for name, impl in patches_spec.items() if hasattr(bridge, name)]
...
class TestBridgeListChangedCapability(EmbodyTestCase):
    ...
    def test_list_changed_true_when_disconnected(self):
        ...
        self.assertEqual(
            ...
            True,
            f"capabilities.tools.listChanged must be True (got {tools_cap!r}). This is bridge v2 step 3 in the plan.",
        )
...
class TestBridgeToolsListCache(EmbodyTestCase):
    ...
    def test_second_tools_list_within_window_uses_cache(self):
        ...
        def forward_counter(url, msg, **kw):
            ...
            return {"jsonrpc": "2.0", "id": msg.get("id"), "result": td_tools_result}
        ...
        tools_list_responses = [
            r for r in responses if "result" in r and isinstance(r["result"], dict) and "tools" in r["result"]
        ]
        self.assertGreaterEqual(
            len(tools_list_responses),
            2,
...
class TestBridgeStdoutSerialization(EmbodyTestCase):
    ...
    def test_concurrent_notify_stdout_produces_valid_jsonl(self):
        ...
        with patch.object(sys, "stdout", capture):
            threads = [threading.Thread(target=hammer, args=(tid,), name=f"notif-{tid}") for tid in range(10)]
            for t in threads:
                t.start()
            for t in threads:
                t.join(timeout=5)
        ...
        # be 1000).
        self.assertEqual(len(lines), 1000, f"Expected 1000 notification lines, got {len(lines)}")
        ...
        self.assertEqual(
            bad_lines,
            [],
            f"{len(bad_lines)} corrupt lines (lock not serializing stdout writes): {bad_lines[:5]}",
        )
