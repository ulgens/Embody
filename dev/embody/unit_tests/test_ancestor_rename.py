...
class TestAncestorRename(EmbodyTestCase):
    def setUp(self):
        ...
        self._embody_snapshot = {p.name for p in embody_root.iterdir() if p.is_dir()} if embody_root.exists() else set()
        ...
        def _stub(title, message, buttons):
            self._captured_dialogs.append({"title": title, "message": message, "buttons": buttons})
    ...
    def _externalize_comp(self, parent, name):
        ...
        old_path = comp.path
        old_rel = self.embody_ext.normalizePath(self.embody_ext.Externalizations[comp.path, "rel_file_path"].val)
    ...
    def _externalize_dat(self, parent, name):
        ...
        old_path = dat.path
        old_rel = self.embody_ext.normalizePath(self.embody_ext.Externalizations[dat.path, "rel_file_path"].val)
    ...
    def test_disk_segment_includes_ext_folder(self):
        ...
        self._add_table_entry(old_prefix + "/comp1", "baseCOMP", "", old_rel)

        rows = [(old_prefix + "/comp1", old_rel, "baseCOMP", "")]

        result = self.embody_ext._handleAncestorRename(old_prefix, new_prefix, rows, ext_folder)
    ...
    def test_disk_segment_works_without_ext_folder(self):
        ...
        self._add_table_entry(old_prefix + "/child1", "baseCOMP", "", old_seg + "/child1/child1.tox")

        rows = [(old_prefix + "/child1", old_seg + "/child1/child1.tox", "baseCOMP", "")]

        result = self.embody_ext._handleAncestorRename(old_prefix, new_prefix, rows, "")  # Empty ext folder
    ...
    def test_phase_a_empty_affected_returns_false(self):
        ...
        result = self.embody_ext._handleAncestorRename("/totally/different", "/totally/renamed", rows, "embody")
    ...
    def test_user_cancel_returns_false(self):
        ...
        result = self.embody_ext._handleAncestorRename(old_prefix, "/cancel_test/renamed", rows, ext_folder)
    ...
    def test_source_dir_not_found_returns_false(self):
        ...
        result = self.embody_ext._handleAncestorRename(old_prefix, "/nosource/renamed", rows, ext_folder)

        self.assertFalse(result, "Should return False when source dir missing")
        # Verify error dialog was shown
        self.assertGreater(len(self._captured_dialogs), 0, "Should show error dialog")
        self.assertIn("Source directory not found", self._captured_dialogs[-1]["message"])
    ...
    def test_target_dir_exists_returns_false(self):
        ...
        result = self.embody_ext._handleAncestorRename(old_prefix, "/conflict/renamed", rows, ext_folder)
    ...
    def test_table_entries_updated_after_rename(self):
        ...
        self.assertTrue(
            self._table_has_path(new_prefix + "/comp1"),
            "Table should have new path for comp1",
        )
        self.assertTrue(
            self._table_has_path(new_prefix + "/comp2"),
        ...
        self.assertNotIn("/scene/", new_rel1, "rel_file should not contain old segment")
    ...
    def test_tdn_strategy_skips_param_update_but_updates_table(self):
        ...
        self.assertTrue(
            self._table_has_path(new_prefix + "/tdn_comp"),
    ...
    def test_returns_false_on_failure(self):
        ...
        result = self.embody_ext._handleAncestorRename(
            ...
            [
                (
                    "/nonexistent/old/c1",
                    "embody/nonexistent/old/c1/c1.tox",
                    "baseCOMP",
                    "",
                )
            ],
    ...
    def test_full_ancestor_rename_with_externalized_ops(self):
        ...
        self.assertFalse(self._table_has_path(old_p1), "Old path should be removed from table")

        # New paths should be present
        new_p1 = new_parent_path + "/full_c1"
        new_p2 = new_parent_path + "/full_c2"
        new_p3 = new_parent_path + "/full_c3"
        self.assertTrue(self._table_has_path(new_p1), "New path should be in table for full_c1")
        self.assertTrue(self._table_has_path(new_p2), "New path should be in table for full_c2")
        self.assertTrue(self._table_has_path(new_p3), "New path should be in table for full_c3")

        # rel_file_path should contain the new parent name
        new_rel = self._get_table_rel_file(new_p1)
        self.assertIn("full_renamed", new_rel, "rel_file should contain renamed parent name")
    ...
    def test_confirmation_dialog_shown(self):
        ...
        self.embody_ext._handleAncestorRename(old_prefix, new_prefix, rows, ext_folder)

        self.assertGreater(len(self._captured_dialogs), 0, "At least one dialog should be shown")
        self.assertEqual(self._captured_dialogs[0]["title"], "Embody -- Ancestor Rename Detected")
