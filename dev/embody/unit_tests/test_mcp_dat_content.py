...
class TestMCPDatContent(EmbodyTestCase):
    ...
    def test_get_dat_content_nonexistent(self):
        result = self.envoy._get_dat_content(op_path="/nonexistent", format="auto")
    ...
    def test_set_dat_content_text(self):
        ...
        result = self.envoy._set_dat_content(op_path=dat.path, text="new content")
        self.assertTrue(result.get("success"))
        self.assertEqual(dat.text, "new content")

    def test_set_dat_content_rows(self):
        dat = self.sandbox.create(tableDAT, "set_rows")
        result = self.envoy._set_dat_content(op_path=dat.path, rows=[["a", "b"], ["1", "2"]])
        self.assertTrue(result.get("success"))
    ...
    def test_set_dat_content_clear_requires_confirm(self):
        ...
        before = dat.numRows
        result = self.envoy._set_dat_content(op_path=dat.path, clear=True)
        self.assertDictHasKey(result, "error")
        self.assertEqual(dat.numRows, before, "guarded clear must not wipe the DAT")
    ...
    def test_set_dat_content_clear_with_confirm(self):
        ...
        dat.appendRow(["existing", "data"])
        result = self.envoy._set_dat_content(op_path=dat.path, clear=True, confirm_wipe=True)
        self.assertTrue(result.get("success"))
    ...
    def test_set_dat_content_nonexistent(self):
        result = self.envoy._set_dat_content(op_path="/nonexistent", text="test")
        self.assertDictHasKey(result, "error")
    ...
    def test_edit_dat_content_unique_match(self):
        ...
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="beta", new_string="BETA")
        self.assertTrue(result.get("success"))
    ...
    def test_edit_dat_content_replace_all(self):
        ...
        dat.text = "x x x"
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="x", new_string="y", replace_all=True)
        self.assertTrue(result.get("success"))
    ...
    def test_edit_dat_content_ambiguous_match_refused(self):
        ...
        dat.text = "dup dup"
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="dup", new_string="z")
        self.assertDictHasKey(result, "error")
    ...
    def test_edit_dat_content_not_found(self):
        ...
        dat.text = "hello"
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="nope", new_string="x")
        self.assertDictHasKey(result, "error")

    def test_edit_dat_content_empty_old_string_refused(self):
        dat = self.sandbox.create(textDAT, "edit_empty")
        dat.text = "content"
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="", new_string="x")
        self.assertDictHasKey(result, "error")
    ...
    def test_edit_dat_content_wipe_requires_confirm(self):
        ...
        dat.text = "all"
        refused = self.envoy._edit_dat_content(op_path=dat.path, old_string="all", new_string="")
        self.assertDictHasKey(refused, "error")
        self.assertEqual(dat.text, "all")
        ok = self.envoy._edit_dat_content(op_path=dat.path, old_string="all", new_string="", confirm_wipe=True)
    ...
    def test_edit_dat_content_table_dat_refused(self):
        ...
        dat.appendRow(["a", "b"])
        result = self.envoy._edit_dat_content(op_path=dat.path, old_string="a", new_string="x")
        self.assertDictHasKey(result, "error")

    def test_edit_dat_content_nonexistent(self):
        result = self.envoy._edit_dat_content(op_path="/nonexistent", old_string="a", new_string="b")
        self.assertDictHasKey(result, "error")
