...
class TestTDNCrashSafety(EmbodyTestCase):
    ...
    def test_A02_atomic_write_replaces_existing(self):
        ...
        self.assertEqual(Path(self._tdn_path).read_text(encoding="utf-8"), new_content)
    ...
    def test_A03_atomic_write_no_temp_residue(self):
        ...
        self.assertEqual(len(tmp_files), 0, f"Residual temp files found: {tmp_files}")
    ...
    def test_A04_atomic_write_readonly_dir_fails_cleanly(self):
        ...
        self.assertFalse(Path(target).exists(), "No partial file should exist")
    ...
    def test_B01_rotate_first_export_creates_bak(self):
        ...
        bak = self.tdn._get_backup_path(self._tdn_path, self._proj_folder, ".bak")
        bak2 = self.tdn._get_backup_path(self._tdn_path, self._proj_folder, ".bak2")
    ...
    def test_B04_rotate_noop_when_no_file(self):
        ...
        bak = self.tdn._get_backup_path(nonexistent, self._proj_folder, ".bak")
    ...
    def test_B06_rotate_creates_parent_dirs(self):
        ...
        bak = self.tdn._get_backup_path(deep_tdn, self._proj_folder, ".bak")
    ...
    def test_C01_validate_valid_tdn(self):
        ...
        Path(self._tdn_path).write_text(_make_valid_tdn_json(), encoding="utf-8")
    ...
    def test_D01_safe_write_full_cycle(self):
        ...
        result = self.tdn._safe_write_tdn(self._tdn_path, v2, self._proj_folder)
        self.assertTrue(result.get("success"))
        # Current file has new content
        self.assertEqual(Path(self._tdn_path).read_text(encoding="utf-8"), v2)
    ...
    def test_D02_safe_write_first_export_no_backup(self):
        ...
        content = _make_valid_tdn_json()
        result = self.tdn._safe_write_tdn(new_path, content, self._proj_folder)
        self.assertTrue(result.get("success"))
        self.assertEqual(Path(new_path).read_text(encoding="utf-8"), content)
        bak = self.tdn._get_backup_path(new_path, self._proj_folder, ".bak")
        self.assertFalse(bak.is_file(), "No backup for first export")
    ...
    def test_D03_safe_write_restores_on_validation_failure(self):
        ...
        result = self.tdn._safe_write_tdn(self._tdn_path, bad_content, self._proj_folder)
        # Should report error
    ...
    def test_F01_1000_operator_export_roundtrip(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True, output_file=tdn_file)
        ...
        for c in list(self.sandbox.children):
            c.destroy()
        imp = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn_doc, clear_first=True)
        ...
        self.assertGreaterEqual(created, expected - 20, f"Expected ~{expected} ops created, got {created}")
    ...
    def test_F02_1000_operator_backup_rotation(self):
        ...
        tdn_file = os.path.join(self._temp_dir, "stress.tdn")
        # First export
        r1 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True, output_file=tdn_file)
        ...
        r2 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True, output_file=tdn_file)
        ...
        bak_temp = self.tdn._get_backup_path(tdn_file, self._proj_folder, ".bak")
        found_bak = None
    ...
    def test_F03_1000_operator_corrupt_and_rollback(self):
        ...
        r = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True, output_file=tdn_file)
        ...
        for c in list(self.sandbox.children):
            c.destroy()
        imp = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=valid_tdn, clear_first=True)
    ...
    def test_F04_1000_operator_partial_import_recovery(self):
        ...
        for c in list(self.sandbox.children):
            c.destroy()
        imp_partial = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=partial_tdn, clear_first=True)
        ...
        imp_full = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=full_tdn, clear_first=True)
    ...
    def test_F05_deep_nesting_20_levels(self):
        ...
        self.assertGreaterEqual(depth, 19, f"Expected 19+ levels in TDN JSON, got {depth}")
        # Clear and reimport
        for c in list(self.sandbox.children):
            try:
                if c.dock is not None:
        ...
        for c in list(self.sandbox.children):
            c.destroy()
        imp = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn, clear_first=True)
        self.assertTrue(imp.get("success"), f"Import failed: {imp}")
        # Verify import created all operators (40 total: 20 COMPs + 20 TOPs)
        created = imp.get("created_count", 0)
        self.assertGreaterEqual(created, 38, f"Expected 38+ operators for 20 nesting levels, got {created}")
    ...
    def test_F06_large_dat_content_roundtrip(self):
        ...
        for i in range(10):
            ...
            lines = [f"DAT {i} line {j}: " + "x" * 80 for j in range(total_lines)]
            d.text = "\n".join(lines)
        # Export
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        self.assertTrue(result.get("success"))
    ...
    def test_F07_mixed_failure_modes(self):
        ...
        result_v1 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        ...
        result_v2 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        ...
        result_v3 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        ...
        bak = self.tdn._get_backup_path(tdn_file, self._proj_folder, ".bak")
        self.assertTrue(bak.is_file(), ".bak should exist")
        bak.write_text("ALSO CORRUPT", encoding="utf-8")
        # .bak2 should still be valid (v1 content)
        bak2 = self.tdn._get_backup_path(tdn_file, self._proj_folder, ".bak2")
        self.assertTrue(bak2.is_file(), ".bak2 should exist")
        v = self.tdn._validate_tdn_file(str(bak2))
        self.assertTrue(v.get("valid"), f".bak2 should be valid: {v}")
        # Recover from .bak2
        recovered_tdn = self.tdn.tdn_load(bak2.read_text(encoding="utf-8"))
        imp = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=recovered_tdn, clear_first=True)
        self.assertTrue(imp.get("success"), f"Recovery from .bak2 failed: {imp}")
        created = imp.get("created_count", 0)
        self.assertGreater(created, 100, f"Should recover substantial ops from .bak2, got {created}")
