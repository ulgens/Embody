...
class TestLaunchAIClient(EmbodyTestCase):
    ...
    def test_A08_cli_names_are_barewords(self):
        ...
        for token, spec in self.embody_ext._AICLIENT_LAUNCH.items():
            if spec["kind"] == "terminal":
                cli = spec["cli"]
                self.assertTrue(
                    cli and " " not in cli and "/" not in cli,
    ...
    def test_B01_bogus_cli_resolves_to_none(self):
        ...
        self.assertIsNone(self.embody_ext._resolveCliAbs("no_such_cli_embody_xyz"))
    ...
    def test_B02_resolve_returns_abspath_or_none(self):
        ...
        self.assertTrue(result is None or (isinstance(result, str) and Path(result).is_absolute()))
    ...
    def test_C01_script_has_shebang_cd_and_exec(self):
        ...
        body = self.embody_ext._buildTerminalScript(self._temp_dir, "claude", "/abs/bin/claude")
    ...
    def test_C02_absent_cli_gets_guard_and_login_shell(self):
        ...
        body = self.embody_ext._buildTerminalScript(self._temp_dir, "gemini", None)
    ...
    def test_C03_single_quote_in_path_is_escaped(self):
        ...
        body = self.embody_ext._buildTerminalScript(Path("/tmp/it's a dir"), "claude", "/abs/claude")
    ...
    def test_C04_install_hint_echoed_when_absent(self):
        ...
        body = self.embody_ext._buildTerminalScript(
            self._temp_dir, "gemini", None, install="npm install -g @google/gemini-cli"
        )
    ...
    def test_D01_missing_editor_returns_false(self):
        ...
        """
        result = self.embody_ext._launchEditor(self._temp_dir, "NoSuchEditor_Embody_XYZ")
    ...
    def test_E02_launch_env_strips_injected_dyld_and_node(self):
        ...
        added = [k for k in ("DYLD_INSERT_LIBRARIES", "NODE_OPTIONS") if k not in os.environ]
        for k in added:
            os.environ[k] = "x"
        try:
            env = self.embody_ext._launchEnv()
    ...
    def test_F02_windows_terminal_allocates_new_console(self):
        ...
        self.assertTrue(result)
        self.assertEqual(kwargs.get("creationflags"), getattr(subprocess, "CREATE_NEW_CONSOLE", 0x10))
    ...
    def test_G01_windows_script_abs_cli_quotes_path_without_where_guard(self):
        ...
        body = self.embody_ext._buildTerminalScriptWin(
            Path("C:/Users/O'Neil/My Project"),
            "claude",
    ...
    def test_G03_windows_script_uses_crlf_exclusively(self):
        ...
        body = self.embody_ext._buildTerminalScriptWin(self._temp_dir, "gemini", None)
        self.assertIn("\r\n", body)
        self.assertFalse(
            "\n" in body.replace("\r\n", ""),
    ...
    def test_H01_launch_editor_failure_consumes_dialog_seed(self):
        ...
        try:
            ...
            with (
                mock.patch.object(
                    type(self.embody_ext),
                    "_findProjectRoot",
                    return_value=self._temp_dir,
                ),
                mock.patch.object(type(self.embody_ext), "_launchEditor", return_value=False),
            ):
                self.embody_ext.LaunchAIClient()
            self.assertIsNone(op.Embody.fetch("_smoke_test_responses", None, search=False))
        finally:
    ...
    def test_H02_launch_terminal_failure_consumes_dialog_seed(self):
        ...
        try:
            ...
            with (
                ...
                mock.patch.object(type(self.embody_ext), "_launchTerminal", return_value=False),
            ):
                self.embody_ext.LaunchAIClient()
            self.assertIsNone(op.Embody.fetch("_smoke_test_responses", None, search=False))
        finally:
    ...
    def test_H03_no_launcher_dialog_uses_selected_menu_label(self):
        ...
        try:
            ...
            with (
                mock.patch.object(type(self.embody_ext), "_AICLIENT_LAUNCH", table),
                mock.patch.object(
                    type(self.embody_ext),
                    "_findProjectRoot",
                    return_value=self._temp_dir,
                ),
                mock.patch.object(type(self.embody_ext), "_messageBox", capture_message),
            ):
                self.embody_ext.LaunchAIClient()
