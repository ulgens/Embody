...
class TestTDNReconstruction(EmbodyTestCase):
    ...
    def _roundTrip(self, parent):
        ...
        """
        orig = self.tdn.ExportNetwork(root_path=parent.path, include_dat_content=True)
        ...
        # Import
        result = self.tdn.ImportNetwork(target_path=parent.path, tdn=orig_tdn, clear_first=False)
        self.assertTrue(result.get("success"), f"Import failed: {result}")

        # Re-export
        reimp = self.tdn.ExportNetwork(root_path=parent.path, include_dat_content=True)
    ...
    def _verifyNetworkFidelity(
        self,
        original_ops,
        reimported_ops,
        orig_type_defaults=None,
        reimp_type_defaults=None,
    ):
        ...
        self.assertEqual(set(orig_by_name.keys()), set(reimp_by_name.keys()), "Operator names differ")
        ...
        for name in orig_by_name:
            ...
            # Type
            self.assertEqual(orig["type"], reimp["type"], f"{name}: type mismatch")
            ...
            for pname in orig_params:
                self.assertIn(pname, reimp_params, f"{name}: missing param {pname}")
                self._assertParamEqual(orig_params[pname], reimp_params[pname], f"{name}.{pname}")
            ...
            self.assertEqual(orig_inputs, reimp_inputs, f"{name}: inputs mismatch")

            # Position
            orig_pos = orig.get("position", [0, 0])
            reimp_pos = reimp.get("position", [0, 0])
            self.assertEqual(orig_pos, reimp_pos, f"{name}: position mismatch")
            ...
            # Comment
            self.assertEqual(orig.get("comment"), reimp.get("comment"), f"{name}: comment mismatch")
            ...
            self.assertEqual(orig_tags, reimp_tags, f"{name}: tags mismatch")

            # Storage
            orig_storage = orig.get("storage", {})
            reimp_storage = reimp.get("storage", {})
            self.assertEqual(
                set(orig_storage.keys()),
                set(reimp_storage.keys()),
            ...
            if "dat_content" in orig:
                self.assertIn("dat_content", reimp, f"{name}: missing dat_content")
                self.assertEqual(
                    orig["dat_content"],
                    reimp["dat_content"],
            ...
            # Children (recursive)
            if "children" in orig:
                self.assertIn("children", reimp, f"{name}: missing children")
    ...
    def test_A08_operator_count_preserved(self):
        ...
        self.assertEqual(len(orig_tdn["operators"]), len(reimp_tdn["operators"]))
    ...
    def test_A10_complex_network_roundtrip(self):
        ...
        self._verifyNetworkFidelity(
            orig_tdn["operators"],
            reimp_tdn["operators"],
            orig_tdn.get("type_defaults"),
            reimp_tdn.get("type_defaults"),
        )
    ...
    def test_A11_idempotent_double_roundtrip(self):
        ...
        self._verifyNetworkFidelity(
            reimp1_tdn["operators"],
            reimp2_tdn["operators"],
            reimp1_tdn.get("type_defaults"),
            reimp2_tdn.get("type_defaults"),
        )
    ...
    def test_B06_bind_mode(self):
        ...
        seed_val = entry["parameters"].get("seed", "")
        self.assertTrue(str(seed_val).startswith("~"), f"Expected bind prefix, got: {seed_val}")
    ...
    def test_G06_content_excluded_toggle(self):
        ...
        orig = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=False)
        entry = orig["tdn"]["operators"][0]
    ...
    def test_H03_wide_and_deep(self):
        ...
        self._roundTrip(self.sandbox)
        for i in range(5):
            self.assertIsNotNone(self.sandbox.op(f"branch_{i}/child/leaf"))
    ...
    def test_I02_stripping_from_individuals(self):
        ...
        if "noiseTOP" in td and "seed" in td["noiseTOP"].get("parameters", {}):
            ...
            for entry in tdn["operators"]:
                params = entry.get("parameters", {})
                self.assertNotIn("seed", params, f"{entry['name']} still has seed after hoisting")
    ...
    def test_I08_flags_hoisted_to_type_defaults(self):
        ...
        for entry in tdn["operators"]:
            self.assertNotIn("flags", entry, f"{entry['name']} should not have per-op flags")
    ...
    def test_I09_size_hoisted_to_type_defaults(self):
        ...
        for entry in tdn["operators"]:
            self.assertNotIn("size", entry, f"{entry['name']} should not have per-op size")
    ...
    def test_I10_color_hoisted_to_type_defaults(self):
        ...
        for entry in tdn["operators"]:
            self.assertNotIn("color", entry, f"{entry['name']} should not have per-op color")
    ...
    def test_I11_tags_hoisted_to_type_defaults(self):
        ...
        self.assertEqual(sorted(td["noiseTOP"]["tags"]), ["audio", "generator"])
        for entry in tdn["operators"]:
            self.assertNotIn("tags", entry, f"{entry['name']} should not have per-op tags")
    ...
    def test_K08_clear_first_removes_old(self):
        ...
        orig = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        # Add another op after export
        self.sandbox.create(waveCHOP, "extra")
        # Import with clear_first
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=orig["tdn"], clear_first=True)
        self.assertTrue(result.get("success"))
        names = self._getOpNames(self.sandbox)
    ...
    def test_K09_target_custom_pars_survive_clear_first(self):
        ...
        self.assertIn("custom_pars", tdn_doc, "TDN should include target COMP custom_pars")

        # Import with clear_first (simulates reconstruction)
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn_doc, clear_first=True)
        self.assertTrue(result.get("success"))
    ...
    def test_K15_diverse_comp_types_roundtrip(self):
        ...
        for ct in comp_types:
            ...
            tdn = result["tdn"]
            self.assertEqual(tdn.get("type"), ct, f"type field mismatch for {ct}")
    ...
    def test_K16_target_comp_flags_roundtrip(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn, clear_first=True)
        self.assertTrue(result.get("success"))
    ...
    def test_K18_target_comp_storage_roundtrip(self):
        ...
        self.assertEqual(self.sandbox.fetch("config", search=False), {"nested": True})
    ...
    def test_M01_corrupted_json(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn={"operators": "not_a_list"})
    ...
    def test_M02_missing_operators_key(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn={"format": "tdn", "version": "1.0"})
    ...
    def test_M03_unknown_op_type(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn)
    ...
    def test_M04_missing_connection_source(self):
        ...
        tdn = {
            ...
            "version": "1.0",
            "operators": [
                {"name": "dst", "type": "levelTOP", "inputs": ["nonexistent_source"]},
            ],
        }
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn)
            ...
            "operators": [
                {
                    "name": "c",
                    "type": "baseCOMP",
                    "custom_pars": {"Test": [{"name": "X", "style": "TotallyFakeStyle"}]},
                },
            ],
        }
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn)
    ...
    def test_M09_operators_as_direct_list(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=ops)
        self.assertTrue(result.get("success"))
    ...
    def test_P02_external_tdn_comps_still_included(self):
        ...
        try:
            ...
            self.assertTrue(found, f"{tdn_path} should be included in TDN strategy comps")
        finally:
    ...
    def test_P03_embody_help_specifically_excluded(self):
        ...
        for comp_path, _ in comps:
            self.assertNotEqual(comp_path, help_path, f"{help_path} must never be stripped")
    ...
    def test_P04_continuity_check_skips_pure_tdn_children(self):
        ...
        try:
            ...
            self.embody_ext.checkOpsForContinuity(self.embody_ext.ExternalizationsFolder)

            # The child's row must still exist (no own strategy -> skipped)
            self.assertTrue(
                self._tableHasRow(child_path),
    ...
    def test_P06_nested_pure_tdn_children_also_skipped(self):
        ...
        self._addTableRow(deep_path, "text", "", f"embody/{tdn_comp.name}/inner/deep_tracked.txt")
    ...
    def test_P07_strip_restore_preserves_children(self):
        ...
        export_result = self.tdn.ExportNetwork(root_path=tdn_comp.path, include_dat_content=True)
    ...
    def test_P08_strip_restore_then_continuity_check_safe(self):
        ...
        self._addTableRow(tdn_path, "base", "tdn", f"embody/{tdn_comp.name}.tdn")
        self._addTableRow(child_path, "text", "", f"embody/{tdn_comp.name}/tracked_dat.txt")

        try:
            # Phase 1: Export (like Update does)
            export_result = self.tdn.ExportNetwork(root_path=tdn_path, include_dat_content=True)
            self.assertTrue(export_result.get("success"))
            tdn_doc = export_result["tdn"]
    ...
    def test_P10_continuity_check_during_strip_window(self):
        ...
        self._addTableRow(tdn_path, "base", "tdn", f"embody/{tdn_comp.name}.tdn")
        self._addTableRow(child_path, "text", "", f"embody/{tdn_comp.name}/victim.txt")
    ...
    def test_Q01_int_storage_roundtrip(self):
        ...
        c.store("count", 42)
        self._roundTrip(self.sandbox)
        self.assertEqual(self.sandbox.op("c").fetch("count", None, search=False), 42)
    ...
    def test_Q03_string_storage_roundtrip(self):
        ...
        self._roundTrip(self.sandbox)
        self.assertEqual(self.sandbox.op("c").fetch("label", None, search=False), "hello world")
    ...
    def test_Q04_bool_storage_roundtrip(self):
        ...
        c.store("active", True)
        self._roundTrip(self.sandbox)
        self.assertTrue(self.sandbox.op("c").fetch("active", None, search=False))
    ...
    def test_Q11_skip_storage_keys_not_exported(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
    ...
    def test_Q13_multiple_ops_different_storage(self):
        ...
        b.store("val", "beta")
        self._roundTrip(self.sandbox)
        self.assertEqual(self.sandbox.op("a").fetch("val", None, search=False), "alpha")
        self.assertEqual(self.sandbox.op("b").fetch("val", None, search=False), "beta")
    ...
    def test_Q14_nested_comp_storage_roundtrip(self):
        ...
        inner.store("depth", 2)
        self._roundTrip(self.sandbox)
        self.assertEqual(self.sandbox.op("outer/inner").fetch("depth", None, search=False), 2)
    ...
    def test_Q15_storage_on_non_comp(self):
        ...
        self._roundTrip(self.sandbox)
        self.assertEqual(self.sandbox.op("n").fetch("seed_offset", None, search=False), 123)
    ...
    def test_Q16_embed_dats_in_tdn_preserved(self):
        ...
        self._roundTrip(self.sandbox)
        result = self.sandbox.op("c").fetch("embed_dats_in_tdn", None, search=False)
    ...
    def test_Q17_storage_through_reconstruction(self):
        ...
        self._simulateReconstruction(self.sandbox)
        result = self.sandbox.op("c").fetch("persist_me", None, search=False)
    ...
    def test_R04_dock_missing_target_warns(self):
        ...
        tdn_data = {
            ...
            "operators": [{"name": "lonely", "type": "infoDAT", "dock": "nonexistent"}],
        }
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=tdn_data, clear_first=True)
        self.assertTrue(result.get("success"))
    ...
    def test_T01_geocomp_children_survive_roundtrip(self):
        ...
        orig = self.tdn.ExportNetwork(root_path=geo.path, include_dat_content=True)
        self.assertTrue(orig.get("success"), f"Export failed: {orig}")
        tdn = orig["tdn"]

        # Verify torus1 is in the export (it has non-default params)
        op_names = [o["name"] for o in tdn["operators"]]
        self.assertIn("torus1", op_names, "Customized torus1 should be in TDN export")

        # Clear and reimport
        result = self.tdn.ImportNetwork(target_path=geo.path, tdn=tdn, clear_first=True)
        self.assertTrue(result.get("success"), f"Import failed: {result}")
        ...
        self.assertEqual(
            int(restored_torus.par.rows),
            30,
            "torus1 rows param should survive roundtrip",
        )
        self.assertEqual(
            int(restored_torus.par.cols),
            30,
    ...
    def test_T02_geocomp_replaced_children_roundtrip(self):
        ...
        self.assertNotIn("torus1", op_names, "Deleted torus1 should not be in export")
        ...
        self.assertIsNone(geo.op("torus1"), "torus1 should NOT reappear after import")
        ...
        self.assertAlmostEqual(
            float(restored_xform.par.tx),
            2.5,
            places=3,
            msg="xform tx param should survive roundtrip",
        )
        # Verify connection survived
        self.assertTrue(len(restored_xform.inputs) > 0, "xform1 should have an input connection")
        self.assertEqual(restored_xform.inputs[0].name, "box1", "xform1 input should be box1")
    ...
    def test_T03_geocomp_builtin_params_roundtrip(self):
        ...
        self.assertIn("parameters", tdn, "Geo COMP non-default built-in params should be in TDN")
        ...
        self.assertAlmostEqual(float(geo.par.tx.eval()), 3.0, places=3, msg="tx should be restored")
        self.assertAlmostEqual(float(geo.par.ty.eval()), -1.5, places=3, msg="ty should be restored")
        self.assertAlmostEqual(float(geo.par.sx.eval()), 2.0, places=3, msg="sx should be restored")
    ...
    def test_T04_geocomp_strip_restore_cycle(self):
        ...
        for c in list(geo.findChildren(depth=1, includeUtility=True)):
            c.destroy()
        self.assertEqual(len(geo.children), 0, "Children should be stripped")

        # Root params should still be intact (strip only removes children)
        self.assertAlmostEqual(
            float(geo.par.tx.eval()),
            5.0,
            places=3,
        ...
        result = self.tdn.ImportNetwork(target_path=geo.path, tdn=tdn, clear_first=True)
        self.assertTrue(result.get("success"), f"Post-save restore failed: {result}")

        # Verify everything survived
        restored_torus = geo.op("torus1")
        self.assertIsNotNone(restored_torus, "torus1 should be restored after save cycle")
        self.assertEqual(
            int(restored_torus.par.rows),
            20,
    ...
    def test_T05_geocomp_default_children_skipped_then_lost(self):
        ...
        for c in list(geo.findChildren(depth=1, includeUtility=True)):
            c.destroy()
        self.assertIsNone(geo.op("torus1"), "torus1 should be gone after strip")
    ...
    def test_T06_geocomp_material_reference_roundtrip(self):
        ...
        self.assertAlmostEqual(
            float(restored_mat.par.colorr.eval()),
            1.0,
            places=3,
            msg="MAT color should survive roundtrip",
        )
    ...
    def test_T07_geocomp_sop_render_display_flags(self):
        ...
        self.assertTrue(restored_box.render, "box1 should have render flag after roundtrip")
        self.assertTrue(restored_box.display, "box1 should have display flag after roundtrip")
        if restored_torus:
            self.assertFalse(
                restored_torus.render,
                "torus1 should NOT have render flag after roundtrip",
            )
            self.assertFalse(
                restored_torus.display,
    ...
    def test_T09_light_comp_roundtrip(self):
        ...
        self.assertAlmostEqual(float(restored.par.dimmer.eval()), 0.5, places=3)
    ...
    def test_T10_camera_tz_zero_roundtrip(self):
        ...
        cam.par.tz = 0.0
        self.assertAlmostEqual(float(cam.par.tz.eval()), 0.0, places=3, msg="Pre-export: tz should be 0")
        ...
        cam_def = next((o for o in tdn["operators"] if o["name"] == "cam_tz0"), None)
        self.assertIsNotNone(cam_def, "cam_tz0 should be in export")
        self.assertIn("parameters", cam_def, "cam_tz0 should have non-default parameters")
        self.assertIn(
            "tz",
            cam_def["parameters"],
    ...
    def test_T11_light_tz_zero_roundtrip(self):
        ...
        light_def = next((o for o in tdn["operators"] if o["name"] == "light_tz0"), None)
        self.assertIsNotNone(light_def)
        self.assertIn(
            "tz",
            light_def.get("parameters", {}),
    ...
    def test_T12_render_resolution_roundtrip(self):
        ...
        render_def = next((o for o in tdn["operators"] if o["name"] == "render_res"), None)
        self.assertIsNotNone(render_def)
        self.assertIn(
            "resolutionw",
            render_def.get("parameters", {}),
        ...
        self.assertEqual(
            int(restored.par.resolutionw.eval()),
            256,
            msg="resolutionw=256 must survive roundtrip",
        )
        self.assertEqual(
            int(restored.par.resolutionh.eval()),
            256,
    ...
    def test_T13_divergent_default_not_false_positive(self):
        ...
        cam_def = next((o for o in tdn["operators"] if o["name"] == "cam_default"), None)
        self.assertIsNotNone(cam_def)
        # tz=5 is the creation default, so it should NOT be in the export
        params = cam_def.get("parameters", {})
        self.assertNotIn("tz", params, "tz at creation default (5) should NOT be exported")
    ...
    def test_U01_nested_tdn_children_skipped(self):
        ...
        try:
            ...
            result = self.tdn.ImportNetwork(target_path=parent.path, tdn=parent_tdn, clear_first=False)
            self.assertTrue(result.get("success"), f"Import failed: {result}")

            # child_inner COMP shell should exist
            restored_child = parent.op("child_inner")
            self.assertIsNotNone(restored_child, "child COMP shell must be created")

            # But its children should NOT have been imported from the parent TDN
            self.assertEqual(
                len(restored_child.children),
                0,
                "child COMP children must be skipped - its own TDN is source of truth",
            )
        finally:
            self._removeTableRow(child_path)
    ...
    def test_U03_depth_sorting_in_getTDNStrategyComps(self):
        ...
        try:
            ...
            self.assertEqual(len(test_paths), 3, "All three test paths must be present")
    ...
    def test_U04_deeply_nested_skip(self):
        ...
        orig = self.tdn.ExportNetwork(root_path=grandparent.path, include_dat_content=True)
        self.assertTrue(orig.get("success"))
        gp_tdn = orig["tdn"]
        ...
        try:
            ...
            result = self.tdn.ImportNetwork(target_path=grandparent.path, tdn=gp_tdn, clear_first=False)
            self.assertTrue(result.get("success"))
            ...
            self.assertIsNotNone(restored_child, "child COMP shell must exist")
            self.assertEqual(
                len(restored_child.children),
                0,
    ...
    def test_V03_native_widget_exports_with_children(self):
        ...
        self.assertFalse(
            btn_entry.get("palette_clone"),
            "Native widget COMPs must NOT carry the palette_clone flag (reserved for true user palette clones)",
        )
        self.assertIn(
            "children",
            btn_entry,
            "Children must be exported in full for native widgets so user customizations round-trip intact",
        )
        self.assertGreater(
            len(btn_entry["children"]),
            0,
            "buttonCOMP from /sys/TDTox/defaultCOMPs/ ships with children -- they must appear in the export",
        )
    ...
    def test_V04_buttontype_matches_default_preserved(self):
        ...
        self.assertIsNotNone(restored)
        self.assertEqual(
            str(restored.par.buttontype.eval()),
            orig_val,
    ...
    def test_V05_clone_source_diff_in_export(self):
        ...
        td = tdn.get("type_defaults", {}).get("buttonCOMP", {}).get("parameters", {})
        all_params = dict(td)
        all_params.update(per_op)
        self.assertIn(
            "buttontype",
            all_params,
            "buttontype must be exported even when it matches p.default (it differs from clone source)",
        )
    ...
    def test_V07_clone_reference_captured_in_export(self):
        ...
        self.assertIn(
            "clone",
            per_op,
            "Clone reference must be captured so the link to /sys/ survives round-trip",
        )
        self.assertNotIn(
            "enablecloning",
            per_op,
            "enablecloning matches its default for fresh widgets and should be omitted by the default-skipping export",
        )
    ...
    def test_V08_old_tdn_clone_params_skipped_on_import(self):
        ...
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=crafted_tdn, clear_first=False)
        self.assertTrue(result.get("success"))
        restored = self.sandbox.op("btn8")
        self.assertIsNotNone(restored)
        # clone should point to /sys/ (auto-set), not NONEXISTENT
        clone_op = restored.par.clone.eval()
        self.assertIsNotNone(clone_op, "Clone should resolve to /sys/ default, not NONEXISTENT")
    ...
    def test_V10_reconstruction_preserves_palette_clone(self):
        ...
        restored = self.sandbox.op(actual_name)
        self.assertIsNotNone(restored)
        self.assertGreater(len(restored.children), 0, "Children must be recreated")
        self.assertEqual(
            str(restored.par.buttontype.eval()),
            orig_buttontype,
    ...
    def test_V12_mixed_network_no_interference(self):
        ...
        for name in (btn_name, sl_name, "base30", "null30"):
            self.assertFalse(
                by_name.get(name, {}).get("palette_clone", False),
    ...
    def test_W02_glsl_top_info_dat_read_only(self):
        ...
        self.assertTrue(info_def.get("dat_read_only"), "Info DAT must have dat_read_only=True")
        self.assertNotIn("dat_content", info_def, "Read-only DAT must not have dat_content exported")
    ...
    def test_W03_timer_callbacks_roundtrip(self):
        ...
        self.assertIsNotNone(restored, "callbacks companion must survive round-trip")
        self.assertIn(
            "custom callbacks",
            restored.text,
    ...
    def test_W04_companion_no_duplicate_after_roundtrip(self):
        ...
        callback_variants = [n for n in children if n.startswith("timer1_callbacks")]
        self.assertEqual(
            len(callback_variants),
            1,
    ...
    def test_W05_glsl_pop_companion_roundtrip(self):
        ...
        self.assertIsNotNone(restored, "compute companion must survive round-trip")
        self.assertIn(
        ...
        for op_def in orig["tdn"].get("operators", []):
            if op_def.get("dat_read_only"):
                self.assertNotIn(
                    "dat_content",
                    op_def,
                    f'Read-only DAT "{op_def["name"]}" must not have dat_content exported',
                )
    ...
    def test_W07_glsl_multi_top_roundtrip(self):
        ...
        self.assertEqual(len(info_variants), 1, f"Expected 1 info DAT, got: {info_variants}")
    ...
    def test_W08_glsl_advanced_pop_roundtrip(self):
        ...
        children = [c.name for c in self.sandbox.children]
        compute_variants = [n for n in children if n.startswith("glsl1_compute")]
        self.assertEqual(len(compute_variants), 1, f"Expected 1 compute DAT, got: {compute_variants}")
    ...
    def test_W09_glsl_copy_pop_roundtrip(self):
        ...
        for suffix in ["ptCompute", "primCompute", "vertCompute", "info"]:
            ...
            self.assertLessEqual(len(variants), 1, f"Expected at most 1 {name}, got: {variants}")
    ...
    def test_W10_ramp_top_keys_roundtrip(self):
        ...
        self.assertIsNotNone(restored)
        self.assertEqual(restored.numRows, 3, "Keys table must have header + 2 key rows")

        children = [c.name for c in self.sandbox.children]
        keys_variants = [n for n in children if n.startswith("ramp1_keys")]
        self.assertEqual(len(keys_variants), 1, f"Expected 1 keys DAT, got: {keys_variants}")
    ...
    def test_W11_script_chop_callbacks_roundtrip(self):
        ...
        children = [c.name for c in self.sandbox.children]
        cb_variants = [n for n in children if n.startswith("script1_callbacks")]
        self.assertEqual(len(cb_variants), 1, f"Expected 1 callbacks DAT, got: {cb_variants}")
