...
class TestInstallManifest(EmbodyTestCase):
    ...
    def test_relpath_outside_root_is_absolute(self):
        ...
        self.assertTrue(os.path.isabs(rel), "a path outside the manifest root must stay absolute")
    ...
    def test_load_tolerates_partial_manifest(self):
        ...
        with open(os.path.join(self.d, ".embody", "manifest.json"), "w", encoding="utf-8") as f:
    ...
    def test_created_file_outside_root_stored_absolute(self):
        ...
        self.assertTrue(os.path.isabs(stored), "a created file outside the root records absolute")

    # ----- appended files (block vs json_key) -----------------------------

    def test_appended_block_recorded(self):
        self.ext._manifestRecordAppendedFile(self.d, os.path.join(self.d, ".gitignore"), "# Embody / Envoy")
    ...
    def test_git_config_list_and_str_deduped(self):
        self.ext._manifestRecordGitConfig(self.d, ["diff.tdn.textconv", "diff.tdn.cachetextconv"])
        self.ext._manifestRecordGitConfig(self.d, "diff.tdn.textconv")  # str + dedup
        self.assertEqual(self._load()["git_config"], ["diff.tdn.textconv", "diff.tdn.cachetextconv"])
