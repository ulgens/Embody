...
class TestServerLifecycle(EmbodyTestCase):
    ...
    def test_execute_operation_unknown(self):
        result = self.envoy._execute_operation("totally_unknown_operation_xyz", {})
    ...
    def test_execute_operation_with_params(self):
        ...
        result = self.envoy._execute_operation("get_op", {"op_path": comp.path})
        self.assertNotIn("error", result)

    def test_execute_operation_handler_error(self):
        # Passing invalid params should return error, not crash
        result = self.envoy._execute_operation("get_op", {"op_path": "/nonexistent_lifecycle_test"})
        self.assertDictHasKey(result, "error")
    ...
    def test_log_cursor_values_are_ints(self):
        ...
        try:
            self.assertIsInstance(self.envoy._log_cursors["lifecycle-cursor-probe"], int)
        finally:
...
class TestAsyncBootstrap(EmbodyTestCase):
    ...
    def test_async_methods_exist(self):
        for name in ("_beginAsyncBootstrap", "_pollBootstrap", "_continueStart"):
            self.assertTrue(hasattr(self.envoy, name), f"EnvoyExt must define {name}")

    def test_embody_bootstrap_pieces_exist(self):
        emb = self.embody.ext.Embody
        for name in ("_venvPaths", "_environmentNeedsInstall", "_installDependencies"):
            self.assertTrue(hasattr(emb, name), f"EmbodyExt must define {name}")
    ...
    def test_mcp_update_marshal_drains_and_logs(self):
        ...
        try:
            ...
            emb.Log = original
        self.assertEqual(len(captured), 1, f"drain must log exactly once, got {captured!r}")
        self.assertEqual(captured[0][1], "WARNING")
        self.assertIsNone(
            getattr(emb, "_mcp_update_notice", "unset"),
