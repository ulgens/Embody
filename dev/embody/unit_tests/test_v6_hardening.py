...
class TestTDNLoadMalformedJSON(EmbodyTestCase):
    ...
    def test_malformed_brace_json_raises_not_degraded(self):
        ...
        self.assertTrue(
            raised,
            f"tdn_load must raise on a doubly-malformed brace doc, not return a lenient value (got {result!r})",
        )
    ...
    def test_malformed_bracket_json_raises_not_degraded(self):
        ...
        try:
            ...
            raised = True
        self.assertTrue(raised, "tdn_load must raise on a doubly-malformed bracket doc")
...
class TestTDNBlockScalarFileIO(EmbodyTestCase):
    ...
    def _export_dat(self, dat, fp):
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, output_file=fp, include_dat_content=True)
    ...
    def test_single_trailing_newline_clip_chomp(self):
        ...
        result = self.tdn.ImportNetworkFromFile(file_path=fp, target_path=target.path)
    ...
    def test_two_trailing_newlines_keep_chomp(self):
        ...
        self.assertIn(b"dat_content: |+", raw, "two trailing newlines must use keep chomp (|+)")
...
class TestTDNBoilerplateNegativeGuards(EmbodyTestCase):
    ...
    def _assert_not_omitted(self, dat):
        ...
        self.assertIsNotNone(result, f"{dat.name}: must NOT be omitted (returned None)")
        self.assertIn("dat_content", result, f"{dat.name}: dat_content must be preserved")
        self.assertEqual(
            result["dat_content"],
            dat.text,
    ...
    def test_default_text_undocked_compute_name_not_omitted(self):
        ...
        self.assertIsNone(dat.dock, "precondition: this DAT must be undocked")
...
class TestTDNTextconvDegrade(EmbodyTestCase):
    def _load_textconv(self):
        ...
        if not os.path.isfile(fp):
            return None
        spec = importlib.util.spec_from_file_location("v6h_tdn_textconv", fp)
...
class TestPOPSequenceResolution(EmbodyTestCase):
    ...
    def test_pop_comb_subscript_none_but_iter_finds_it(self):
        ...
        self.assertIsNotNone(resolved, "_getSequenceByName must resolve the comb sequence by iteration")
    ...
    def test_getSequenceByName_unknown_returns_none(self):
        ...
        self.assertIsNone(self.tdn._getSequenceByName(pop, "nonexistent_seq_xyz"))
    ...
    def test_custom_sequence_block_par_resolves(self):
        ...
        par = self.tdn._resolveSequenceBlockPar(comp, seq, block, 0, "Itemlabel")
        self.assertIsNotNone(par, "_resolveSequenceBlockPar must resolve the custom block par")
    ...
    def test_resolveSequenceBlockPar_unknown_base_returns_none(self):
        ...
        comp.seq.Items.numBlocks = 1
        seq = self.tdn._getSequenceByName(comp, "Items")
        par = self.tdn._resolveSequenceBlockPar(comp, seq, seq[0], 0, "Doesnotexist")
...
class TestTDNFingerprintExclusionAndRefs(EmbodyTestCase):
    ...
    def test_excluded_child_omitted_from_fingerprint(self):
        ...
        excluded_fp = self._fp(parent, tdn_paths=None, exclude_tag=self.exclude_tag)
        ...
        self.assertEqual(
            removed_fp,
            excluded_fp,
            "omitting via exclude_tag must equal physically removing the child -- the exclusion is the only difference",
        )
