...
class TestEnvoyToolGuards(EmbodyTestCase):
    ...
    def _make_search_fixture(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(expr_holder.path, expr_par.name, expr="absTime.frame + 12345")
    ...
    def _find_strmenu_parameter(self):
        ...
        for op_type in candidates:
            try:
                candidate = self.sandbox.create(op_type, "strmenu_{}".format(op_type.lower()))
            except Exception:
                continue
    ...
    def test_undoable_ops_are_registered_handlers(self):
        unknown = op.Embody.ext.Envoy._execute_operation("definitely_not_a_real_op", {})
    ...
    def test_undo_block_reentrancy_guard(self):
        ...
        self.assertTrue(opened)
        try:
            self.assertFalse(op.Embody.ext.Envoy._beginUndoBlock("set_parameter"))
        finally:
            op.Embody.ext.Envoy._endUndoBlock()
    ...
    def test_create_op_dispatch_is_undoable(self):
        ...
        result = op.Embody.ext.Envoy._execute_operation(
            "create_op",
            {
    ...
    def test_invalid_menu_value_reports_names_and_preserves_value(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(noise.path, "type", value="notamenuvalue")
    ...
    def test_menu_label_value_hints_internal_name(self):
        ...
        for name, label in zip(list(par.menuNames), list(par.menuLabels)):
            if name != label:
                before = par.eval()
                result = op.Embody.ext.Envoy._set_parameter(noise.path, "type", value=label)
                self._assert_error_contains(result, "Invalid menu value")
    ...
    def test_valid_menu_name_still_sets_value(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(noise.path, "type", value=value)
    ...
    def test_strmenu_value_is_not_menu_guard_rejected(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(target.path, par.name, value=value)
    ...
    def test_set_parameter_grows_constant_chop_sequence(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(chop.path, "const5name", value="mychan")
    ...
    def test_sequence_growth_rejects_absurd_index(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(chop.path, "const500name", value="x")
    ...
    def test_sequence_growth_rejects_typoed_block_suffix(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(chop.path, "const5nam", value="x")
    ...
    def test_non_sequence_missing_parameter_stays_missing(self):
        ...
        result = op.Embody.ext.Envoy._set_parameter(chop.path, "definitelynotapar", value="x")
    ...
    def test_get_parameter_search_by_name_reports_hit_fields(self):
        fixture, value_holder, value_par, _expr_holder, _expr_par = self._make_search_fixture()

        result = op.Embody.ext.Envoy._get_parameter(fixture.path, search=value_par.name, search_in="name")
    ...
    def test_get_parameter_search_by_value_wraps_plain_pattern(self):
        fixture, value_holder, _value_par, _expr_holder, _expr_par = self._make_search_fixture()

        result = op.Embody.ext.Envoy._get_parameter(fixture.path, search="zzsearchtoken", search_in="value")

        self.assertEqual(result["count"], 1)
        self.assertEqual(result["results"][0]["op"], value_holder.path)

    def test_get_parameter_search_in_expr_returns_expr_text(self):
        fixture, _value_holder, _value_par, expr_holder, expr_par = self._make_search_fixture()

        result = op.Embody.ext.Envoy._get_parameter(fixture.path, search="12345", search_in="expr")
    ...
    def test_get_parameter_search_honors_max_results(self):
        fixture, _value_holder, _value_par, _expr_holder, _expr_par = self._make_search_fixture()

        result = op.Embody.ext.Envoy._get_parameter(fixture.path, search="*", max_results=1)
    ...
    def test_get_parameter_search_rejects_invalid_search_in(self):
        ...
        result = op.Embody.ext.Envoy._get_parameter(fixture.path, search="tx", search_in="bogus")
    ...
    def test_get_op_omits_default_parameters_by_default(self):
        ...
        full = op.Embody.ext.Envoy._get_op(top.path, include_defaults=True)
    ...
    def test_get_parameter_compact_and_details_modes(self):
        ...
        compact = op.Embody.ext.Envoy._get_parameter(noise.path, par_name="type")

        self.assertDictHasKey(compact, "menuNames")
        self.assertNotIn("style", compact)
        self.assertNotIn("menuLabels", compact)

        details = op.Embody.ext.Envoy._get_parameter(noise.path, par_name="type", details=True)
    ...
    def test_query_network_child_rows_lack_name(self):
        ...
        result = op.Embody.ext.Envoy._query_network(parent_path=self.sandbox.path)
    ...
    def test_get_network_layout_compact_shape_and_annotation_text_cap(self):
        ...
        annotation = op.Embody.ext.Envoy._create_annotation(
            parent.path, text=long_text, x=-200, y=-200, width=400, height=200
        )
    ...
    def test_execute_python_rolls_back_created_ops_on_error(self):
        ...
        code = "\n".join([
            "target = op({!r}).create('textDAT', {!r})".format(self.sandbox.path, name),
...
class TestCaptureTopSampleGrid(EmbodyTestCase):
    ...
    def test_gradient_grid_stats_vary(self):
        ...
        self.assertNotIn("error", result, repr(result))
        varying = [name for name in ("r", "g", "b", "a") if result["stats"][name]["min"] < result["stats"][name]["max"]]
    ...
    def test_image_params_ignored_in_grid_mode(self):
        ...
        result = op.Embody.ext.Envoy._capture_top(top.path, format="png", inline=True, sample_grid=2)
...
class TestEnvoyDocsPlumbing(EmbodyTestCase):
    ...
    def test_docs_normalize_collapses_case_and_spacing(self):
        self.assertEqual(EnvoyMCPServer._docsNormalize("Movie File In TOP"), "moviefileintop")
        self.assertEqual(
