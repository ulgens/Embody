...
class TestMCPExternalization(EmbodyTestCase):
    ...
    def test_get_externalization_status_existing(self):
        ...
        result = self.envoy._get_externalization_status(op_path=self.embody.path)
        # Should return some status info
        self.assertNotIn("error", result)

    def test_get_externalization_status_nonexistent(self):
        result = self.envoy._get_externalization_status(op_path="/nonexistent")
    ...
    def test_remove_externalization_tag_nonexistent(self):
        result = self.envoy._remove_externalization_tag(op_path="/nonexistent")
        self.assertDictHasKey(result, "error")
    ...
    def test_tag_explicit_type_overrides_language(self):
        ...
        dat.par.language = "python"
        result = self.envoy._externalize_op(op_path=dat.path, tag_type="txt")
        self.assertTrue(result.get("success"))
    ...
    def test_save_externalization_comp(self):
        ...
        self.assertTrue(result.get("success"), f"save_externalization failed: {result.get('error')}")
