...
class TestGLSLExternalize(EmbodyTestCase):
    ...
    def test_glsl_dat_externalizes_to_glsl_not_py(self):
        ...
        self.assertTrue(result.get("success"), f"externalize failed: {result.get('error')}")

        # _externalize_op returns 'file' = target.par.file.eval() for DATs.
        file_path = result.get("file", "")
        self.assertTrue(file_path, "externalized DAT should have a non-empty file path")
        self.assertEndsWith(file_path, ".glsl", f"GLSL DAT externalized to wrong extension: {file_path}")
        self.assertFalse(
    ...
    def test_glsl_dat_file_par_ends_with_glsl(self):
        ...
        self.assertEndsWith(live_file, ".glsl", f"live file par should end in .glsl, got {live_file}")
