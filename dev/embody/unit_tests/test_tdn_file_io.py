...
class TestTDNFileIO(EmbodyTestCase):
                ...
                "children": [
                    {
                        "name": "i",
                        "type": "baseCOMP",
                        "children": [{"name": "leaf", "type": "textDAT"}],
                    }
                ],
            }
        ]
        ...
        ops = [
            {
                "name": "help",
                "type": "baseCOMP",
                "children": [{"name": "text_help", "type": "textDAT"}],
            }
        ]
        files = self.embody.ext.TDN._splitPerComp(ops, "/embody/Embody", "P", self._temp_dir)
        ...
        ops = [
            {
                "name": "Embody",
                "type": "baseCOMP",
                "children": [{"name": "Ext", "type": "textDAT"}],
            }
        ]
        files = self.embody.ext.TDN._splitPerComp(ops, "/embody", "P", self._temp_dir)
    ...
    def test_splitPerComp_mixed_comps_and_leaves(self):
        ...
        ops = [
            {"name": "leaf1", "type": "textDAT"},
            {
                "name": "comp1",
                "type": "baseCOMP",
                "children": [{"name": "inner", "type": "textDAT"}],
            },
    ...
    def test_collectExisting_scoped_to_prefix(self):
        ...
        result = self.embody.ext.TDN._collectExistingTDNFiles(self._temp_dir, "/embody")
        self.assertLen(result, 1)
    ...
    def test_cleanup_deletes_stale(self):
        ...
        deleted = self.embody.ext.TDN._cleanupStaleTDNFiles({stale, kept}, [kept], self._temp_dir)
    ...
    def test_cleanup_keeps_written_files(self):
        ...
        deleted = self.embody.ext.TDN._cleanupStaleTDNFiles({written}, [written], self._temp_dir)
    ...
    def test_cleanup_rejects_non_tdn(self):
        ...
        deleted = self.embody.ext.TDN._cleanupStaleTDNFiles({non_tdn}, [], self._temp_dir)
        self.assertLen(deleted, 0)
    ...
    def test_cleanup_rejects_outside_base(self):
        ...
        try:
            ...
            deleted = self.embody.ext.TDN._cleanupStaleTDNFiles({outside}, [], self._temp_dir)
            self.assertLen(deleted, 0)
    ...
    def test_cleanup_multiple_stale_files(self):
        ...
        for i in range(5):
            ...
            Path(f).write_text("{}")
            stale_files.add(f)
        deleted = self.embody.ext.TDN._cleanupStaleTDNFiles(stale_files, [], self._temp_dir)
    ...
    def test_resolve_auto_root_uses_project_name(self):
        ...
        self.assertTrue(
            ...
            f"Expected build-stripped name '{stable_name}.tdn' in path, got '{resolved}'",
        )
    ...
    def test_resolve_auto_nonroot_no_double_ext_folder(self):
        ...
        self.assertGreaterEqual(first_idx, 0, f"TD path segment '{td_segment}' not found in '{normalized}'")
        # Should not appear again after the first occurrence
        second_idx = normalized.find(td_segment, first_idx + 1)
        self.assertEqual(second_idx, -1, f"TD path segment appears twice in '{normalized}'")
    ...
    def test_savetdn_root_filename_strips_build_suffix(self):
        ...
        for project_name, expected in cases:
            ...
            self.assertEqual(
                safe_name,
                expected,
                f"Expected SaveTDN root filename '{expected}.tdn' for project '{project_name}', got '{safe_name}.tdn'",
            )
    ...
    def test_savetdn_root_filename_matches_resolve_output_path(self):
        ...
        self.assertTrue(
            ...
            f"SaveTDN would write '{savetdn_name}.tdn' but _resolveOutputPath returned '{resolved}'",
        )
    ...
    def test_export_file_is_valid_yaml(self):
        ...
        result = self.embody.ext.TDN.ExportNetwork(root_path=self.sandbox.path, output_file=fp)
    ...
    def test_export_file_contains_all_operators(self):
        ...
        self.embody.ext.TDN.ExportNetwork(root_path=self.sandbox.path, output_file=fp)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
    ...
    def test_export_file_dat_text_content(self):
        ...
        self.embody.ext.TDN.ExportNetwork(root_path=self.sandbox.path, output_file=fp, include_dat_content=True)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
    ...
    def test_export_file_roundtrip_reimport(self):
        ...
        result = self.embody.ext.TDN.ImportNetwork(target_path=target.path, tdn=tdn_data)
        self.assertTrue(result.get("success"))
    ...
    def test_importFromFile_basic(self):
        ...
        result = self.embody.ext.TDN.ImportNetworkFromFile(file_path=fp, target_path=target.path)
        self.assertTrue(result.get("success"))
    ...
    def test_importFromFile_nonexistent(self):
        result = self.embody.ext.TDN.ImportNetworkFromFile(
            file_path="/nonexistent/xyz.tdn", target_path=self.sandbox.path
        )
    ...
    def test_importFromFile_invalid_tdn(self):
        ...
        result = self.embody.ext.TDN.ImportNetworkFromFile(file_path=bad, target_path=self.sandbox.path)
        self.assertIn("error", result)
        self.assertIn("Invalid TDN", result["error"])

    def test_importFromFile_empty_string_path(self):
        result = self.embody.ext.TDN.ImportNetworkFromFile(file_path="", target_path=self.sandbox.path)
        self.assertIn("error", result)
    ...
    def test_importFromFile_roundtrip_preserves_connections(self):
        ...
        self.embody.ext.TDN.ImportNetworkFromFile(file_path=fp, target_path=target.path)
    ...
    def test_export_file_encoding_utf8(self):
        ...
        self.assertEqual(entry["dat_content"], "\u65e5\u672c\u8a9e\u30c6\u30b9\u30c8")
    ...
    def test_export_dat_content_excluded_when_disabled(self):
        ...
        self.embody.ext.TDN.ExportNetwork(root_path=self.sandbox.path, output_file=fp, include_dat_content=False)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
    ...
    def test_tdn_ref_written_on_export(self):
        ...
        self.embody.ext.TDN.ExportNetwork(root_path=child.path, output_file=str(child_abs))
        from datetime import datetime

        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        self.embody_ext._addToTable(child, str(child_path), timestamp, False, 1, str(app.build), "tdn")
        # Now export the parent
        fp = str(Path(self._temp_dir) / "parent.tdn")
        self.embody.ext.TDN.ExportNetwork(root_path=parent.path, output_file=fp)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        child_entry = [o for o in data["operators"] if o["name"] == "child_comp"][0]
    ...
    def test_tdn_ref_absent_with_embed_all(self):
        ...
        self.embody.ext.TDN.ExportNetwork(root_path=parent.path, output_file=fp, embed_all=True)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        child_entry = [o for o in data["operators"] if o["name"] == "child_comp"][0]
        self.assertNotIn("tdn_ref", child_entry)
    ...
    def test_validateTDNRefs_happy_path(self):
        ...
        warnings = self.embody.ext.TDN._validateTDNRefs(op_defs, parent.path)
    ...
    def test_validateTDNRefs_missing_file(self):
        ...
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        self.embody_ext._addToTable(child, "nonexistent/child.tdn", timestamp, False, 1, str(app.build), "tdn")
    ...
    def test_tox_ref_written_on_export(self):
        ...
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        self.embody_ext._addToTable(child, "fake/tox_child.tox", timestamp, False, 1, str(app.build), "tox")
        # Export the parent
        fp = str(Path(self._temp_dir) / "parent_tox.tdn")
        self.embody.ext.TDN.ExportNetwork(root_path=parent.path, output_file=fp)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        child_entry = [o for o in data["operators"] if o["name"] == "tox_child"][0]
    ...
    def test_tox_ref_absent_without_tag(self):
        ...
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        child_entry = [o for o in data["operators"] if o["name"] == "plain_child"][0]
    ...
    def test_tox_ref_absent_with_embed_all(self):
        ...
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        self.embody_ext._addToTable(child, "fake/tox_child3.tox", timestamp, False, 1, str(app.build), "tox")
        fp = str(Path(self._temp_dir) / "embed_tox.tdn")
        self.embody.ext.TDN.ExportNetwork(root_path=parent.path, output_file=fp, embed_all=True)
        with open(fp, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        child_entry = [o for o in data["operators"] if o["name"] == "tox_child3"][0]
        self.assertNotIn("tox_ref", child_entry)
        self.assertIn("children", child_entry)
    ...
    def test_tox_children_stripped_on_import(self):
        ...
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        self.embody_ext._addToTable(
            child,
            "fake/tox_strip_child.tox",
            timestamp,
            False,
            1,
            str(app.build),
        ...
        skipped = self.embody.ext.TDN._stripNestedTOXChildren(op_defs, parent.path, tox_paths)
    ...
    def test_tox_type_defaults_not_polluted(self):
        ...
        for cname in ("tox_a", "tox_b"):
            ...
            timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
            self.embody_ext._addToTable(child, f"fake/{cname}.tox", timestamp, False, 1, str(app.build), "tox")
    ...
    def test_large_tdn_warning_shown(self):
        ...
        self.embody.store("_smoke_test_responses", {"Large TDN File": 0})
        try:
            self.embody.ext.TDN._warnLargeTDN(str(big_file), "/test")
