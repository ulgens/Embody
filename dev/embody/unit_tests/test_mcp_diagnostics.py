...
class TestMCPDiagnostics(EmbodyTestCase):
    ...
    def test_get_op_errors_clean_op(self):
        ...
        result = self.envoy._get_op_errors(op_path=comp.path, recurse=False)
    ...
    def test_get_op_errors_recursive(self):
        ...
        result = self.envoy._get_op_errors(op_path=comp.path, recurse=True)
        self.assertDictHasKey(result, "errorCount")

    def test_get_op_errors_nonexistent(self):
        result = self.envoy._get_op_errors(op_path="/nonexistent", recurse=False)
    ...
    def test_exec_op_method_cook(self):
        ...
        result = self.envoy._exec_op_method(op_path=comp.path, method="cook", args=[], kwargs={"force": True})
        self.assertNotIn("error", result)

    def test_exec_op_method_nonexistent_method(self):
        comp = self.sandbox.create(baseCOMP, "bad_method")
        result = self.envoy._exec_op_method(op_path=comp.path, method="nonExistentMethod123")
        self.assertDictHasKey(result, "error")
    ...
    def test_get_td_class_details_nonexistent(self):
        result = self.envoy._get_td_class_details(class_name="NonExistentClass12345")
        self.assertDictHasKey(result, "error")
