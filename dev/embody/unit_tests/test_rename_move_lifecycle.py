...
class TestRenameMoveLifecycle(EmbodyTestCase):
    ...
    def test_findMovedOp_returns_false_when_truly_missing(self):
        ...
        processed = set()
        ext_folder = self.embody_ext.ExternalizationsFolder
        found = self.embody_ext._findMovedOp("/nonexistent/path", "nonexistent/file.tox", ext_folder, processed)
    ...
    def test_findMovedTDNOp_renames_file_on_disk(self):
        ...
        old_abs = self.embody_ext.buildAbsolutePath(self.embody_ext.normalizePath(old_rel)).resolve()
        ...
        new_rel = self.embody_ext.Externalizations[comp.path, "rel_file_path"].val
        new_abs = self.embody_ext.buildAbsolutePath(self.embody_ext.normalizePath(new_rel)).resolve()
    ...
    def test_findMovedTDNOp_returns_false_when_truly_missing(self):
        ...
        processed = set()
        found = self.embody_ext._findMovedTDNOp("/nonexistent/tdn_comp", "nonexistent/tdn_comp.tdn", processed)
    ...
    def test_findMovedTDNOp_does_not_match_already_tracked(self):
        ...
        # Destroy comp2 to simulate it being truly gone
        comp2.destroy()
