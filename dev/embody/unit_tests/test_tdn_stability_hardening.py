...
class TestTDNStabilityHardening(EmbodyTestCase):
    ...
    def test_A01_export_preserves_live_table_cells(self):
        ...
        self.assertEqual(tbl[2, 1].val, "a\tb", "export corrupted an embedded tab in a live cell")
        # And the exported document carries the exact cell values.
        entry = next(o for o in result["tdn"]["operators"] if o["name"] == "cells_tbl")
    ...
    def test_A02_table_cell_special_chars_round_trip(self):
        ...
        tbl.appendRow(["multiline", "line1\nline2"])
        tbl.appendRow(["tabbed", "a\tb"])
        exported = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)["tdn"]
    ...
    def test_A03_read_only_companion_still_marked(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        entry = next((o for o in result["tdn"]["operators"] if o["name"] == "glsl_ro_info"), None)
        self.assertIsNotNone(entry, "info DAT missing from export")
        self.assertTrue(
            entry.get("dat_read_only"),
    ...
    def test_A04_locked_dat_content_still_exports(self):
        ...
        try:
            result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
            entry = next(o for o in result["tdn"]["operators"] if o["name"] == "locked_txt")
    ...
    def test_B01_geo_render_off_exports_negative_flags(self):
        ...
        entry = next(o for o in result["tdn"]["operators"] if o["name"] == "geo_off")
        flags = entry.get("flags", [])
        self.assertIn("-render", flags, "geo render-off not captured (creation default is ON)")
        self.assertIn("-display", flags, "geo display-off not captured (creation default is ON)")

    def test_B02_geo_defaults_export_no_render_flags(self):
        self.sandbox.create(geometryCOMP, "geo_def")
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path)
        entry = next(o for o in result["tdn"]["operators"] if o["name"] == "geo_def")
        flags = entry.get("flags", [])
        for noisy in ("render", "display", "-render", "-display"):
            self.assertNotIn(noisy, flags, f"default geo exported spurious flag {noisy}")
    ...
    def test_B03_geo_render_off_round_trips(self):
        ...
        res = self.tdn.ImportNetwork(self.sandbox.path, exported, clear_first=True)
        self.assertTrue(res.get("success"))
        rebuilt = self.sandbox.op("geo_rt")
        self.assertIsNotNone(rebuilt)
        self.assertFalse(rebuilt.render, "render=False lost through TDN round-trip")
        self.assertFalse(rebuilt.display, "display=False lost through TDN round-trip")
    ...
    def test_B04_common_types_flag_export_unchanged(self):
        ...
        n.viewer = True
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path)
        entry = next(o for o in result["tdn"]["operators"] if o["name"] == "noise_v")
    ...
    def test_C01_malformed_operator_entry_never_destroys_children(self):
        ...
        result = self.tdn.ImportNetwork(self.sandbox.path, bad_doc, clear_first=True)
        self.assertDictHasKey(result, "error")
        self.assertIsNotNone(
            self.sandbox.op("keeper"),
            "clear_first destroyed children before structural validation rejected the document",
        )
    ...
    def test_C02_malformed_children_never_destroys_children(self):
        ...
        bad_doc = {
            ...
            "version": "2.0",
            "operators": [
                {"name": "parent", "type": "baseCOMP", "children": "not a list"},
            ],
        }
        result = self.tdn.ImportNetwork(self.sandbox.path, bad_doc, clear_first=True)
        self.assertDictHasKey(result, "error")
    ...
    def test_C03_malformed_par_templates_degrades_gracefully(self):
        ...
        self.assertTrue(result.get("success"), f"graceful degradation failed: {result}")
    ...
    def test_C04_validate_op_defs_pure(self):
        ...
        self.assertIsNotNone(v([{"name": "a", "children": [["nested", "junk"]]}]))
    ...
    def test_D01_untagged_export_writes_file_but_no_row(self):
        ...
        result = self.tdn.ExportNetwork(root_path=comp.path, output_file=str(out))
        self.assertTrue(result.get("success"))
        ...
        self.assertNotIn(
            comp.path,
            rows,
            "ad-hoc export of an UNTAGGED COMP appended a table row (silent lifecycle enrollment)",
        )
    ...
    def test_D03_recovery_pointer_not_serialized(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path)
        entry = next(o for o in result["tdn"]["operators"] if o["name"] == "pointer_skip")
        self.assertNotIn(
            "_tdn_rel_path",
            entry.get("storage", {}),
    ...
    def test_E01_restrict_to_tracked_drops_strays(self):
        ...
        stray = str(Path(project.folder) / "embody" / "unit_tests" / "_stray_never_tracked.tdn")
        kept = self.tdn._restrictToTrackedTDN({tracked_files[0], stray})
        self.assertIn(tracked_files[0], kept)
        self.assertNotIn(stray, kept, "untracked stray survived into deletion candidates")
    ...
    def test_E02_export_does_not_delete_untracked_stray(self):
        ...
        result = self.tdn.ExportNetwork(root_path=comp.path, output_file=str(out))
        self.assertTrue(result.get("success"))
        self.assertTrue(stray.is_file(), "stale cleanup deleted an untracked stray .tdn")
    ...
    def test_F01_orphan_recovered_via_storage_pointer(self):
        ...
        self.assertIn(comp.path, res.get("restored", []), f"orphan not restored: {res}")
        self.assertIsNotNone(comp.op("payload"), "children not rebuilt from the .tdn")
        table = self.embody_ext.Externalizations
        rows = [table[i, "path"].val for i in range(1, table.numRows)]
