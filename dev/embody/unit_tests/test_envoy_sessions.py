...
class TestHeartbeatSessions(EmbodyTestCase):
    ...
    def test_heartbeat_carries_sid_and_label(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            path = os.path.join(logdir, "envoy-bridge-{}.heartbeat".format(os.getpid()))
    ...
    def test_list_live_sessions_includes_self_and_fresh_peer(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(os.path.join(logdir, "envoy-bridge-{}.heartbeat".format(peer_pid)), "w") as f:
                json.dump(peer, f)
    ...
    def test_list_live_sessions_drops_stale_heartbeat(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(
                os.path.join(logdir, "envoy-bridge-{}.heartbeat".format(os.getppid())),
                "w",
            ) as f:
                json.dump(stale, f)
            sessions = bridge._list_live_sessions(cfg)
    ...
    def test_list_live_sessions_drops_dead_pid(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(os.path.join(logdir, "envoy-bridge-99999999.heartbeat"), "w") as f:
                json.dump(dead, f)
            sessions = bridge._list_live_sessions(cfg)
            self.assertEqual(sessions, [])
...
class TestScopeHelpers(EmbodyTestCase):
    def test_scopes_create_op(self):
        scopes = _envoy_mod._scopes_for_operation("create_op", {"parent_path": "/a"}, {"path": "/a/b"})
        self.assertEqual(scopes, ["/a", "/a/b"])

    def test_scopes_special_and_paths(self):
        self.assertEqual(_envoy_mod._scopes_for_operation("run_tests", {}), ["project:tests"])
        self.assertIn(
            "project:python",
    ...
    def test_scopes_rename_includes_new_path(self):
        scopes = _envoy_mod._scopes_for_operation("rename_op", {"op_path": "/a/old", "new_name": "new"})
        ...
        scopes = _envoy_mod._scopes_for_operation(
            "batch_operations",
            {
                "operations": [
                    ...
                    {
                        "tool": "connect_ops",
                        "params": {"source_path": "/a/x", "dest_path": "/a/y"},
                    },
                ]
            },
        )
...
class TestTouchMapAndAdvisories(EmbodyTestCase):
    ...
    def _purge(self):
        with self.lock:
            for k in [k for k in self.touches if k.startswith("/_tc_") or k.startswith("file:_tc")]:
                del self.touches[k]
    ...
    def _plant_touch(self, sid, scope, tool="set_parameter", age_s=0.0):
        with self.lock:
            ...
            ring.append({"sid": sid, "tool": tool, "ts": time.time() - age_s})
    ...
    def test_conflict_advisory_for_peer_write(self):
        ...
        result = {}
        self.ext._attachPeerAdvisories(result, self.B, "set_parameter", ["/_tc_scope_x"])
    ...
    def test_no_advisory_for_own_touch(self):
        self._plant_touch(self.A, "/_tc_scope_x")
        result = {}
        self.ext._attachPeerAdvisories(result, self.A, "set_parameter", ["/_tc_scope_x"])
    ...
    def test_ancestor_scope_triggers_advisory(self):
        self._plant_touch(self.A, "/_tc_scope_x")
        result = {}
        self.ext._attachPeerAdvisories(result, self.B, "set_parameter", ["/_tc_scope_x/child"])
    ...
    def test_advisories_collapse_per_peer_prefer_op_path(self):
        ...
        self._plant_touch(self.A, "/_tc_scope_x")
        result = {}
        self.ext._attachPeerAdvisories(result, self.B, "set_parameter", ["/_tc_scope_x", "file:_tc/x.py"])
        peers = result.get("_peers", [])
        self.assertEqual(len(peers), 1)
        self.assertEqual(peers[0]["scope"], "/_tc_scope_x")
    ...
    def test_collapsed_scope_surfaces_on_next_call(self):
        ...
        self._plant_touch(self.A, "/_tc_scope_x", age_s=120)
        first, second = {}, {}
        self.ext._attachPeerAdvisories(first, self.B, "get_op", ["/_tc_scope_x", "file:_tc/x.py"])
        self.ext._attachPeerAdvisories(second, self.B, "get_op", ["/_tc_scope_x", "file:_tc/x.py"])
    ...
    def test_expand_file_scopes_nearest_two_only(self):
        scopes = self.ext._expandFileScopes(["/embody/unit_tests/test_envoy_sessions"])
        self.assertIn("file:embody/unit_tests/test_envoy_sessions.py", scopes)
        self.assertIn("file:embody/unit_tests.tdn", scopes)
        self.assertNotIn("file:embody.tdn", scopes, "project-root .tdn must not blanket every op")

    def test_snapshot_includes_recent_scopes(self):
        w = _bare_worker()
        w._touch_session("7-1-aaaa", "lab")
        w._touches["/_tc_scope_w"] = [{"sid": "7-1-aaaa", "tool": "set_parameter", "ts": time.time()}]
        snap = w._sessions_snapshot()
...
class TestClaimLeases(EmbodyTestCase):
    ...
    def test_own_claims_never_conflict(self):
        w = self._worker_with_sessions()
        self.assertTrue(w._claim_scope("11-1-aaaa", "alpha@x", "/_tc_c", "", 300)["granted"])
        self.assertTrue(w._claim_scope("11-1-aaaa", "alpha@x", "/_tc_c/sub", "", 300)["granted"])

    def test_scope_validation_and_ttl_clamp(self):
        w = self._worker_with_sessions()
        self.assertIn("error", w._claim_scope("11-1-aaaa", "alpha@x", "not-a-scope", "", 300))
    ...
    def test_expiry_ttl_and_dead_holder(self):
        ...
        self.assertNotIn("/_tc_dead_holder", w._claims, "silent holder (not in registry) must expire")
        self.assertIn(
            "/_tc_anon_ok",
            w._claims,
    ...
    def test_renewal_via_record_touches(self):
        ...
        try:
            ext._recordTouches("_tc_claim_a", "set_parameter", ["/_tc_renew/child"])
            with lock:
                self.assertGreater(claims["/_tc_renew"]["ts"], old_ts, "own write must renew the lease")
        finally:
            with lock:
...
class TestDestructiveGates(EmbodyTestCase):
    ...
    def _purge(self):
        with self.lock:
            for k in [k for k in self.claims if "/_tc_gate" in k or k == "project:tests_tc"]:
                del self.claims[k]
    ...
    def test_targets_matrix(self):
        ...
        self.assertEqual(dt("delete_op", {"op_path": "/_tc_gate/x"}), (["/_tc_gate/x"], "delete_op"))
        self.assertEqual(
            dt("import_network", {"target_path": "/_tc_gate", "clear_first": False}),
            ([], ""),
        )
        scopes, reason = dt("import_network", {"target_path": "/_tc_gate", "clear_first": True})
    ...
    def test_gate_refuses_on_live_peer_claim(self):
        ...
        gate = self.ext._checkDestructiveGate(self.B, "delete_op", {"op_path": "/_tc_gate/child"})
        self.assertIsNotNone(gate)
        self.assertIn("MULTI-SESSION GATE", gate["error"])
        self.assertEqual(gate["holder"]["label"], "gate-a@x")
        # Holder itself passes
        self.assertIsNone(self.ext._checkDestructiveGate(self.A, "delete_op", {"op_path": "/_tc_gate/child"}))
        # override passes
        self.assertIsNone(
            self.ext._checkDestructiveGate(self.B, "delete_op", {"op_path": "/_tc_gate/child", "override": True})
        )
    ...
    def test_gate_ignores_stale_holder(self):
        ...
        self.assertIsNone(self.ext._checkDestructiveGate(self.B, "delete_op", {"op_path": "/_tc_gate/child"}))

    def test_gate_refuses_on_fresh_peer_write(self):
        with self.lock:
            self.touches["/_tc_gate/x"] = [{"sid": self.A, "tool": "set_parameter", "ts": time.time() - 5}]
        gate = self.ext._checkDestructiveGate(self.B, "delete_op", {"op_path": "/_tc_gate/x"})
        self.assertIsNotNone(gate)
        self.assertEqual(gate["peer"]["label"], "gate-a@x")
        # Old write (outside the conflict window) passes
        with self.lock:
            self.touches["/_tc_gate/x"] = [{"sid": self.A, "tool": "set_parameter", "ts": time.time() - 120}]
        self.assertIsNone(self.ext._checkDestructiveGate(self.B, "delete_op", {"op_path": "/_tc_gate/x"}))
    ...
    def test_non_destructive_ops_never_gated(self):
        ...
        self.assertIsNone(self.ext._checkDestructiveGate(self.B, "set_parameter", {"op_path": "/_tc_gate/child"}))
        self.assertIsNone(self.ext._checkDestructiveGate(self.B, "get_op", {"op_path": "/_tc_gate/child"}))
