...
class TestTDNSequences(EmbodyTestCase):
    ...
    def test_sequence_base_names_no_prefix(self):
        ...
        result = self._export()
        tdn = result["tdn"]
        chop_def = [o for o in tdn["operators"] if o["name"] == "basenames_chop"][0]
        ...
        for block in blocks:
            for key in block:
                self.assertFalse(key.startswith("const"), f"Expected base name, got prefixed: {key}")
    ...
    def test_sequence_nondefault_value_in_first_block_exported(self):
        ...
        result = self._export()
        tdn = result["tdn"]
        chop_def = [o for o in tdn["operators"] if o["name"] == "val_chop"][0]
        self.assertDictHasKey(chop_def, "sequences")
        blocks = chop_def["sequences"]["const"]
    ...
    def test_sequence_roundtrip_expression_values(self):
        ...
        result = self._export()
        tdn = result["tdn"]
        chop_def = [o for o in tdn["operators"] if o["name"] == "expr_chop"][0]
        blocks = chop_def["sequences"]["const"]
    ...
    def test_custom_sequence_export_skips_per_block_pars(self):
        ...
        result = self._export()
        comp_def = [o for o in result["tdn"]["operators"] if o["name"] == "cs_skip_test"][0]
        ...
        for name in names:
            self.assertFalse(name.startswith("Items0"), f"Per-block instance leaked: {name}")
            self.assertFalse(name.startswith("Items1"), f"Per-block instance leaked: {name}")
    ...
    def test_custom_sequence_template_pars_have_sequence_field(self):
        ...
        result = self._export()
        comp_def = [o for o in result["tdn"]["operators"] if o["name"] == "cs_field_test"][0]
    ...
    def test_custom_sequence_template_par_names_are_capitalized_base(self):
        ...
        result = self._export()
        comp_def = [o for o in result["tdn"]["operators"] if o["name"] == "cs_name_test"][0]
        custom_pars = comp_def["custom_pars"]["Items"]
        names = [p["name"] for p in custom_pars]
    ...
    def test_custom_sequence_roundtrip_three_blocks(self):
        ...
        for i in range(3):
            self.assertEqual(getattr(imp.par, f"Items{i}itemlabel").eval(), f"item{i}")
            self.assertEqual(getattr(imp.par, f"Items{i}itemweight").eval(), float(i * 10))
