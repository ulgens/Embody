...
class _FakeTDN:
    ...
    def ExportNetwork(self, root_path=None, include_dat_content=None, embed_all=None, **kwargs):
...
class TestSpecimenPublishBuckets(EmbodyTestCase):
    ...
    def _write_manifest(self, specimens):
        ...
        Path(self._spec_dir, "manifest.json").write_text(json.dumps({"specimens": specimens}), encoding="utf-8")
    ...
    def _install(self, present=None, export_result=None, export_fn=None):
        ...
        fake_tdn = _FakeTDN(real_tdn, export_result=export_result, export_fn=export_fn)
    ...
    def test_export_failure_marks_missing_with_suffix(self):
        ...
        self._install(present=present, export_result={"success": False, "error": "boom"})
        result = self.mod._publish()
    ...
    def test_slug_hyphen_maps_to_underscore_path(self):
        ...
        fake_tdn, _ = self._install(
            present=present,
            export_result={"success": True, "tdn": {"format": "tdn", "operators": []}},
        )
        result = self.mod._publish()
    ...
    def test_written_file_is_valid_tdn_roundtrip(self):
        ...
        self._install(present=present, export_result={"success": True, "tdn": tdn})
        self.mod._publish()
...
class TestSpecimenPublishLive(EmbodyTestCase):
    ...
    def test_live_first_writes_second_skips(self):
        ...
        res = TDN.ExportNetwork(root_path=self._live.path, include_dat_content=True, embed_all=True)
        self.assertTrue(res.get("success"), f"live export failed: {res.get('error')}")
        new = res["tdn"]
        ...
        res2 = TDN.ExportNetwork(root_path=self._live.path, include_dat_content=True, embed_all=True)
        self.assertTrue(res2.get("success"))
        old = TDN._read_existing_tdn(str(out))
        self.assertIsNotNone(old)
        self.assertTrue(
            TDN._tdn_content_equal(res2["tdn"], old),
