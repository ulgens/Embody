...
class TestSetupWizard(EmbodyTestCase):
    ...
    def tearDown(self):
        for obj, name in ((self._ext, "_extractAIConfig"), (self._envoy, "Stop")):
    ...
    def test_claudecode_sets_params_and_enables(self):
        self._ext._applyWizardSetup(mode="advanced", assistant="claudecode", root="projectfolder")
        ...
        self.assertTrue(
            bool(self._emb.par.Envoyenable.eval()),
            "first-run claudecode must enable Envoy",
        )
        self.assertEqual(self._extract_calls, [1], "first run must write AI config exactly once")
    ...
    def test_permissions_plumbs_to_param(self):
        ...
        self._ext._applyWizardSetup(mode="auto", assistant="claudecode", permissions="some")
        self.assertEqual(
            self._emb.par.Toolpermissions.eval(),
            "some",
    ...
    def test_unknown_permissions_falls_back_to_all(self):
        self._ext._applyWizardSetup(mode="auto", assistant="claudecode", permissions="bogus")
        self.assertEqual(
            self._emb.par.Toolpermissions.eval(),
            "all",
    ...
    def test_other_maps_client_token(self):
        self._ext._applyWizardSetup(mode="auto", assistant="other", client="cursor", root="gitroot")
    ...
    def test_none_disables_and_skips_enable(self):
        ...
        self.assertEqual(self._extract_calls, [], "assistant='none' must NOT write AI config")
    ...
    def test_unknown_assistant_is_noop_not_enable(self):
        ...
        self.assertEqual(self._extract_calls, [], "an unknown assistant token must not write config")
        self.assertEqual(
            self._emb.par.Aiclient.eval(),
            before_client,
    ...
    def test_custom_root_sets_path_and_mode(self):
        self._ext._applyWizardSetup(
            ...
            root="custom",
            custom_root="/tmp/embody-cfg",
        )
        self.assertEqual(self._emb.par.Aiprojectroot.eval(), "custom")
        self.assertEqual(self._emb.par.Aiprojectrootcustom.eval(), "/tmp/embody-cfg")
    ...
    def test_rerun_restarts_instead_of_reenabling(self):
        ...
        self.assertEqual(self._stop_calls, [1], "a re-run must Stop() to force a restart")
        self.assertEqual(self._extract_calls, [], "a re-run must not re-extract AI config")
        self.assertTrue(
            bool(self._emb.par.Envoyenable.eval()),
    ...
    def test_first_run_enable_consents_the_config_batch(self):
        ...
        self.assertTrue(
            self._ext._consent_bulk,
            "first-run enable must consent the config batch so the deferred Start writes apply without re-prompting",
        )
        self.assertTrue(bool(self._emb.par.Envoyenable.eval()))
        self.assertEqual(self._extract_calls, [1], "AI config is written once under the consent")
    ...
    def test_open_wizard_suppressed_during_test_run(self):
        ...
        self.assertTrue(self._ext._suppressDialogs(), "a running suite must suppress dialogs")
