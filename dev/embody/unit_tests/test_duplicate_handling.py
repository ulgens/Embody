...
class TestReplicantHandling(EmbodyTestCase):
    ...
    def test_build_path_groups_skips_replicants(self):
        ...
        replicants = [
            ...
            if c is not master and c is not replicator and c.family == "COMP" and c.type != "tableDAT"
        ]
...
class TestCheckForDuplicates(EmbodyTestCase):
    ...
    def test_group_level_clone_tag_skips_entire_group(self):
        ...
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": 1})
    ...
    def test_dismiss_does_not_tag_anyone(self):
        ...
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": 0})
        self.embody_ext.checkForDuplicates()
        self.assertNotIn("clone", dats[0].tags)
    ...
    def test_three_ops_user_selects_middle(self):
        ...
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": 2})
        self.embody_ext.checkForDuplicates()
        ...
        if has_strategy:
            for i in range(1, table.numRows):
                if table[i, "path"].val == dats[1].path:
                    ...
                    self.assertTrue(len(strategy) > 0, "strategy column should not be empty")
                    return
    ...
    def test_at_threshold_uses_enumerated_buttons(self):
        ...
        dats = self._make_tagged_dats([f"thr_{i}" for i in range(n)], shared_path="test/threshold.py")
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": n})  # last op = master
        self.embody_ext.checkForDuplicates()
        clone_count = sum(1 for d in dats if "clone" in d.tags)
        self.assertEqual(clone_count, n - 1, "Enumerated dialog should tag all but the chosen op")
    ...
    def test_large_group_strategy_keep_first(self):
        ...
        dats = self._make_tagged_dats([f"big_{i}" for i in range(n)], shared_path="test/biggroup.py")
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": 1})  # Keep first
        self.embody_ext.checkForDuplicates()
        clone_count = sum(1 for d in dats if "clone" in d.tags)
        self.assertEqual(clone_count, n - 1, "Strategy prompt should tag all but the first op")
    ...
    def test_large_group_dismiss_tags_nothing(self):
        ...
        dats = self._make_tagged_dats([f"bigd_{i}" for i in range(n)], shared_path="test/biggroupd.py")
        self.embody.store("_smoke_test_responses", {"Duplicate Path Detected": 0})  # Dismiss
        self.embody_ext.checkForDuplicates()
        for d in dats:
            self.assertNotIn("clone", d.tags, f"{d.path} should not be tagged after Dismiss")
...
class TestBatchResolution(EmbodyTestCase):
    ...
    def test_single_unresolved_group_skips_batch_prompt(self):
        ...
        self.assertEqual(
            clone_count,
            1,
            "Exactly one op should be tagged clone after per-group prompt",
        )
    ...
    def test_batch_auto_resolve_tags_all_groups(self):
        ...
        self.embody.store("_smoke_test_responses", {"Duplicate Paths Detected": 2})
        self.embody_ext.checkForDuplicates()
        g1_clones = sum(1 for d in grp1 if "clone" in d.tags)
        g2_clones = sum(1 for d in grp2 if "clone" in d.tags)
        self.assertEqual(g1_clones, len(grp1) - 1, "Group 1: all but master should be tagged clone")
        self.assertEqual(g2_clones, len(grp2) - 1, "Group 2: all but master should be tagged clone")
    ...
    def test_batch_dismiss_tags_nothing(self):
        ...
        # Button 0 = 'Dismiss'
        self.embody.store("_smoke_test_responses", {"Duplicate Paths Detected": 0})
        self.embody_ext.checkForDuplicates()
        for d in grp1 + grp2:
            self.assertNotIn("clone", d.tags, f"{d.path} should not be tagged after Dismiss")
    ...
    def test_batch_review_falls_through_to_per_group(self):
        ...
        g1_clones = sum(1 for d in grp1 if "clone" in d.tags)
        g2_clones = sum(1 for d in grp2 if "clone" in d.tags)
        self.assertEqual(g1_clones, 1, "Group 1: one op should be tagged via per-group prompt")
        self.assertEqual(g2_clones, 1, "Group 2: one op should be tagged via per-group prompt")
    ...
    def test_auto_resolve_helper_first_op_is_master(self):
        ...
        self.assertNotIn("clone", ops[0].tags, "First op should be retained as master")
...
class TestTemplateMarkerResolution(EmbodyTestCase):
    ...
    def test_marker_match_is_exact_segment_not_substring(self):
        ...
        result = self.embody_ext._resolveByTemplateMarker(ops)
        self.assertFalse(result, "Substring (mytemplate) must not match marker")
