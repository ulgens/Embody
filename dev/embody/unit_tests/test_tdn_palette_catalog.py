...
class TestTDNPaletteCatalog(EmbodyTestCase):
    ...
    def test_A02_catalog_lookup_wrong_type_rejected(self):
        ...
        self.assertFalse(self.tdn._isPaletteClone(comp), "Wrong OPType must not match catalog entry")
    ...
    def test_B04_child_count_floor_zero_skips_check(self):
        ...
        self.assertTrue(self.tdn._isPaletteClone(comp), "min_children=0 must skip the floor check")
    ...
    def test_E01_animationcomp_dat_content_exported_despite_flag(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=False)
        self.assertTrue(result.get("success"))

        # Walk the export to find the 'attributes' DAT inside anim1.
        attrs_export = self._findOpInExport(result["tdn"], "attributes")
        self.assertIsNotNone(attrs_export, "attributes DAT inside animationCOMP must appear in export")
        self.assertIn(
            "dat_content",
            attrs_export,
            "animationCOMP DAT must carry content even with include_dat_content=False",
        )
    ...
    def test_E03_animationcomp_dats_roundtrip(self):
        ...
        for c in list(self.sandbox.children):
            c.destroy()
        result = self.tdn.ImportNetwork(target_path=self.sandbox.path, tdn=orig["tdn"])
        self.assertTrue(result.get("success"), f"Import failed: {result}")

        restored_attrs = self.sandbox.op("anim2/attributes")
        self.assertIsNotNone(restored_attrs, "attributes DAT must be restored inside animationCOMP")
    ...
    def test_G01_storage_override_blackbox(self):
        ...
        try:
            ...
            self.assertEqual(
                self.tdn._resolvePaletteHandling(comp),
                "blackbox",
    ...
    def test_G02_storage_override_fullexport(self):
        ...
        try:
            ...
            self.assertEqual(self.tdn._resolvePaletteHandling(comp), "fullexport")
        finally:
            self.embody.par.Tdnpalettehandling = saved_par
    ...
    def test_G03_par_blackbox_no_prompt(self):
        ...
        try:
            self.embody.par.Tdnpalettehandling = "blackbox"
            self.assertEqual(self.tdn._resolvePaletteHandling(comp), "blackbox")
        finally:
            self.embody.par.Tdnpalettehandling = saved_par
    ...
    def test_G05_ask_prompt_blackbox_this(self):
        ...
        try:
            ...
            self.embody.store("_smoke_test_responses", {"Embody - Palette Component Detected": 0})
    ...
    def test_G06_ask_prompt_fullexport_this(self):
        ...
        try:
            self.embody.par.Tdnpalettehandling = "ask"
            self.embody.store("_smoke_test_responses", {"Embody - Palette Component Detected": 1})
            result = self.tdn._resolvePaletteHandling(comp)
    ...
    def test_G07_ask_prompt_blackbox_for_all(self):
        ...
        try:
            self.embody.par.Tdnpalettehandling = "ask"
            self.embody.store("_smoke_test_responses", {"Embody - Palette Component Detected": 2})
            result = self.tdn._resolvePaletteHandling(comp)
            self.assertEqual(result, "blackbox")
            self.assertEqual(
                self.embody.par.Tdnpalettehandling.eval(),
                "blackbox",
    ...
    def test_G08_ask_prompt_fullexport_for_all(self):
        ...
        try:
            self.embody.par.Tdnpalettehandling = "ask"
            self.embody.store("_smoke_test_responses", {"Embody - Palette Component Detected": 3})
            result = self.tdn._resolvePaletteHandling(comp)
            self.assertEqual(result, "fullexport")
            self.assertEqual(self.embody.par.Tdnpalettehandling.eval(), "fullexport")
        finally:
            self.embody.par.Tdnpalettehandling = saved_par
    ...
    def test_G09_export_blackbox_omits_children(self):
        ...
        self.assertTrue(entry.get("palette_clone"), "blackbox must set palette_clone=true")
        self.assertNotIn("children", entry, "blackbox must not emit internal children")
    ...
    def test_G10_export_fullexport_includes_children(self):
        ...
        self.assertIsNotNone(entry)
        self.assertFalse(entry.get("palette_clone"), "fullexport must not set palette_clone flag")
        self.assertIn("children", entry, "fullexport must recurse into children")
