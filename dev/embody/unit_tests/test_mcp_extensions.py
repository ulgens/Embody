"""
Test suite: MCP extension creation handler in EnvoyExt.
...
class TestMCPExtensions(EmbodyTestCase):
    ...
    def test_create_extension_new_comp(self):
        result = self.envoy._create_extension(parent_path=self.sandbox.path, class_name="TestExt")
    ...
    def test_create_extension_existing_comp(self):
        ...
        result = self.envoy._create_extension(parent_path=comp.path, class_name="MyExt", existing_comp=True)
        self.assertTrue(result.get("success"))
    ...
    def MyMethod(self):
        return 42
"""
        result = self.envoy._create_extension(parent_path=self.sandbox.path, class_name="CustomExt", code=code)
        self.assertTrue(result.get("success"))

    def test_create_extension_nonexistent_parent(self):
        result = self.envoy._create_extension(parent_path="/nonexistent", class_name="BadExt")
        self.assertDictHasKey(result, "error")

    def test_create_extension_invalid_class_name(self):
        result = self.envoy._create_extension(parent_path=self.sandbox.path, class_name="123-invalid")
        self.assertDictHasKey(result, "error")

    def test_create_extension_reports_ext_index(self):
        result = self.envoy._create_extension(parent_path=self.sandbox.path, class_name="SlotTest")
