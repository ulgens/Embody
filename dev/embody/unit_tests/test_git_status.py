...
class TestParseGitPorcelain(EmbodyTestCase):
    ...
    def test_modified(self):
        ...
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"dev/embody/a.py": " M"})

    def test_untracked_counts_as_changed(self):
        out = "?? dev/embody/new.py\0"
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"dev/embody/new.py": "??"})

    def test_staged_add(self):
        out = "A  dev/embody/a.py\0"
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"dev/embody/a.py": "A "})

    def test_deleted(self):
        out = " D dev/embody/gone.py\0"
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"dev/embody/gone.py": " D"})
    ...
    def test_path_with_space_unquoted(self):
        ...
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"dev/my file.py": " M"})

    def test_garbage_tokens_ignored(self):
        # Stray/short tokens (no "XY " prefix) are skipped, not crashed on.
        out = "x\0\0 M ok.py\0"
        self.assertEqual(op.Embody.ext.Embody._parseGitPorcelain(out), {"ok.py": " M"})
...
class TestMapChangedToOps(EmbodyTestCase):
    ...
    def test_no_prefix_when_root_is_project(self):
        self.assertEqual(self._m({"foo.tox": "??"}, "", [("/y", "foo.tox")]), {"/y": "??"})
