...
class TestTDNSafetyGuards(EmbodyTestCase):
    def setUp(self):
        ...
        def _stub(title, message, buttons):
            self._captured.append({"title": title, "message": message, "buttons": list(buttons)})
    ...
    def test_always_skip_persists_ignore_preference(self):
        ...
        try:
            ...
            self.embody_ext._checkTDNContentSafety()
            self.assertEqual(
                self.embody.par.Tdndatsafety.eval(),
                "ignore",
    ...
    def test_skip_logs_success_summary(self):
        ...
        try:
            ...
            self.embody_ext._checkTDNContentSafety()
            new_logs = [e for e in self.embody_ext._log_buffer if e["id"] > log_count_before]
            # Look for a SUCCESS-level entry that names the key.
            success_entries = [
                e for e in new_logs if e.get("level") == "SUCCESS" and "soon_gone" in e.get("message", "")
            ]
            self.assertTrue(
                success_entries,
                f"Skip must log a SUCCESS summary naming dropped keys; got: {[e.get('message', '') for e in new_logs]}",
            )
        finally:
    ...
    def test_TD_MANAGED_DAT_TYPES_membership(self):
        ...
        for t in (
            ...
            self.assertIn(t, types, f"TD-managed DAT type {t!r} missing from skip set")
        ...
        for t in (
            ...
            self.assertNotIn(
                t,
                types,
                f"Callback DAT type {t!r} must NOT be in skip set -- callbacks hold user-authored Python",
            )
        # Common user-authored types that must never be skipped
        for t in ("text", "table"):
            self.assertNotIn(t, types, f"User-authored type {t!r} must NOT be in skip set")
    ...
    def test_findAtRiskDATs_ignores_td_managed_folder_dat(self):
        ...
        try:
            ...
            self.assertGreater(
                folder_dat.numRows,
                0,
                f"Test setup: folder DAT must have rows (got {folder_dat.numRows})",
            )
            flat = self._flatten_dats(self.embody_ext._findAtRiskDATs())
            self.assertNotIn(
                folder_dat.path,
                flat,
                "TD-managed folder DAT with content was incorrectly flagged as at-risk",
            )
        finally:
            folder_dat.destroy()
    ...
    def test_findAtRiskDATs_keeps_callback_dats(self):
        ...
        cb_dat.text = "# user-authored callback\ndef onValueChange(channel, sampleIndex, val, prev):\n\tpass\n"
        try:
            flat = self._flatten_dats(self.embody_ext._findAtRiskDATs())
            self.assertIn(
                cb_dat.path,
    ...
    def test_findAtRiskDATs_flags_plain_text_dat(self):
        ...
        try:
            flat = self._flatten_dats(self.embody_ext._findAtRiskDATs())
            self.assertIn(text_dat.path, flat, "Plain textDAT with content must still be flagged")
        finally:
            text_dat.destroy()
