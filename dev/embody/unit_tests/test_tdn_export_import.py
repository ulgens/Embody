...
class TestTDNExportImport(EmbodyTestCase):
    ...
    def test_export_max_depth_zero(self):
        ...
        result = self.tdn.ExportNetwork(root_path=self.sandbox.path, max_depth=0)
        tdn = result["tdn"]
    ...
    def test_roundtrip_preserves_multiline_dat_content(self):
        ...
        dat.text = original
        export = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
    ...
    def test_import_basic(self):
        ...
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=export["tdn"])
        self.assertTrue(result.get("success"))
    ...
    def test_import_clear_first(self):
        ...
        tdn = {"operators": []}
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=True)
        self.assertTrue(result.get("success"))
        self.assertEqual(len(target.children), 0)

    def test_import_nonexistent_target(self):
        result = self.tdn.ImportNetwork(target_path="/nonexistent_imp", tdn={"operators": []})
        self.assertDictHasKey(result, "error")

    def test_import_invalid_tdn(self):
        target = self.sandbox.create(baseCOMP, "invalid_target")
        result = self.tdn.ImportNetwork(target_path=target.path, tdn="not a dict")
        self.assertDictHasKey(result, "error")
    ...
    def test_import_operators_array_directly(self):
        ...
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=export["tdn"]["operators"])
        self.assertTrue(result.get("success"))
    ...
    def test_import_clear_first_removes_existing_then_imports(self):
        ...
        tdn = {"operators": [{"name": "new_dat", "type": "textDAT"}]}
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=True)
        self.assertTrue(result.get("success"))
        child_names = [c.name for c in target.children]
    ...
    def test_import_without_clear_keeps_existing(self):
        ...
        tdn = {"operators": [{"name": "added_dat", "type": "textDAT"}]}
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=False)
        self.assertTrue(result.get("success"))
        child_names = [c.name for c in target.children]
    ...
    def test_import_merge_mode_renamed_op_gets_correct_position(self):
        ...
        tdn = {"operators": [{"name": "text1", "type": "textDAT", "position": [300, 400]}]}
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=False)
        self.assertTrue(result.get("success"))
    ...
    def test_import_merge_mode_renamed_op_gets_correct_params(self):
        ...
        tdn = {"operators": [{"name": "text1", "type": "textDAT", "parameters": {"language": "glsl"}}]}
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=False)
        self.assertTrue(result.get("success"))
    ...
    def test_import_merge_mode_logs_rename_warning(self):
        ...
        self.tdn.ImportNetwork(target_path=target.path, tdn=tdn, clear_first=False)
    ...
    def test_import_preserves_positions(self):
        ...
        tdn = {"operators": [{"name": "pos_op", "type": "textDAT", "position": [250, 175]}]}
    ...
    def test_export_storage_type_wrappers(self):
        ...
        ops = result["tdn"]["operators"]
        op_data = [o for o in ops if o["name"] == "c"][0]
        self.assertEqual(op_data["storage"]["my_tuple"], {"$type": "tuple", "$value": [1, 2]})
    ...
    def test_import_restores_storage(self):
        ...
        self.assertIsNotNone(imported)
        self.assertEqual(imported.fetch("count", None, search=False), 42)
        self.assertEqual(imported.fetch("label", None, search=False), "test")
    ...
    def test_import_restores_typed_storage(self):
        ...
        self.assertEqual(imported.fetch("coords", None, search=False), (10, 20))
        self.assertEqual(imported.fetch("tags", None, search=False), {"a", "b"})
    ...
    def test_import_restores_startup_storage(self):
        ...
        self.assertEqual(imported.fetch("version", None, search=False), 1)
        self.assertEqual(imported.fetch("mode", None, search=False), "auto")
