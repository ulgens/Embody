...
class _FakeTask:
    def __init__(self, target=None, args=(), SuccessHook=None, ExceptHook=None, RefreshHook=None):
        self.target = target
        self.args = args
...
class EnvoyLifecycleContractBase(EmbodyTestCase):
    ...
    def setUp(self):
        ...
        self._patch(self.envoy_mod, "run", lambda *a, **k: self._runs.append((a, k)))
    ...
    def _prepareMockedStart(self, enqueue, port=None):
        ...
        self._patch(self.envoy, "_findAvailablePort", lambda base_port, range_size=10: port)
...
class TestH1StartupStatusTruth(EnvoyLifecycleContractBase):
    ...
    def test_H1_starting_window_suppresses_duplicate_start(self):
        ...
        self.assertTrue(self.envoy._starting, "starting window should be open after _continueStart")
        ...
        self.assertEqual(before, after, "Start() must be a no-op while a start is in progress")
