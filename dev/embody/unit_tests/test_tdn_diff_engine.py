            ...
            "operators": [
                {
                    "name": "c",
                    "type": "baseCOMP",
                    "custom_pars": {"Settings": {"$t": "settings", "Speed": 5}},
                }
            ],
...
class TestTDNDiffEngine(EmbodyTestCase):
    def test_compression_equivalence_is_empty(self):
        ...
        self.assertEqual(d["counts"], {"added": 0, "removed": 0, "modified": 0})
    ...
    def test_operator_reorder_is_clean(self):
        ...
        self.assertFalse(op.Embody.ext.TDN._diff_normalized(a, b, comp_path="/p")["changed"])
    ...
    def test_op_parameter_change_detail(self):
        a = {"operators": [{"name": "lv", "type": "levelCHOP", "parameters": {"gain": "0.5"}}]}
        b = {"operators": [{"name": "lv", "type": "levelCHOP", "parameters": {"gain": "0.8"}}]}
        m = op.Embody.ext.TDN._diff_normalized(a, b, comp_path="/p")["modified"][0]
        self.assertEqual(m["changed_keys"], ["parameters"])
        self.assertEqual(m["changes"]["parameters"], [{"name": "gain", "old": "0.8", "new": "0.5"}])
    ...
    def test_annotation_change_is_separate_kind(self):
        a = {
            "operators": [],
            "annotations": [{"name": "Note1", "mode": "comment", "title": "old"}],
        }
        b = {
            "operators": [],
            "annotations": [{"name": "Note1", "mode": "comment", "title": "new"}],
        }
        d = op.Embody.ext.TDN._diff_normalized(a, b, comp_path="/p")
    ...
    def test_tdn_ref_pointer_change(self):
        ...
        m = op.Embody.ext.TDN._diff_normalized(a, b, comp_path="/p")["modified"][0]
        self.assertEqual(m["changes"]["tdn_ref"], {"old": "old.tdn", "new": "ch.tdn"})
    ...
    def test_truncation_is_honest(self):
        live = {"operators": [{"name": "o%d" % i, "type": "noiseTOP", "parameters": {"period": i}} for i in range(50)]}
        disk = {
            "operators": [
                {
                    "name": "o%d" % i,
                    "type": "noiseTOP",
                    "parameters": {"period": i + 100},
                }
                for i in range(50)
            ]
        }
        d = op.Embody.ext.TDN._diff_normalized(live, disk, comp_path="/p", max_changed_ops=10)
    ...
    def test_truncation_caps_added_and_removed(self):
        ...
        live = {"operators": [{"name": "a%d" % i, "type": "noiseTOP"} for i in range(40)]}
        disk = {"operators": [{"name": "d%d" % i, "type": "noiseTOP"} for i in range(40)]}
        d = op.Embody.ext.TDN._diff_normalized(live, disk, comp_path="/p", max_changed_ops=10)
    ...
    def test_identical_is_clean(self):
        same = {
            "type": "baseCOMP",
            "operators": [{"name": "x", "type": "noiseTOP", "parameters": {"period": 3}}],
        }
        self.assertFalse(op.Embody.ext.TDN._diff_normalized(same, copy.deepcopy(same), comp_path="/p")["changed"])

    def test_baseline_and_schema(self):
        d = op.Embody.ext.TDN._diff_normalized({"operators": []}, {"operators": []}, comp_path="/p")
        self.assertEqual(d["baseline"], "disk")
        self.assertEqual(d["schema_version"], op.Embody.ext.TDN._DIFF_SCHEMA_VERSION)

    def test_baseline_label_parameterized(self):
        # HEAD mode reuses the same engine with baseline='head'.
        d = op.Embody.ext.TDN._diff_normalized({"operators": []}, {"operators": []}, comp_path="/p", baseline="head")
...
class TestTDNDiffDatContent(EmbodyTestCase):
    ...
    def test_v15_array_vs_v2_string_normalizes_equal(self):
        ...
        self.assertEqual(tdn._normalize_tdn_for_compare(disk), tdn._normalize_tdn_for_compare(live))
    ...
    def test_v15_unchanged_dat_does_not_diff(self):
        ...
        env = op.Embody.ext.TDN._diff_normalized(live, disk)
        self.assertFalse(
            env["changed"],
            "v1.5/v2.0 dat_content reformat must not diff: %s" % env.get("counts"),
        )
