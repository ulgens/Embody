...
class TestTDNYaml(EmbodyTestCase):
    ...
    def test_tdn_yaml_roundtrip(self):
        ...
        for rel in names:
            ...
            self.assertEqual(reloaded, doc, f"{rel}: round-trip mismatch")
            # Determinism: re-dump must be byte-identical.
            self.assertEqual(self.tdn.tdn_dump(doc), dumped, f"{rel}: re-dump not byte-identical")
        if checked == 0:
    ...
    def test_tdn_block_scalar_lossless(self):
        ...
        for label, (s, indicator) in cases.items():
            dumped = self.tdn.tdn_dump({"dat_content": s, "dat_content_format": "text"})
            self.assertIn(indicator, dumped, f"{label}: expected chomping indicator {indicator!r}")
            back = self.tdn.tdn_load(dumped)
            self.assertEqual(back["dat_content"], s, f"{label}: not byte-identical")
        ...
        for label, s in tricky.items():
            back = self.tdn.tdn_load(self.tdn.tdn_dump({"dat_content": s, "dat_content_format": "text"}))
            self.assertEqual(back["dat_content"], s, f"{label}: tricky string not byte-identical")
    ...
    def test_tdn_tab_shader_lossless(self):
        ...
        """
        shader = "void main()\n{\n\tvec4 c = vec4(1.0);\n\tfragColor = c;\n}"
        dumped = self.tdn.tdn_dump({"dat_content": shader, "dat_content_format": "text"})
        ...
        self.assertIn('"', dumped, "tab shader should serialize as a double-quoted scalar")
        back = self.tdn.tdn_load(dumped)
        self.assertEqual(
            back["dat_content"],
            shader,
    ...
    def test_tdn_typing_safety(self):
        ...
        for i, v in enumerate(ambiguous):
            ...
            self.assertEqual(back[key], v, f"{v!r} changed value to {back[key]!r}")
        ...
        for key, val in shorthand.items():
            self.assertEqual(back[key], val, f"{val!r} did not survive as str")
        # The leading '=' and '~' tokens must appear UNQUOTED in the output.
        self.assertIn("=me.time.frame", dumped)
        self.assertIn("~Speed", dumped)
        self.assertNotIn("'=me.time.frame'", dumped, "expression shorthand must not be quoted")
        self.assertNotIn("'~Speed'", dumped, "bind shorthand must not be quoted")
    ...
    def test_tdn_backcompat_legacy_json(self):
        ...
        self.assertEqual(parsed["operators"][0]["dat_content"], ["one", "two", "three"])

        # Import it and confirm the list dat_content is rejoined with '\n'.
        target = self.sandbox.create(baseCOMP, "legacy_json_target")
        result = self.tdn.ImportNetwork(target_path=target.path, tdn=parsed)
        ...
        self.assertRaises(yaml.YAMLError, yaml.load, legacy_text, Loader=yaml.SafeLoader)
    ...
    def test_tdn_backcompat_bom_legacy_json(self):
        ...
        self.assertRaises(yaml.YAMLError, yaml.load, bom_text, Loader=yaml.SafeLoader)
    ...
    def test_tdn_determinism(self):
        ...
        for _ in range(5):
            self.assertEqual(self.tdn.tdn_dump(doc), first, "tdn_dump is not deterministic")
    ...
    def test_tdn_dumper_isolation(self):
        ...
        DOES."""
        plain = yaml.dump({"k": "a\nb"})
        self.assertNotIn("|", plain, "global SafeDumper unexpectedly uses block style")
        ours = self.tdn.tdn_dump({"k": "a\nb"})
        self.assertIn("|", ours, "tdn_dump should use a literal block scalar for multi-line text")
    ...
    def test_tdn_validate_yaml_file(self):
        ...
        try:
            ...
            Path(yaml_fp).write_text(self.tdn.tdn_dump(doc), encoding="utf-8")
            v = self.tdn._validate_tdn_file(yaml_fp)
            self.assertTrue(v.get("valid"), f"v2.0 YAML should validate: {v}")
            # Legacy JSON
            json_fp = os.path.join(d, "legacy.tdn")
            Path(json_fp).write_text(json.dumps(doc, indent="\t"), encoding="utf-8")
            v2 = self.tdn._validate_tdn_file(json_fp)
            self.assertTrue(v2.get("valid"), f"legacy JSON should still validate: {v2}")
        finally:
            import shutil
    ...
    def _load_textconv(self):
        ...
        fp = os.path.join(project.folder, "embody", "Embody", "templates", "text_tdn_textconv.py")
        if not os.path.isfile(fp):
            return None
        spec = importlib.util.spec_from_file_location("tdn_textconv_under_test", fp)
    ...
    def test_tdn_save_path_writes_yaml(self):
        ...
        self.assertFalse(out.lstrip().startswith("{"), "save-path serializer emitted JSON, not YAML")
    ...
    def test_tdn_boilerplate_omission(self):
        ...
        self.assertIsNotNone(entry, "compute DAT should still appear as an operator entry")
        self.assertNotIn(
            "dat_content",
            entry,
        ...
        self.tdn.ImportNetwork(target_path=target.path, tdn=result["tdn"], clear_first=True)
        imported_glsl = target.op("glsl_omit")
        self.assertIsNotNone(imported_glsl)
        imported_compute = target.op(f"{compute.name}")
        self.assertIsNotNone(imported_compute, "TD should recreate the docked compute DAT on import")
        self.assertEqual(
            imported_compute.text,
            default_text,
        ...
        result2 = self.tdn.ExportNetwork(root_path=self.sandbox.path, include_dat_content=True)
        entry2 = self._find_op(result2["tdn"]["operators"], compute.name)
        self.assertIsNotNone(entry2)
        self.assertIn("dat_content", entry2, "customized compute DAT must be exported")
