...
class TestVerifyMcpImportableFastPath(EmbodyTestCase):
    def setUp(self):
        ...
        self._saved = {k: v for k, v in sys.modules.items() if k == "mcp" or k.startswith("mcp.")}
    ...
    def test_A03_only_mcp_in_modules_not_mcp_server_does_not_short_circuit(self):
        ...
        self.assertIsNot(
            surviving,
            half_loaded,
            "half-loaded mcp parent must have been cleared and re-imported (or removed when import failed)",
        )
...
class TestVenvPaths(EmbodyTestCase):
    ...
    def test_deps_pin_mcp_and_attrs(self):
        spec = self.ext._venvPaths()
        self.assertTrue(
            any(d.startswith("mcp>=") for d in spec["deps"]),
    ...
    def test_wire_python_paths_is_idempotent(self):
        ...
        try:
            ...
            self.assertLessEqual(
                sys.path.count(site_packages) - start_count,
                1,
...
class TestEnvironmentNeedsInstall(EmbodyTestCase):
    ...
    def _make_dist(self, name, version):
        ...
        os.makedirs(os.path.join(self.tmp, f"{name}-{version}.dist-info"), exist_ok=True)
    ...
    def test_attrs_25_forces_install(self):
        ...
        self.assertTrue(
            self.ext._environmentNeedsInstall(self._spec()),
...
class TestInstallDependenciesWorkerSafe(EmbodyTestCase):
    ...
    def test_success_logs_via_callback_not_Log(self):
        ...
        msgs = []
        ok = self.ext._installDependencies(self._spec(), log=lambda m, lvl="INFO": msgs.append((lvl, m)))
        self.assertTrue(ok)
