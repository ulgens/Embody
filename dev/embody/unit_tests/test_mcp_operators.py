"""
Test suite: MCP operator management handlers in EnvoyExt.
...
class TestMCPOperators(EmbodyTestCase):
    ...
    def test_create_op_valid(self):
        result = self.envoy._create_op(parent_path=self.sandbox.path, op_type="baseCOMP", name="test_comp")
        self.assertDictHasKey(result, "path")
        self.assertIn("test_comp", result["path"])

    def test_create_op_without_name(self):
        result = self.envoy._create_op(parent_path=self.sandbox.path, op_type="textDAT")
        self.assertDictHasKey(result, "path")

    def test_create_op_invalid_parent(self):
        result = self.envoy._create_op(parent_path="/nonexistent/path", op_type="baseCOMP")
        self.assertDictHasKey(result, "error")

    def test_create_op_in_non_comp(self):
        dat = self.sandbox.create(textDAT, "not_a_comp")
        result = self.envoy._create_op(parent_path=dat.path, op_type="baseCOMP")
        self.assertDictHasKey(result, "error")
    ...
    def test_delete_op_existing(self):
        ...
        result = self.envoy._delete_op(op_path=comp.path)
        self.assertTrue(result.get("success"))

    def test_delete_op_nonexistent(self):
        result = self.envoy._delete_op(op_path="/nonexistent/op")
        self.assertDictHasKey(result, "error")
    ...
    def test_get_op_returns_info(self):
        ...
        result = self.envoy._get_op(op_path=comp.path)
    ...
    def test_get_op_nonexistent(self):
        result = self.envoy._get_op(op_path="/nonexistent")
        self.assertDictHasKey(result, "error")
    ...
    def test_copy_op_basic(self):
        ...
        result = self.envoy._copy_op(source_path=source.path, dest_parent=self.sandbox.path, new_name="copy")
    ...
    def test_copy_op_without_name(self):
        ...
        result = self.envoy._copy_op(source_path=source.path, dest_parent=self.sandbox.path)
        self.assertDictHasKey(result, "new_path")
    ...
    def test_rename_op(self):
        ...
        result = self.envoy._rename_op(op_path=comp.path, new_name="new_name")
        self.assertTrue(result.get("success"))

    def test_rename_op_nonexistent(self):
        result = self.envoy._rename_op(op_path="/nonexistent", new_name="whatever")
        self.assertDictHasKey(result, "error")
    ...
    def test_query_network_basic(self):
        ...
        result = self.envoy._query_network(parent_path=self.sandbox.path)
    ...
    def test_query_network_with_type_filter(self):
        ...
        result = self.envoy._query_network(parent_path=self.sandbox.path, op_type="baseCOMP")
        for op_info in result["operators"]:
            self.assertEqual(op_info["type"], "baseCOMP")

    def test_query_network_nonexistent(self):
        result = self.envoy._query_network(parent_path="/nonexistent")
        self.assertDictHasKey(result, "error")
    ...
    def test_find_children_by_name(self):
        ...
        result = self.envoy._find_children(op_path=self.sandbox.path, name="target*")
        self.assertDictHasKey(result, "operators")
    ...
    def test_find_children_by_type(self):
        ...
        result = self.envoy._find_children(op_path=self.sandbox.path, type="textDAT")
    ...
    def test_cook_op(self):
        ...
        result = self.envoy._cook_op(op_path=comp.path)
        self.assertTrue(result.get("success"))

    def test_cook_op_nonexistent(self):
        result = self.envoy._cook_op(op_path="/nonexistent")
        self.assertDictHasKey(result, "error")
