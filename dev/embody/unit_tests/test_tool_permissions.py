...
class TestToolPermissions(EmbodyTestCase):
    ...
    def test_readonly_tools_exclude_mutating_ops(self):
        ...
        self.assertFalse(
            forbidden & set(self._env.READ_ONLY_TOOLS),
    ...
    def test_deploy_leave_writes_nothing(self):
        ...
        self.assertFalse(
            (cdir / "settings.local.json").exists(),
    ...
    def test_deploy_merges_preserving_user_keys(self):
        ...
        user = {
            "permissions": {
                "allow": ["Bash", "Edit(/x/**)", "mcp__envoy__get_op"],
                "additionalDirectories": ["/tmp"],
            },
            "hooks": {"Stop": "x"},
            "model": "sonnet",
    ...
    def test_deploy_is_idempotent(self):
        ...
        self.assertEqual(before, after, "an already-satisfying file must not be rewritten (no churn)")
