...
class TestSmokeRelease(EmbodyTestCase):
    ...
    def test_message_box_auto_response(self):
        ...
        self.embody.store("_smoke_test_responses", {"Test Dialog": 1})
        result = self.embody_ext._messageBox("Test Dialog", "Test message", buttons=["Cancel", "OK"])
        self.assertEqual(result, 1)
        responses = self.embody.fetch("_smoke_test_responses", None, search=False)
        self.assertIsNone(responses, "Storage should be unstored when last response is consumed")

    def test_message_box_multiple_responses(self):
        """Multiple seeded responses are consumed independently."""
        self.embody.store("_smoke_test_responses", {"Dialog A": 0, "Dialog B": 2})
        result_a = self.embody_ext._messageBox("Dialog A", "msg", buttons=["OK"])
        self.assertEqual(result_a, 0)
        responses = self.embody.fetch("_smoke_test_responses", None, search=False)
        self.assertIsNotNone(responses)
        self.assertIn("Dialog B", responses)
        result_b = self.embody_ext._messageBox("Dialog B", "msg", buttons=["A", "B", "C"])
    ...
    def test_message_box_no_response_seeded(self):
        ...
        responses = self.embody.fetch("_smoke_test_responses", None, search=False)
        self.assertIsNone(responses, "No responses should be seeded at test start")

    def test_message_box_unmatched_title_left_intact(self):
        """A title with no matching response is left for ui.messageBox."""
        self.embody.store("_smoke_test_responses", {"Other Dialog": 1})
    ...
    def test_no_script_errors(self):
        ...
        self.assertEqual(len(errors), 0, f"Embody has script errors: {errors}")
    ...
    def test_promoted_methods_exist(self):
        ...
        for method_name in ["Update", "Save", "Verify", "Reset"]:
            ...
            self.assertIsNotNone(method, f"Promoted method {method_name} missing")
    ...
    def test_global_op_shortcut(self):
        ...
        self.assertIs(op.Embody, self.embody, "op.Embody should resolve to the Embody COMP")
    ...
    def test_v6_uninstall_pulse_and_handler_present(self):
        ...
        for method_name in ["UninstallHandler", "Uninstall", "PreviewUninstall"]:
            method = getattr(self.embody, method_name, None)
            self.assertIsNotNone(method, f"Promoted method {method_name} missing from the release build")
            self.assertTrue(callable(method), f"{method_name} should be callable")
    ...
    def test_log_buffer_initialized(self):
        ...
        self.assertGreater(len(buffer), 0, "Log buffer should have entries after init")
    ...
    def test_prompt_envoy_skip(self):
        ...
        try:
            self.embody.store("_smoke_test_responses", {"Embody - AI Coding Assistant Integration": 0})
            self.embody_ext._promptEnvoy()
            self.assertFalse(
                self.embody.par.Envoyenable.eval(),
    ...
    def test_prompt_envoy_enable(self):
        ...
        self.embody.par.Envoyenable = False
        try:
            self.embody.store("_smoke_test_responses", {"Embody - AI Coding Assistant Integration": 1})
            self.embody_ext._promptEnvoy()
            self.assertTrue(
                self.embody.par.Envoyenable.eval(),
    ...
    def test_v6_tdn_clipboard_methods_exist_and_callable(self):
        ...
        for name in (
            ...
            self.assertIsNotNone(method, f"TDN clipboard method {name} is missing on the release .tox")
            self.assertTrue(callable(method), f"TDN clipboard method {name} is not callable")
    ...
    def test_v6_collection_subcomp_and_scanner_present(self):
        ...
        self.assertIsNotNone(collection.op("scanner"), "Collection/scanner DAT must exist")
        self.assertIsNotNone(collection.op("safe_import"), "Collection/safe_import DAT must exist")
    ...
    def test_v6_collection_scan_clean_tdn_returns_clean(self):
        ...
        self.assertIn("verdict", result, "Capability report must carry a verdict key")
        self.assertEqual(
            result["verdict"],
            "clean",
    ...
    def test_v6_tdn_v2_yaml_roundtrip_dat_byte_identical(self):
        ...
        try:
            export = tdn.ExportNetwork(root_path=src.path, output_file=fp, include_dat_content=True)
            self.assertTrue(export.get("success"), f"ExportNetwork failed: {export}")
            self.assertTrue(Path(fp).exists(), "ExportNetwork did not write the .tdn file")

            # The on-disk file must parse as YAML and self-identify as TDN.
            with open(fp, "r", encoding="utf-8") as f:
                doc = yaml.safe_load(f)
            self.assertEqual(doc.get("format"), "tdn", "Exported .tdn must declare format == 'tdn'")
            ...
            self.assertTrue(result.get("success"), f"ImportNetwork failed: {result}")

            imported = target.op("multiline")
            self.assertIsNotNone(imported, "multiline DAT was not recreated on import")
            self.assertEqual(
                imported.text,
                payload,
    ...
    def test_v6_glsl_externalizes_to_dot_glsl_with_glsl_tag(self):
        ...
        try:
            self.assertTrue(result.get("success"), f"GLSL externalize failed: {result}")
            ...
            self.assertEqual(
                result.get("tag"),
                glsl_tag,
                f"glsl-language DAT must infer the glsl tag {glsl_tag!r} (the v6.0.34 fix), got {result.get('tag')!r}",
            )
        finally:
    ...
    def test_v6_envoy_watchdog_methods_exist(self):
        ...
        for name in ("_watchdogTick", "_reviveDeadServer", "_probeAlive"):
            ...
            self.assertIsNotNone(method, f"Envoy watchdog method {name} is missing on the release .tox")
            self.assertTrue(callable(method), f"Envoy watchdog method {name} is not callable")
    ...
    def test_v6_envoy_watchdog_armed(self):
        ...
        self.assertGreater(int(gen), 0, "Envoy watchdog generation must be > 0 (no loop was armed)")
    ...
    def test_v6_tdn_feature_roundtrip_pop_float_exclude(self):
        ...
        self.assertGreaterEqual(len(rt.inputs), 1, "transformPOP lost its input wire on import")
        self.assertEqual(rt.inputs[0].name, "pop_grid", "POP chain wiring not preserved")
        ...
        self.assertIsNotNone(speed, "Custom Float parameter Speed was lost on import")
        self.assertApproxEqual(speed.default, 2.5, msg="Custom Float default value was not preserved")
