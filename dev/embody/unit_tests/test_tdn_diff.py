...
class TestDiffTdnHandler(EmbodyTestCase):
    ...
    def _cleanup(self, comp, rel):
        if rel:
            try:
                p = str(self.embody_ext.buildAbsolutePath(self.embody_ext.normalizePath(rel)))
                if os.path.isfile(p):
                    os.remove(p)
            except Exception:
                pass
    ...
    def test_clean_right_after_externalize(self):
        ...
        try:
            ...
            self.assertFalse(
                res["changed"],
                "a freshly externalized COMP must diff clean (live == disk); got: %s" % res.get("counts"),
            )
        finally:
    ...
    def test_detects_child_param_change(self):
        ...
        try:
            ...
            res = op.Embody.ext.Envoy._diff_tdn(comp.path)
            self.assertNotIn("error", res)
            self.assertTrue(res["changed"], "a live param edit must show as changed")
            paths = [m["path"] for m in res["modified"]]
            self.assertTrue(
                any(p.endswith("/c") for p in paths),
    ...
    def test_detects_root_param_change(self):
        ...
        try:
            ...
            self.assertIn("root", kinds, "root COMP own-par change must surface as kind=root")
        finally:
            self._cleanup(comp, rel)
