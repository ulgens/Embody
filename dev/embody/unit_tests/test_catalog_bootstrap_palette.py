...
class TestCatalogBootstrapPalette(EmbodyTestCase):
    ...
    def test_A01_build_match_returns_entries(self):
        ...
        self.assertEqual(out["3DScope"], {"type": "baseCOMP", "min_children": 2})
        self.assertEqual(out["TDVR"], {"type": "containerCOMP", "min_children": 13})
    ...
    def test_C01_shipped_catalog_covers_current_build(self):
        ...
        self.assertGreater(
            len(out),
            100,
            f"Shipped palette_catalog has only {len(out)} entries for {build_str} -- expected >100.",
        )
