...
class TestUninstallHandler(EmbodyTestCase):
    def setUp(self):
        ...
        self._saved_resp = op.Embody.fetch("_smoke_test_responses", None, search=False)
    ...
    def _seed_footprint(self):
        ...
        self._embody_json("generated-hashes.json", {"CLAUDE.md": self.ext._contentHash(body)})
    ...
    def test_cancel_removes_nothing(self):
        ...
        self.assertFalse(result.get("ran", True), "Cancel must not run the uninstall")
        self.assertEqual(result.get("reason"), "cancelled")
        self.assertTrue(self._exists("CLAUDE.md"), "Cancel must delete nothing")
    ...
    def test_suppressed_defers_like_cancel(self):
        ...
        op.Embody.unstore("_smoke_test_responses")
        result = self.ext.UninstallHandler(target_dir=self.d)
        self.assertFalse(result.get("ran", True), "a suppressed confirm must not uninstall")
    ...
    def test_confirm_uninstalls(self):
        ...
        self.assertTrue(result.get("ran", False), "confirm must run the uninstall")
        self.assertGreaterEqual(result.get("deleted", 0), 1)
        self.assertFalse(self._exists("CLAUDE.md"), "confirm must remove the generated config file")
        self.assertFalse(self._exists(".embody"), "confirm must remove the .embody state dir")
    ...
    def test_confirm_keeps_edited_file_via_review(self):
        ...
        self.assertTrue(self._exists("CLAUDE.md"), "an edited generated file must survive confirm")
