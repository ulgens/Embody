...
class TestTDNExclude(EmbodyTestCase):
    ...
    def _export_doc(self, parent, embed_all=False):
        result = self.tdn_ext.ExportNetwork(parent.path, output_file=None, embed_all=embed_all)
    ...
    def test_getTags_excludes_exclude_tag_from_all_selectors(self):
        ...
        self.assertNotIn(
            tag,
            self.embody_ext.getTags(),
            "Exclude tag must not appear in getTags() (all)",
        )
        self.assertNotIn(
            tag,
            self.embody_ext.getTags("DAT"),
            "Exclude tag must not appear in the DAT selector - it would wrongly drive DAT externalization",
        )
    ...
    def test_export_omits_excluded_comp(self):
        ...
        self.assertNotIn("drop", names, "Excluded COMP must not appear in the export")
    ...
    def test_export_emits_no_ref_for_excluded(self):
        ...
        self.assertNotIn(
            "drop",
            blob,
            "No trace of the excluded COMP (inline, tdn_ref, or tox_ref) may appear anywhere in the export",
        )
        self.assertNotIn("inside", blob, "The excluded subtree must be fully absent from the export")
        self.assertNotIn("tdn_ref", blob, "Exclusion must not emit a tdn_ref (unlike the .tdn tag)")
    ...
    def test_collectAllPaths_skips_excluded_subtree(self):
        ...
        self.assertNotIn(drop.path, paths, "Excluded COMP must not be collected")
        self.assertNotIn(inside.path, paths, "Descendants of an excluded COMP must not be collected")
    ...
    def test_reconstruct_preserves_excluded_subtree(self):
        ...
        self.tdn_ext.ImportNetwork(target_path=parent.path, tdn=doc, clear_first=True)
        self.assertIsNotNone(
            op(parent.path + "/drop"),
            "Excluded COMP must survive clear_first reconstruction",
        )
        self.assertIsNotNone(
            op(parent.path + "/drop/inside"),
            "Excluded COMP subtree must survive intact",
        )
        self.assertIsNotNone(op(parent.path + "/keep"), "Normal child must be rebuilt from the doc")
    ...
    def test_reconstruct_does_not_duplicate_excluded(self):
        ...
        self.assertEqual(len(drops), 1, f"Exactly one excluded COMP expected, found {len(drops)}")
    ...
    def test_strip_preserves_excluded_comp(self):
        ...
        self.assertIsNone(op(parent.path + "/keep"), "Normal child must be stripped")
    ...
    def test_tag_removal_restores_export(self):
        ...
        self.assertIn("drop", names, "After tag removal the COMP must be exported normally")
    ...
    def test_cascade_autotag_skips_excluded(self):
        ...
        self.assertIn(keep.path, calls, "Normal child must be cascade-tagged")
        self.assertNotIn(
            drop.path,
            calls,
    ...
    def test_nested_excluded_under_normal_warns_and_is_preserved(self):
        ...
        doc = self._export_doc(parent)
        new = [e for e in self.embody_ext._log_buffer if e["id"] > before]
        warns = [e for e in new if e.get("level") == "WARNING" and "nested under" in e.get("message", "")]
        self.assertTrue(
            warns,
            f"Excluded COMP nested under a non-excluded child must warn; got: {[e.get('message', '') for e in new]}",
        )
    ...
    def test_hasExcludeTag_false_for_annotation_comp(self):
        ...
        self.assertFalse(
            self.tdn_ext._hasExcludeTag(note),
    ...
    def test_excluded_child_does_not_dirty_parent(self):
        ...
        try:
            ...
            self.assertFalse(
                self.embody_ext._isTDNDirty(parent),
    ...
    def test_excluded_comp_unmodified_by_reconstruct(self):
        ...
        self.assertEqual(
            d2.fetch("app_state", None, search=False),
            {"k": 1},
            "excluded COMP storage must be unmodified",
        )
        self.assertEqual(d2.nodeX, 1234, "excluded COMP position must be unmodified")
    ...
    def test_export_has_no_structural_reference_to_excluded(self):
        ...
        for r in refs:
            self.assertNotIn("drop", r, f"Excluded COMP must not be referenced anywhere: {r}")
