...
class TestTDNExternalConnections(EmbodyTestCase):
    ...
    def test_strip_then_import_restores_input_wire(self):
        ...
        self.assertTrue(stashed, "StripCompChildren did not stash external wires")

        imp = self.tdn.ImportNetwork(target_path=base.path, tdn=tdn_doc, clear_first=True)
        ...
        self.assertEqual(
            self._countInputWires(base),
            1,
            "External input wire not restored after strip+rebuild",
        )
        # Stash consumed.
        self.assertEqual(base.fetch("_tdn_external_wires", [], search=False), [])
    ...
    def test_strip_then_import_restores_both_directions(self):
        ...
        self.assertEqual(self._countInputWires(base), 1, "Input wire not restored")
        self.assertEqual(self._countInputWires(dst), 1, "Output wire not restored")
