...
def _load_theme():
    ...
    amber = (saved[0] + 0.12, max(0, saved[1] - 0.02), max(0, saved[2] - 0.04), 1.0)
    ...
    tdn_amber = (
        tdn_saved[0] + 0.12,
        max(0, tdn_saved[1] - 0.02),
        max(0, tdn_saved[2] - 0.04),
        1.0,
    )
...
def onRollover(comp, row, col, coords, prevRow, prevCol, prevCoords):
    ...
    if prevRow != row:
        if prevRow is not None and prevRow > 0 and prevRow < data.numRows:
            for c in range(ncols):
                _apply_cell(comp.cellAttribs[prevRow, c], prevRow, c, data, highlight=False)
        if row is not None and row > 0 and row < data.numRows:
            for c in range(ncols):
                _apply_cell(comp.cellAttribs[row, c], row, c, data, highlight=True)
    ...
    if prevRow == row and prevCol != col and row is not None and row > 0:
        if prevCol >= 0 and prevCol < ncols and row < data.numRows:
            _apply_cell(comp.cellAttribs[row, prevCol], row, prevCol, data, highlight=True)
...
def onSelect(comp, startRow, startCol, startCoords, endRow, endCol, endCoords, start, end):
    ...
    if col in (COL_EXPANDO, COL_PATH):
        ...
        if st == "Comp" and oper:
            ...
            run(
                f"op('{parent.Embody.op('window_tagging_menu')}').par.winopen.pulse()",
                delayFrames=2,
            )
            ...
            parent.Embody.op("tagger/switch_family").par.index = 2
            run(
                lambda s=st: parent.Embody.ext.Embody.SetupTaggerManageMode(oper, s),
                delayFrames=1,
            )
            run(
                f"op('{parent.Embody.op('window_tagging_menu')}').par.winopen.pulse()",
