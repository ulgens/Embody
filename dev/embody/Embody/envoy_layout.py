...
def lint_layout(comp):
    ...
    if len(zeros) >= 2:
        issues.append("%d ops stacked at (0,0): %s" % (len(zeros), ", ".join(z.name for z in zeros[:6])))
    n = len(main)
    if n <= 80:
        ov = 0
        for i in range(n):
            ...
            for j in range(i + 1, n):
                b = main[j]
                if (
                    a.nodeX < b.nodeX + b.nodeWidth
                    and a.nodeX + a.nodeWidth > b.nodeX
                    and a.nodeY < b.nodeY + b.nodeHeight
                    and a.nodeY + a.nodeHeight > b.nodeY
                ):
                    ov += 1
        if ov:
...
def same_network_docks(host):
    ...
    try:
        ...
        return [
            d
            for d in getattr(host, "docked", ())
            if d.valid and d.parent() is not None and d.parent().path == parent_path
        ]
    except Exception:
        return []
...
def find_non_overlapping_position(parent, new_op):
    ...
    siblings = [
        ...
        if child.path != new_op.path and child.path not in own_dock_paths and child.type != "annotate"
    ]
    if not siblings:
    ...
    def has_overlap(x, y):
        ...
        for sx, sy, sw, sh in rects:
            if x0 < sx + sw + MARGIN and x0 + fw + MARGIN > sx and y0 < sy + sh + MARGIN and y0 + fh + MARGIN > sy:
                return True
        return False
