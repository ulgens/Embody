...
class EmbodyExt:
    ...
    def _setupEnvironment(self):
        ...
        if self._environmentNeedsInstall(spec):
            msgs = []
            ok = self._installDependencies(spec, log=lambda m, lvl="INFO": msgs.append((lvl, m)))
            for lvl, m in msgs:
                self.Log(m, lvl)
            if not ok:
                return False
        ...
        if not self._wirePythonPaths(spec):
            self.Log(
                self._importGateFailureMessage(site_packages, "venv site-packages path is missing"),
                "ERROR",
            )
            return False
        if sys.platform.startswith("win"):
    ...
    def _findOrInstallUv(self, python_exe, log=None):
        ...
        if sys.platform.startswith("win"):
            ...
            mac_candidates = glob(os.path.join(home, "Library", "Python", "3.*", "bin", "uv")) + [
    ...
    def _cellVal(self, row, col, default: str = "") -> str:
        ...
        cell = table[row, col]
        if cell is None:
            if isinstance(row, int) and 0 < row < table.numRows and table[0, col] is not None:
                self.Log(
    ...
    def getOpPaths(
        self, opToExternalize: OP, externalizationsFolder: Optional[str] = None
    ...
    def _messageBox(self, title, message, buttons):
        ...
        if self.my.fetch("_suppress_dialogs", False, search=False):
            self.Log(
                f'Dialog "{title}" suppressed during save -- using default (-1).',
                "DEBUG",
            )
            return -1
    ...
    def _enableEnvoy(self):
        ...
        self.Log(
            f"Envoy enabled! Config generated for {client_label}. Connect your AI coding assistant via MCP.",
            "SUCCESS",
        )
    ...
    def _openSetupWizard(self):
        ...
        if win is None or wiz is None:
            self.Log("Setup wizard UI not found -- using the classic Envoy prompt.", "DEBUG")
            self._promptEnvoy()
            return
        logic = wiz.op("logic")
    ...
    def _applyWizardSetup(
        ...
        if assistant not in ("claudecode", "other", "none"):
            self.Log(
                f'Setup wizard: unrecognized assistant "{assistant}" -- no change made.',
                "WARNING",
            )
            return
        ...
        if assistant == "none":
            ...
            self.Log(
                ...
                "re-running setup (the Setup Wizard button).",
                "SUCCESS",
            )
            return
        if assistant == "claudecode":
            ...
            try:
                ...
                self.Log(
                    f'Unknown AI client "{client}" -- keeping the current selection.',
                    "WARNING",
                )
    ...
    def _enableEnvoyResolved(self):
        ...
        if git_root == "no-git":
            self.Log(
                "No git repo found -- generating MCP + AI config only (no "
                ".gitignore / .gitattributes). Run op.Embody.InitGit() "
                "later to add git integration.",
                "INFO",
            )
        ...
        if already_on:
            ...
            self.Log(f"Envoy setup updated for {self._aiClientLabel()}.", "SUCCESS")
            return
    ...
    def _atomicMove(self, src, dst):
        ...
        try:
            ...
            self.Log(
                f"Migration left source at {src}: {e}. The new copy at {dst} is valid; remove the source manually.",
                "WARNING",
            )

    def _migrateRootFiles(self, old_mode, new_mode, old_custom=None, new_custom=None):
        ...
        for orphan in critical_srcs:
            if orphan.is_file():
                ...
                try:
                    ...
                    self.Log(f"Could not rename orphan {orphan}: {e}", "WARNING")
        ...
        if old_settings.is_file():
            if new_settings.is_file():
                ...
                try:
                    ...
                    self.Log(f"Moved .claude/settings.local.json -> {new_root}", "INFO")
                except Exception as e:
                    self.Log(f"Could not move .claude/settings.local.json: {e}", "WARNING")

        # --- Pass 2: delete Embody-generated AI files at old root ---
        self._cleanupOldRootFiles(old_root)

        self.Log(
            f"AI config root: {old_mode} -> {new_mode}. Old root {old_root} cleaned, regenerating at {new_root}.",
            "INFO",
        )
            ...
            try:
                ...
                if "envoy" in servers:
                    ...
                    if servers:
                        ...
                        self.Log(
                            f"Pruned envoy server from {mcp_file} (other servers preserved)",
                            "DEBUG",
                        )
                    else:
                        mcp_file.unlink()
    ...
    def _guardFileWrite(self, category, action, details, apply_fn, mode=None):
        ...
        if self._startup_config_pass:
            ...
            self.Log(
                f"Advanced mode: deferred a repo change on open ({action}). "
                f"Nothing was written -- run op.Embody.InitEnvoy() / "
                f"InitGit(), or set Embodymode to Auto, to apply it.",
                "INFO",
            )
            return False
    ...
    def Uninstall(self, confirm=False, include_review=False, target_dir=None):
        ...
        return mod.embody_admin.uninstall(self, confirm=confirm, include_review=include_review, target_dir=target_dir)
    ...
    def Verify(self) -> None:
        ...
        if settings_restored and has_prior_data:
            ...
            if not self._suppressDialogs() and not getattr(self, "_pending_envoy_prompt", False):
                self.my.par.Envoyenable = False
    ...
    def safeDeleteTrackedFiles(self, folder_path: Union[str, Path]) -> tuple[int, int]:
        ...
        if skipped > 0:
            self.Log(
                f"SAFETY: Preserved {skipped} untracked file(s) in {folder_path}",
                "INFO",
            )
    ...
    def Disable(
        self,
        prevFolder: Union[str, bool, None] = False,
        removeTags: Union[bool, int] = False,
    ) -> None:
        """
    ...
    def normalizeAllPaths(self) -> None:
        ...
        if paths_fixed > 0:
            self.Log(
                f"Normalized {paths_fixed} path(s) for cross-platform compatibility",
                "SUCCESS",
            )
    ...
    def Update(self, suppress_refresh: bool = False) -> None:
        ...
        if not self._tdnEnabled() and tdn_comps:
            self.Log(
                f"TDN disabled -- skipping export for {len(tdn_comps)} tracked TDN COMP(s)",
                "INFO",
            )
        ...
        additions = [
            oper
            for oper in ops_to_externalize
    ...
    def _reportResults(self, dirties, additions, subtractions):
        ...
        if dirties:
            self.Log(
                f"Saved {len(dirties)} externalization{'s' if plural else ''}",
                "SUCCESS",
            )
        if additions:
            self.Log(
                f"Added {len(additions)} operator{'s' if plural else ''} in total",
                "SUCCESS",
            )
        if subtractions:
            self.Log(
                f"Removed {len(subtractions)} operator{'s' if plural else ''} in total",
                "SUCCESS",
            )
    ...
    def getTags(self, selection: Optional[str] = None) -> list[str]:
        ...
        tags = [par.eval() for par in self.my.pars("*tag") if par.name != "Tdnexcludetag"]
        if selection == "tox":
    ...
    def getExternalizedOps(self, opFamily: type, strategy: Optional[str] = None) -> list[OP]:
        ...
        has_strategy_col = "strategy" in [self._cellVal(0, c) for c in range(self.Externalizations.numCols)]
        ops = []
    ...
    def getOpsToExternalize(self, opFamily: type) -> list[OP]:
        ...
        base_filter = lambda x: (
            self.isOpEligibleToBeExternalized(x)
        ...
        if opFamily == COMP:
            ...
            tox_ops = self.root.findChildren(type=COMP, tags=tox_tags, parName="externaltox", key=base_filter)
            # TDN-tagged COMPs (no externaltox needed)
            tdn_tags = self.getTags("tdn")
            tdn_ops = self.root.findChildren(type=COMP, tags=tdn_tags, key=base_filter)
            return tox_ops + tdn_ops
        else:
            tags = self.getTags("DAT")
            return self.root.findChildren(type=DAT, tags=tags, parName="file", key=base_filter)
    ...
    def getOpsByPar(self, opFamily: type) -> list[OP]:
        ...
        if opFamily == COMP:
            return self.root.findChildren(
                type=COMP,
                key=lambda x: x.par.externaltox.eval() != "" and x.type not in ["engine", "time", "annotate"],
            )
        else:
            return self.root.findChildren(
                type=DAT,
    ...
    def isOpProcessable(self, oper: OP) -> bool:
        ...
        return (
            not self.isReplicant(oper)
            and not self.isInsideClone(oper)
            and not oper.path.startswith("/local/")
            and oper.path != "/local"
            and oper.type not in ["engine", "time", "annotate"]
        )
            ...
            if p.family == "COMP":
                ...
                if clone_par is not None and enable_par is not None:
                    try:
                        ...
                        if clone_val and clone_val is not p and enable_par.eval():
                            return True
                    except Exception:
                        pass
            p = p.parent()
    ...
    def Save(self, opPath: str) -> None:
        ...
        try:
            ...
            if not oper or oper.family != "COMP":
                self.Log(
                    f"Save() requires a COMP, got {oper.family if oper else 'None'}: {opPath}",
                    "ERROR",
                )
                return
    ...
    def SaveTDN(self, opPath: str) -> None:
        ...
        try:
            ...
            if opPath == "/":
                ...
                new_rel = self.normalizePath(str(Path(ext_folder) / f"{safe_name}.tdn"))
                if new_rel != rel_path:
            ...
            result = self.my.ext.TDN.ExportNetwork(root_path=opPath, output_file=abs_path, cleanup_protected=protected)
    ...
    def Checkpoint(self, opPath: str) -> bool:
        ...
        try:
            ...
            if not rel_path:
                return False
            abs_path = str(self.buildAbsolutePath(rel_path))
            result = self.my.ext.TDN.ExportNetwork(root_path=opPath, output_file=abs_path, skip_cleanup=True)
            if not result.get("success"):
                self.Log(
                    f"Checkpoint export failed for {opPath}: {result.get('error')}",
                    "WARNING",
                )
                return False
            ...
            self.Externalizations[opPath, "timestamp"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
            self.Externalizations[opPath, "dirty"] = ""
            ...
            self._updatePositionInTable(oper, opPath)
            self._setAutosaveStatus("Saved " + datetime.utcnow().strftime("%H:%M:%S") + " UTC")
            ...
            run(
                f"op({self.my.path!r}).ext.Embody._reBaselineCheckpoint({opPath!r})",
                fromOP=self.my,
                delayFrames=2,
            )
            return True
    ...
    def _armAutosaveDrain(self) -> None:
        ...
        run(
            f"op({self.my.path!r}).ext.Embody._autosaveDrain({self._autosave_gen})",
            fromOP=self.my,
            delayFrames=self._AUTOSAVE_POLL_FRAMES,
        )
    ...
    def _autosaveDrain(self, gen: int) -> None:
        ...
        if not self._autosaveEnabled() or self._performMode:
            self._setAutosaveStatus("Disabled" if not self._autosaveEnabled() else "Bypassed (Perform Mode)")
    ...
    def ExportPortableTox(self, target: "OP" = None, save_path: Optional[str] = None) -> bool:
        ...
        if save_path is None:
            ...
            save_path = str(Path(project.folder).parents[0] / "release" / f"{target.name}-v{version}.tox")
            ...
            if op_ref.family == "DAT" and hasattr(op_ref.par, "file"):
                ...
                if file_val and (file_val.startswith("/") or (len(file_val) > 1 and file_val[1] == ":")):
                    ...
                    if not file_val.startswith("/sys/"):
                        self.Log(
                            f"Absolute path won't be portable: {op_ref.path} -> {file_val}",
                            "WARNING",
                        )
                else:
                    saved_state.append({
                ...
                if tox_val and (tox_val.startswith("/") or (len(tox_val) > 1 and tox_val[1] == ":")):
                    if not tox_val.startswith("/sys/"):
                        self.Log(
                            f"Absolute path won't be portable: {op_ref.path} -> {tox_val}",
                            "WARNING",
                        )
                else:
                    saved_state.append({
        ...
        for op_ref, tags_to_remove in saved_tags:
            try:
                ...
                self.Log(f"Failed to strip tags from {op_ref.path}: {e}", "WARNING")
        ...
        try:
            ...
            try:
                rel_path = Path(save_path).relative_to(Path(project.folder).parents[0])
            except ValueError:
                rel_path = save_path
        ...
        for entry in saved_state:
            try:
                ...
                self.Log(f"Failed to restore {entry['op'].path}: {e}", "WARNING")
        ...
        for op_ref, tags_to_restore in saved_tags:
            try:
                ...
                self.Log(f"Failed to restore tags on {op_ref.path}: {e}", "WARNING")
    ...
    @staticmethod
    def _computeTDNFingerprint(comp, tdn_paths: set = None, exclude_tag: str = None) -> tuple:
        ...
        for c in sorted(comp.children, key=lambda c: c.name):
            ...
            if is_embedded_comp:
                ...
                child_fp = EmbodyExt._computeTDNFingerprint(c, tdn_paths, None)
                parts.append((c.name, "children", child_fp))
        # All annotations (utility=True or False) -- uses annotation-specific attrs
        for ann in sorted(
            comp.findChildren(type=annotateCOMP, depth=1, includeUtility=True),
            key=lambda a: a.name,
        ):
            ann_color = tuple(
                round(v, 4)
    ...
    def _isTDNDirty(self, comp, tdn_paths: set = None, exclude_tag: str = None) -> bool:
    ...
    def _storeTDNFingerprint(self, comp, tdn_paths: set = None, exclude_tag: str = None) -> None:
        """Snapshot the TDN COMP's network structure after export."""
        if tdn_paths is None:
            tdn_paths = self._getTDNPaths()
        if exclude_tag is None:
            exclude_tag = self.my.par.Tdnexcludetag.eval()
        self._tdn_fingerprints[comp.path] = self._computeTDNFingerprint(comp, tdn_paths, exclude_tag)
    ...
    def _handleTDNAddition(self, oper: OP) -> None:
        ...
        result = self.my.ext.TDN.ExportNetwork(
            root_path=oper.path, output_file=str(abs_path), cleanup_protected=protected
        )
        ...
        if result.get("success"):
            ...
            touch_build = str(oper.par.Touchbuild.eval()) if hasattr(oper.par, "Touchbuild") else app.build
            self.param_tracker.updateParamStore(oper)
            self._addToTable(oper, str(rel_path), timestamp, False, build_num, touch_build, "tdn")
    ...
    def _setupCompForExternalization(self, oper, rel_file_path, save_file_path):
            ...
            for row in range(1, self.Externalizations.numRows):
                if self._cellVal(row, "path") == oper.path:
                    try:
                        ...
                        self.Log(
                            f"Failed to parse build number for {oper.path}: {e}",
                            "DEBUG",
                        )
                        pass
                    break
        ...
        try:
            ...
            self.Log(
                f"Failed to save COMP {oper.path}",
                "ERROR",
                f"Path: {save_path_str}, Error: {e}",
            )
    ...
    def _setupDatForExternalization(self, oper, rel_file_path, save_file_path):
        ...
        try:
            ...
            self.Log(
                f"Failed to save DAT {oper.path}",
                "ERROR",
                f"Path: {save_path_str}, Error: {e}",
            )
    ...
    def _updatePositionInTable(self, oper: "OP", op_path: str) -> None:
        ...
        c = oper.color
        self.Externalizations[op_path, "node_color"] = f"{c[0]:.4f},{c[1]:.4f},{c[2]:.4f}"
    ...
    def _reconstructAboutPage(self, comp: "COMP", comp_path: str) -> None:
        ...
        touch_build = (
            (touch_cell.val if hasattr(touch_cell, "val") else str(touch_cell)) if touch_cell else str(app.build)
        )
    ...
    def checkOpsForContinuity(self, externalizationsFolder: str) -> None:
        ...
        try:
            rows_to_check = []
            tdn_comp_paths = set()
            headers = [self._cellVal(0, c) for c in range(self.Externalizations.numCols)]
            ...
            for i in range(1, self.Externalizations.numRows):
                ...
                if row_path:
                    ...
                    if row_path == embody_root or row_path.startswith(embody_root + "/"):
                        continue
            ...
            if ancestor_result:
                ...
                success = self._handleAncestorRename(old_prefix, new_prefix, rows_to_check, externalizationsFolder)
                if success:
                    return
                self.Log(
                    "Ancestor rename batch failed, falling back to per-operator handling",
                    "WARNING",
                )
            ...
            for old_op_path, rel_file_path, row_type, strategy in rows_to_check:
                ...
                if is_tdn:
                    if not op(old_op_path):
                        ...
                        found = self._findMovedTDNOp(old_op_path, rel_file_path, processed_ops)
                        if not found:
                            # Check if .tdn file exists on disk
                            if rel_file_path:
                                abs_tdn = self.buildAbsolutePath(self.normalizePath(rel_file_path))
                                if abs_tdn.is_file():
                                    missing_with_files.append((old_op_path, rel_file_path, "tdn"))
                                    continue
                            self.Log(
                                f"Operator for TDN entry '{old_op_path}' no longer exists",
                                "WARNING",
                            )
                ...
                parent_tdn = next((p for p in tdn_comp_paths if old_op_path.startswith(p + "/")), None)
                ...
                if existing_op:
                    ...
                    if current_ext_path == rel_file_path:
                        ...
                        if not found_moved:
                            ...
                            if rel_file_path:
                                abs_file = self.buildAbsolutePath(self.normalizePath(rel_file_path))
                                if abs_file.is_file():
                                    missing_with_files.append((old_op_path, rel_file_path, "replaced"))
                                    continue
                    ...
                    if not found_renamed:
                        ...
                        if rel_file_path:
                            ...
                            self.Debug(
                                f"File check: rel='{rel_file_path}' norm='{normalized}' abs='{abs_file}' exists={abs_file.is_file()}"
                            )
                            if abs_file.is_file():
                                missing_with_files.append((old_op_path, rel_file_path, "missing"))
                                continue
                        else:
    ...
    def _checkExternalToxPar(self):
        ...
        comps_with_filefolder = self.root.findChildren(
            type=COMP,
            key=lambda x: x.par.externaltox.expr and "me.parent().fileFolder + '/' +" in x.par.externaltox.expr,
        )
    ...
    def _resolveToxdropExternals(self, external) -> None:
        ...
        if choice == 2 and par:
            ...
            self.Log("Dropped .tox expression handling set to Always Clean", "INFO")
        elif choice == 3 and par:
            self.my.par.Toxdropexpr = "ignore"
            self.Log("Dropped .tox expression handling set to Always Ignore", "INFO")
    ...
    def _findMovedOp(self, old_op_path, rel_file_path, externalizationsFolder, processed_ops):
        ...
        for potential_op in self.root.findChildren(type=COMP):
            potential_path = (
                self.normalizePath(potential_op.par.externaltox.eval()) if potential_op.par.externaltox else ""
            )
            if potential_path and potential_path == rel_file_path and potential_op.path != old_op_path:
                self.Log(
                    f"Found moved/renamed COMP: {old_op_path} -> {potential_op.path}",
                    "INFO",
                )
        ...
        for potential_op in self.root.findChildren(type=DAT):
            ...
            if potential_path and potential_path == rel_file_path and potential_op.path != old_op_path:
                self.Log(
                    f"Found moved/renamed DAT: {old_op_path} -> {potential_op.path}",
                    "INFO",
                )
                self.updateMovedOp(potential_op, old_op_path, rel_file_path, externalizationsFolder)
                processed_ops.add(potential_op.path)
    ...
    def _findMovedTDNOp(self, old_op_path: str, old_rel_file_path: str, processed_ops: set) -> bool:
    ...
    def _updateMovedTDNOp(self, new_op: OP, old_op_path: str, old_rel_file_path: str) -> None:
        ...
        try:
            ...
            old_abs = self.buildAbsolutePath(self.normalizePath(old_rel_file_path)).resolve()
            new_abs = self.buildAbsolutePath(self.normalizePath(new_rel_path)).resolve()
            ...
            if old_abs.is_file():
                try:
                    ...
                    self.Log(
                        f"Renamed TDN file: {old_rel_file_path} -> {new_rel_path}",
                        "SUCCESS",
                    )
                except Exception as e:
                ...
                result = self.my.ext.TDN.ExportNetwork(root_path=new_op.path, output_file=str(new_abs))
                if result.get("success"):
    ...
    def _updateTDNChildren(self, old_prefix: str, new_prefix: str) -> None:
        ...
        for child_path, child_rel_file in children:
            ...
            if new_child:
                self.updateMovedOp(new_child, child_path, child_rel_file, self.ExternalizationsFolder)
            else:
    ...
    def _detectAncestorRename(self, rows_to_check):
        ...
        for candidate in self.root.findChildren(type=COMP):
            ext_path = self.normalizePath(candidate.par.externaltox.eval()) if candidate.par.externaltox else ""
        ...
        if not new_op:
            for candidate in self.root.findChildren(type=DAT):
                if not hasattr(candidate.par, "file"):
                    continue
                file_path = self.normalizePath(candidate.par.file.eval()) if candidate.par.file else ""
        ...
        self.Log(
            f"Detected ancestor rename: {ancestor_path} -> {new_prefix} ({len(missing)} operators affected)",
            "INFO",
        )
        return (ancestor_path, new_prefix)

    def _handleAncestorRename(self, old_prefix, new_prefix, rows_to_check, externalizationsFolder):
        ...
        for old_path, rel_file, row_type, strategy in rows_to_check:
            if old_path.startswith(old_prefix + "/") or old_path == old_prefix:
                ...
                affected.append((old_path, new_path, rel_file, new_rel_file, row_type, strategy))
        ...
        # --- Phase B: Prompt user ---
        msg = (
            f"Detected rename: {old_prefix} -> {new_prefix}\n\n"
        ...
        choice = self._messageBox("Embody -- Ancestor Rename Detected", msg, ["Cancel", "Proceed"])
        if choice != 1:
            self.Log(
                f"Ancestor rename cancelled by user: {old_prefix} -> {new_prefix}",
                "INFO",
            )
            return False
        ...
        if new_dir.exists():
            ...
            self._messageBox(
                "Embody Error",
                f'Cannot rename: directory "{new_disk_segment}/" already exists.',
                ["OK"],
            )
            return False
        ...
        try:
            ...
            self.Log(
                f"Renamed directory: {old_disk_segment}/ -> {new_disk_segment}/",
                "SUCCESS",
            )
        except Exception as e:
            self.Log("Failed to rename directory", "ERROR", str(e))
            self._messageBox("Embody Error", f"Failed to rename directory:\n{e}", ["OK"])
            return False
        ...
        for _, new_path, old_rel, new_rel, row_type, strategy in affected:
            ...
            if not target_op:
                self.Log(f"Operator not found at new path: {new_path}", "WARNING")
                continue
            ...
            try:
                if target_op.family == "COMP":
                    current = self.normalizePath(target_op.par.externaltox.eval()) if target_op.par.externaltox else ""
                    if current == old_rel:
                        # Defer Embody's own COMP to avoid self-reinit
                        if new_path == embody_path or new_path.startswith(embody_path + "/"):
                            deferred_updates.append((new_path, "externaltox", new_rel))
                        else:
                    ...
                    current = self.normalizePath(target_op.par.file.eval()) if target_op.par.file else ""
                    if current == old_rel:
                        # Defer Embody's own DATs to avoid reinit mid-method
                        if new_path.startswith(embody_path + "/"):
                            deferred_updates.append((new_path, "file", new_rel))
                        else:
                            target_op.par.file.readOnly = False
                            target_op.par.file = new_rel
                            target_op.par.file.readOnly = True
            except Exception as e:
                self.Log(f"Failed to update file param for {new_path}", "WARNING", str(e))
        ...
        self.Log(
            f"Ancestor rename complete: {old_prefix} -> {new_prefix} ({len(affected)} operators updated)",
            "SUCCESS",
        )
        ...
        if deferred_updates:
            ...
            self.Log(
                f"Deferred {len(deferred_updates)} file param updates for Embody components",
                "DEBUG",
            )
    ...
    def _handleMissingOperator(self, old_op_path, old_rel_file_path, delete_file=True):
        ...
        for i in range(1, self.Externalizations.numRows):
            if (
                self._cellVal(i, "path") == old_op_path
                and self.normalizePath(self._cellVal(i, "rel_file_path")) == normalized
            ):
                self.RemoveListerRow(old_op_path, old_rel_file_path, delete_file=delete_file)
                break
    ...
    def _handleMissingOpsWithFiles(self, missing_ops: list) -> None:
        ...
        if self._suppressDialogs():
            self.Debug(f"File-cleanup continuity dialog suppressed (test/save active, {len(missing_ops)} missing ops)")
            return
        ...
        try:
            runner = getattr(op, "unit_tests", None)
            if runner:
                sandbox_comp = getattr(runner, "op", lambda x: None)("test_sandbox")
                sandbox_prefixes = []
                if sandbox_comp:
                    sandbox_prefixes.append(sandbox_comp.path + "/")
                sandbox_prefixes.append("/_test_")
                filtered = [
                    (p, f, r) for p, f, r in missing_ops if not any(p.startswith(px) for px in sandbox_prefixes)
                ]
                if len(filtered) < len(missing_ops):
                    self.Debug(f"Filtered {len(missing_ops) - len(filtered)} test sandbox ops from continuity check")
        ...
        if preference == "ask":
            ...
            s = "" if count == 1 else "s"
            msg = (
                f"{count} externalized {noun} removed from the network:\n\n"
                f"{op_list}\n\n"
                f"External file{s} still exist{'s' if count == 1 else ''} on disk.\n"
                f"Remove from tracking only, or also delete file{s}?"
            )

            title = "Removed Operator Detected" if count == 1 else "Removed Operators Detected"
            choice = ui.messageBox(
                title,
                msg,
                buttons=[
        ...
        for op_path, rel_file_path, reason in missing_ops:
            if reason == "tdn":
                self.Log(f"Operator for TDN entry '{op_path}' no longer exists", "WARNING")
                self._removeTDNStrategy(op_path, delete_file=delete_files)
            else:
                if reason == "replaced":
                    self.Log(f"Operator at '{op_path}' was replaced", "WARNING")
                self._handleMissingOperator(op_path, rel_file_path, delete_file=delete_files)
    ...
    def _resolveClonesByCloningAPI(self, ops: list) -> bool:
        ...
        self.Log(
            f"Auto-resolved clone master '{master.path}' for path shared by {len(ops)} operators",
            "SUCCESS",
        )
        return True
    ...
    def _resolveDATsInClonedCOMPs(self, ops: list) -> bool:
        ...
        self.Log(
            ...
            f"{', '.join(d.path for d in masters)})",
            "SUCCESS",
        )
        return True
    ...
    def _duplicateButtonLabels(self, ops: list) -> list:
        ...
        diff_idx = next(
            (idx for idx in range(min_len) if len({s[idx] for s in seg_lists}) > 1),
            None,
        )
        labels = []
    ...
    def _promptForDuplicateGroup(self, path: str, ops: list) -> None:
        ...
        """
        op_list = "\n".join(f"  {i + 1}. {o.path} ({o.family})" for i, o in enumerate(ops))
        ...
        if 0 <= master_idx < len(ops):
            for i, o in enumerate(ops):
                if i != master_idx:
                    self._handleDuplicateAsReference(o)
            self.Log(
                f"User selected '{ops[master_idx].path}' as master for '{path}'",
                "SUCCESS",
            )

    def _promptForLargeDuplicateGroup(self, path: str, ops: list, op_list: str) -> None:
        ...
        marker = self.my.par.Templatemaster.eval().strip()
        if marker:
            tip = f"Tip: name one operator's COMP '{marker}' to auto-resolve groups like this without prompting."
        else:
            tip = "Tip: set the 'Template Master Name' parameter to auto-resolve groups like this by naming convention."
    ...
    def _promptForBatchResolution(self, unresolved: list) -> str:
        ...
        choice = self._messageBox(
            ...
            buttons=["Dismiss", "Review individually", f"Auto-resolve all ({n})"],
        )
    ...
    def _autoResolveFirstAsMaster(self, path: str, ops: list) -> None:
        ...
        self.Log(
            f"Auto-resolved '{master.path}' as master for '{path}' ({len(clones)} clone{plural})",
            "SUCCESS",
        )
    ...
    def _handleDuplicateAsReference(self, oper):
        ...
        row_exists = any(
            self.Externalizations[row, "path"] == oper.path for row in range(1, self.Externalizations.numRows)
        )
    ...
    def TagGetter(self) -> None:
        ...
        if oper.type in self.supported_dat_types:
            ...
            if active_tag:
                run(
                    lambda: self.SetupTaggerDATManageMode(oper, active_tag),
                    delayFrames=1,
                )
                run(
                    f"op('{self.tagging_menu_window}').par.winopen.pulse()",
                    delayFrames=2,
                )
                return
    ...
    def SetupTaggerManageMode(self, oper: OP, strategy_state: str) -> None:
        ...
        if btn_embed_storage:
            ...
            if embed_visible:
                ...
                btn_embed_storage.par.label = (
                    "\u229e  Embed storage in tdn  \u2713" if effective else "\u229e  Embed storage in tdn"
                )
        ...
        if tag not in oper.tags:
            ...
            if oper.family == "COMP":
                if tag == self.my.par.Toxtag.val:
                    rel_file_path = self.getExternalPath(oper)
                    self.RemoveListerRow(oper.path, rel_file_path, delete_file=delete_file)
                    oper.par.externaltox = ""
                    oper.par.externaltox.readOnly = False
    ...
    def _shouldDeleteFile(self) -> bool:
        ...
        if preference == "keep":
            ...
            choice = ui.messageBox(
                ...
                buttons=["Keep File", "Delete File", "Always Keep", "Always Delete"],
            )
            if choice == 0:
                return False
            elif choice == 1:
    ...
    def _removeTDNStrategy(self, op_path: str, delete_file: bool = True) -> None:
        ...
        self.Log(
            f"_removeTDNStrategy: searching for '{op_path}' delete_file={delete_file} rows={table.numRows}",
            "INFO",
        )
        for i in range(1, table.numRows):
            if self._cellVal(i, "path") == op_path and self._cellVal(i, "strategy") == "tdn":
                rel_path = self._cellVal(i, "rel_file_path")
                self.Log(
                    f"_removeTDNStrategy: found row {i}, rel_path='{rel_path}' delete_file={delete_file}",
                    "INFO",
                )
                if delete_file and rel_path:
                    full_path = self.buildAbsolutePath(self.normalizePath(rel_path)).resolve()
                    self.Debug(
                        f"TDN delete: rel='{rel_path}' abs='{full_path}' exists={full_path.is_file()} suffix='{full_path.suffix}'"
                    )
                    ...
                    def _delete(fp=full_path, rp=rel_path, opp=op_path):
                        try:
                            ...
                            if fp.is_file() and fp.suffix.lower() == ".tdn":
                                fp.unlink()
                                self.Log(
                                    f"Removed TDN externalization for {opp} ({rp})",
                                    "SUCCESS",
                                )
                            else:
    ...
    def _getTagColor(self, oper, tag):
        ...
        if oper.family == "COMP":
            if tag == self.my.par.Toxtag.val:
                return (
                    self.my.par.Toxtagcolorr,
                    self.my.par.Toxtagcolorg,
                    self.my.par.Toxtagcolorb,
                )
            elif tag == self.my.par.Tdntag.val:
                return (
                    self.my.par.Tdntagcolorr,
                    self.my.par.Tdntagcolorg,
                    self.my.par.Tdntagcolorb,
                )
            ...
            if oper.family == "COMP":
                ...
                for existing in list(oper.tags):
                    if existing in dat_tags:
                        ...
                        oper.par.file.readOnly = False
                        self.Log(
                            f"Removed existing '{existing}' tag from '{oper.path}' (replaced by '{tag}')",
                            "INFO",
                        )
                        break
    ...
    def AutoExternalizeNewOp(self, oper: OP) -> Optional[str]:
        ...
        try:
            ...
            self.Log(
                f"AutoExternalizeNewOp failed for {getattr(oper, 'path', '?')}: {e}",
                "WARNING",
            )
            return None
    ...
    def _autoExternalizeTagFor(self, oper: OP) -> Optional[str]:
        ...
        while ancestor is not None and ancestor.path != "/":
            if tox_tag in ancestor.tags or tdn_tag in ancestor.tags or self._findExternalizedComp(ancestor.path):
                return None
    ...
    def _scheduleAutoExternalizeFlush(self) -> None:
        ...
        run(
            f"op('{self.my}').ext.Embody._autoExternalizeFlush()",
            delayFrames=8,
            fromOP=self.my,
        )
    ...
    def AutoExternalizeCopiedOp(self, oper: OP) -> Optional[str]:
        ...
        try:
            ...
            self.Log(
                f"AutoExternalizeCopiedOp failed for {getattr(oper, 'path', '?')}: {e}",
                "WARNING",
            )
            return None
    ...
    def HandleEmbed(self, oper: OP) -> None:
        ...
        if rel_tdn_path:
            ...
            protected = self._getAllTrackedTDNFiles(exclude_path=oper.path)
            self.my.ext.TDN.ExportNetwork(root_path=oper.path, output_file=abs_path, cleanup_protected=protected)
    ...
    def HandlePortableExport(self, oper: OP) -> None:
        ...
        path = ui.chooseFile(load=False, start=start_dir, fileTypes=["tox"], title="Export portable tox")
        if path is None:
            return
    ...
    def _dispatchTaggerButton(self, oper: OP, tag: str, label: str) -> None:
    ...
    def ExternalizeImmediate(self, oper: OP) -> None:
        ...
        is_dat = not is_tox and not is_tdn and oper.family == "DAT" and any(t in oper.tags for t in self.getTags("DAT"))
        ...
        table = self.Externalizations
        for i in range(1, table.numRows):
            if self._cellVal(i, "path") == oper.path and self._cellVal(i, "strategy") == strategy:
    ...
    def ExternalizeProject(self) -> None:
        ...
        if self._performMode:
            return
        choice = ui.messageBox(
            "Embody -- Externalize Full Project",
            ...
            buttons=["Cancel", "TOX", "TDN", "TOX + Project TDN", "TDN + Project TDN"],
        )
        ...
        if export_project_tdn:
            self.my.ext.TDN.ExportNetworkAsync(output_file="auto", embed_all=True)

    def _shouldSkipOp(self, oper, paths_to_exclude):
        """Check if operator should be skipped in project externalization."""
        return (
            oper.path in paths_to_exclude
            or self.isReplicant(oper)
            or self.isInsideClone(oper)
            or self.my.ext.TDN._hasExcludeTag(oper)
            or oper.path.startswith("/local/")
            or oper.path == "/local"
        )
    ...
    def ReconstructTDNComps(self) -> None:
        ...
        if mode == "export":
            ...
            self.Log(
                "TDN mode=export -- additive recovery only, existing COMPs kept (no full reconstruction)",
                "INFO",
            )
        ...
        for comp_path, rel_tdn_path in tdn_comps:
            ...
            if comp is None:
                ...
                comp = self._createMissingCompShell(comp_path, "tdn", comp_type_override=tdn_type)
                if comp is None:
                    errors_total += 1
                    continue
            ...
            if result.get("error"):
                ...
                try:
                    backup_path = self.my.ext.TDN._get_backup_path_instance(str(abs_path))
                    if backup_path.is_file():
                        backup_tdn = self.my.ext.TDN.tdn_load(backup_path.read_text(encoding="utf-8"))
                        ...
                        if rb_result.get("success"):
                            self.Log(f"Rolled back {comp_path} from backup", "WARNING")
                            continue
                        else:
                            self.Log(
                                f"Rollback failed for {comp_path}: {rb_result.get('error')}",
                                "ERROR",
                            )
                except Exception as rb_e:
                    self.Log(f"Rollback error for {comp_path}: {rb_e}", "ERROR")
                errors_total += 1
                continue
    ...
    def _recoverMissingTDNComps(self) -> None:
        ...
        for comp_path, abs_path in missing:
            try:
                ...
                shell = op(comp_path)
                if shell is None:
                    shell = self._createMissingCompShell(comp_path, "tdn", comp_type_override=tdn_doc.get("type"))
                if shell is None:
                    self.Log(
                        f"Auto-save recovery: cannot create shell for {comp_path} (parent missing?)",
                        "WARNING",
                    )
                    continue
                ...
                if res.get("success"):
                    ...
                    self.Log(
                        f"Auto-save recovery import failed for {comp_path}: {res.get('error')}",
                        "ERROR",
                    )
            except Exception as e:
        ...
        if recovered:
            self.Log(
                f"Auto-save recovery: rebuilt {recovered} unsaved COMP(s) from .tdn (crash-before-save)",
                "SUCCESS",
            )
    ...
    def RecoverOrphanShells(self, auto: bool = False) -> dict:
        ...
        for comp in tagged:
            p = comp.path
            if p == "/" or p == embody_path or p.startswith(embody_path + "/") or embody_path.startswith(p + "/"):
                continue
            if p in tracked:
                continue
        ...
        if not auto:
            ...
            if choice != 0:
                if choice == -1:
                    self.Log(
                        "Orphan-shell recovery deferred (dialogs suppressed); will re-offer next open",
                        "INFO",
                    )
                return results
        ...
        for comp_path, abs_path in candidates:
            try:
                ...
                if res.get("success"):
                    ...
                    self.Log(
                        f"Orphan-shell restore failed for {comp_path}: {res.get('error')}",
                        "ERROR",
                    )
            except Exception as e:
                results["failed"].append(comp_path)
                self.Log(f"Orphan-shell restore error for {comp_path}: {e}", "ERROR")
        if results["restored"]:
            self.Log(
                f"Orphan-shell recovery restored {len(results['restored'])} COMP(s) and re-tracked them",
                "SUCCESS",
            )
        return results
        ...
        if mode == "off":
            ...
            if existing:
                ...
                if choice != 1:
                    ...
                    was_active = parexec.par.active.eval() if parexec else None
                    if parexec:
    ...
    def _findAtRiskDATs(self) -> list:
        ...
        for comp_path, _ in tdn_comps:
            ...
            embed_on = per_comp if per_comp is not None else self.my.par.Embeddatsintdns.eval()
            if embed_on:
    ...
    _STORAGE_SKIP_KEYS = {
        ...
        "_tdn_mode_migration_shown",
        "_tdn_migration_scheduled",
        "_tdn_migration_prev_enable",
        "_tdn_rel_path",
        "_pending_tox_restore",
        "pressed",
    }
    ...
    def _findAtRiskStorage(self) -> list:
        ...
        for comp_path, _ in tdn_comps:
            ...
            embed_on = per_comp if per_comp is not None else self.my.par.Embedstorageintdns.eval()
            if embed_on:
            ...
            for target in candidates:
                ...
                risky_keys = [
                    ...
                    if k not in self._STORAGE_CONTROL_KEYS and k not in self._STORAGE_SKIP_KEYS
                ]
                if risky_keys:
    ...
    def _promptTDNContentSafety(self, at_risk_dats: list, at_risk_storage: list) -> str:
        ...
        storage_entries = [(op_path, keys) for _, entries in at_risk_storage for op_path, keys in entries]
        ...
        if dat_count:
            ...
            sections.append(f"{dat_count} {noun} will lose content (Embed DATs OFF):\n" + "\n".join(lines))
        ...
        if storage_count:
            ...
            sections.append(
                f"{storage_count} storage {key_noun} will be lost (Embed Storage OFF):\n" + "\n".join(lines)
            )

        body = "\n\n".join(sections)
        externalize_verb = "Externalize DATs" if dat_count else "Continue"
        msg = (
            f"TDN content will be dropped on next save.\n\n"
            f"{body}\n\n"
        ...
        buttons = [externalize_verb, "Always Externalize", "Skip Once", "Always Skip"]
        choice = self._messageBox("TDN Content at Risk", msg, buttons=buttons)

        if choice == 0:
            return "externalize"
        elif choice == 1:
            self.my.par.Tdndatsafety = "externalize"
            self.Log("TDN content safety preference set to Always Externalize", "INFO")
            return "externalize"
        elif choice == 3:
            self.my.par.Tdndatsafety = "ignore"
            self.Log(
                "TDN content safety preference set to Always Skip "
                "-- save-time warnings disabled (re-enable via the "
                "TDN content-safety parameter on Embody)",
                "INFO",
            )
            return "skip"
        return "skip"
    ...
    def _logSkippedDATs(self, dats: list) -> None:
        ...
        self.Log(f"Skipped externalization of {len(dats)} at-risk DAT(s): {names}", "SUCCESS")
    ...
    def _logSkippedStorage(self, at_risk_storage: list) -> None:
        ...
        self.Log(
            f"Dropping {total} TDN storage entr{'y' if total == 1 else 'ies'} on save (Embed Storage OFF): {shown}",
            "SUCCESS",
        )
    ...
    def StripCompChildren(self, comp: OP) -> int:
        ...
        try:
            ...
            if externals:
                ...
                self.Log(
                    f"Captured {len(externals)} external connection(s) on {comp.path} before strip",
                    "DEBUG",
                )
        except Exception as e:
            self.Log(f"External capture failed on {comp.path}: {e}", "WARNING")
        ...
        excluded_paths = {c.path for c in all_ops if self.my.ext.TDN._hasExcludeTag(c)}
        destroy_ops = [c for c in all_ops if c.path not in excluded_paths]
        if excluded_paths:
            self.Log(
                f"Preserving {len(excluded_paths)} excluded COMP(s) during strip of {comp.path}",
                "DEBUG",
            )
        ...
        for child in all_ops:
            try:
                if child.dock is not None and child.dock.path not in excluded_paths:
                    child.dock = None
            except Exception:
                pass
        ...
        if count:
            self.Log(
                f"Stripped {count} operators from {comp.path} ({count - n_utility} children, {n_utility} annotations)",
                "INFO",
            )
        return count
    ...
    def _logReconstructionReport(self, tdn_comps, errors_total) -> None:
        ...
        if errors_total:
            self.Log(
                f"TDN reconstruction complete: {count} COMP(s), {errors_total} error(s) detected",
                "WARNING",
            )
        else:
            self.Log(
    ...
    def _createMissingCompShell(self, comp_path: str, strategy: str, comp_type_override: str = None) -> "OP | None":
        ...
        if not parent_op or not hasattr(parent_op, "create"):
            self.Log(
                f"Cannot create {comp_path}: parent {parent_path} not found or not a COMP",
                "WARNING",
            )
            return None
    ...
    def RestoreTOXComps(self) -> None:
        ...
        for comp_path, rel_tox_path, comp_type in tox_comps:
            ...
            if not abs_path.is_file():
                self.Log(
                    f"TOX file not found for missing COMP {comp_path}: {rel_tox_path}",
                    "WARNING",
                )
                continue
        ...
        for comp_path, rel_tox_path, comp_type in to_restore:
            ...
            if op(comp_path):
                restored += 1
                self.Log(
                    f"COMP {comp_path} already present (loaded from parent .tox)",
                    "INFO",
                )
                continue
            ...
            if not parent_op:
                self.Log(
                    f"Parent {parent_path} not found, cannot restore {comp_path}",
                    "WARNING",
                )
                errors += 1
                continue
            ...
            if not hasattr(parent_op, "create"):
                self.Log(
                    f"Parent {parent_path} is not a COMP, cannot restore {comp_path}",
                    "WARNING",
                )
                errors += 1
                continue
            ...
            try:
                ...
                self.Log(f"Failed to create {comp_path} (type {td_type}): {e}", "ERROR")
                errors += 1
                continue
            ...
            try:
                ...
                if "Cannot load external tox from path" in new_comp.scriptErrors():
                    new_comp.allowCooking = False
                    run(
                        lambda p=new_comp.path: self._safeAllowCooking(p, True),
                        delayFrames=1,
                    )
                ...
                self.Log(f"Failed to configure externaltox for {comp_path}: {e}", "ERROR")
                errors += 1
    ...
    def _logTOXRestorationReport(self, total, restored, errors) -> None:
        ...
        if errors:
            self.Log(
                f"TOX restoration complete: {restored}/{total} COMP(s) restored, {errors} error(s)",
                "WARNING",
            )
        else:
            self.Log(
                f"TOX restoration complete: {restored} COMP(s) restored successfully",
                "SUCCESS",
            )
    ...
    def RestoreDATs(self) -> None:
        ...
        for dat_path, rel_file_path, dat_type, strategy in dat_entries:
            ...
            if not abs_path.is_file():
                self.Log(
                    f"File not found for missing DAT {dat_path}: {rel_file_path}",
                    "WARNING",
                )
                continue
        ...
        for dat_path, rel_file_path, dat_type, strategy in to_restore:
            ...
            if op(dat_path):
                restored += 1
                self.Log(f"DAT {dat_path} already present (loaded from parent)", "INFO")
                continue
            ...
            if not parent_op:
                self.Log(
                    f"Parent {parent_path} not found, cannot restore {dat_path}",
                    "WARNING",
                )
                errors += 1
                continue
            ...
            if not hasattr(parent_op, "create"):
                self.Log(
                    f"Parent {parent_path} is not a COMP, cannot restore {dat_path}",
                    "WARNING",
                )
                errors += 1
                continue
            ...
            if dat_type not in valid_dat_types:
                self.Log(f'Unknown DAT type "{dat_type}" for {dat_path}', "WARNING")
                errors += 1
                continue
            ...
            try:
                ...
                self.Log(f"Failed to create {dat_path} (type {td_type}): {e}", "ERROR")
                errors += 1
                continue
            ...
            try:
                ...
                op_path = str(new_dat)
                run(lambda p=op_path: self._safeSyncFile(p, False), delayFrames=1)
                run(lambda p=op_path: self._safeSyncFile(p, True), delayFrames=2)
                ...
                restored += 1
                self.Log(f"Restored {dat_path} from {rel_file_path}", "SUCCESS")
    ...
    def _getDATEntries(self) -> list[tuple[str, str, str, str]]:
        ...
        for i in range(1, table.numRows):
            ...
            if dat_path == embody_path or dat_path.startswith(embody_path + "/"):
                continue

            # Skip DATs inside TOX/TDN COMPs
            inside_comp = any(dat_path.startswith(cp + "/") for cp in comp_paths)
            if inside_comp:
                continue
    ...
    def _logDATRestorationReport(self, total, restored, errors) -> None:
        ...
        if errors:
            self.Log(
                f"DAT restoration complete: {restored}/{total} DAT(s) restored, {errors} error(s)",
                "WARNING",
            )
        else:
            self.Log(
                f"DAT restoration complete: {restored} DAT(s) restored successfully",
                "SUCCESS",
            )
    ...
    def _launchEditor(
        ...
        return mod.embody_launch.launch_editor(
            ...
            app_name,
            bundle_id=bundle_id,
            win_exe_candidates=win_exe_candidates,
            win_shim=win_shim,
            mac_cli=mac_cli,
            mac_alt_names=mac_alt_names,
            install=install,
        )
    ...
    def compareParameters(self, comp):
        ...
        for name in stored:
            if name not in current:
                return True
            if (
                stored[name]["value"] != current[name]["value"]
                or stored[name]["expr"] != current[name]["expr"]
                or stored[name].get("bindExpr") != current[name].get("bindExpr")
                or stored[name]["mode"] != current[name]["mode"]
            ):
                return True
