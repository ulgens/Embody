...
def tdn_dump(data) -> str:
    ...
    """
    out = yaml.dump(data, Dumper=_TDNYamlDumper, sort_keys=False, width=4096, allow_unicode=True)
...
def canonical_tdn_bytes(tdn: dict) -> bytes:
    ...
    return json.dumps(tdn, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
class TDNExt:
    ...
    def __init__(self, ownerComp: "COMP") -> None:
        ...
        try:
            ...
            run(
                "o = op(%r)\nif o and o.valid: o.ext.TDN._clipboardWatchTick(%d)" % (self.ownerComp.path, _clip_gen),
    ...
    @staticmethod
    def _get_backup_path(tdn_path: str, project_folder: str, suffix: str = ".bak") -> Path:
    ...
    def _atomic_write(filepath: str, content: str) -> None:
        ...
        tmp_path = None
        try:
            tmp_fd, tmp_path = tempfile.mkstemp(dir=str(target.parent), suffix=".tdn.tmp")
    ...
    @staticmethod
    def _safe_write_tdn(tdn_path: str, content: str, project_folder: str) -> dict:
        ...
        if bak.is_file():
            try:
                ...
                return {"error": f"Validation failed ({error_msg}), restored from backup"}
            except Exception as restore_e:
                return {"error": f"Validation failed ({error_msg}) and backup restore failed: {restore_e}"}
        return {"error": f"Validation failed ({error_msg}), no backup available"}

    def _get_backup_path_instance(self, tdn_path: str, suffix: str = ".bak") -> Path:
    ...
    def _diff_index_by_name(items, warnings, parent_path, side):
        ...
        for item in items or []:
            ...
            if name in idx:
                warnings.append(
                    "Duplicate sibling name %r under %r (%s); diff may be ambiguous" % (name, parent_path or "/", side)
                )
            idx[name] = item
        return idx
    ...
    @staticmethod
    def _diff_annotations(live_anns, disk_anns, parent_path, added, removed, modified, warnings):
        live_idx = TDNExt._diff_index_by_name(live_anns, warnings, parent_path, "live-annotation")
        disk_idx = TDNExt._diff_index_by_name(disk_anns, warnings, parent_path, "disk-annotation")
        ...
        for name in live_idx:
            ...
            changes = TDNExt._diff_own_fields(live_idx[name], disk_idx[name], keys=keys)
            if changes:
                modified.append({
                    "path": parent_path,
                    "name": name,
                    "type": live_idx[name].get("mode"),
                    "kind": "annotation",
    ...
    @staticmethod
    def _diff_level(live_ops, disk_ops, parent_path, added, removed, modified, warnings):
        live_idx = TDNExt._diff_index_by_name(live_ops, warnings, parent_path, "live")
        disk_idx = TDNExt._diff_index_by_name(disk_ops, warnings, parent_path, "disk")

        def _entry(path, op_def, kind):
            return {
                "path": path,
                "name": op_def.get("name"),
                "type": op_def.get("type"),
                "kind": kind,
            }

        for name in live_idx:
            if name not in disk_idx:
                added.append(_entry(TDNExt._diff_join(parent_path, name), live_idx[name], "op"))
        for name in disk_idx:
            if name not in live_idx:
                removed.append(_entry(TDNExt._diff_join(parent_path, name), disk_idx[name], "op"))
        for name in live_idx:
            if name not in disk_idx:
                continue
            lo = live_idx[name]
            ...
            TDNExt._diff_annotations(
                lo.get("annotations", []) or [],
    ...
    def _diff_header_warnings(live_raw, disk_raw):
        ...
        if lb and db and lb != db:
            warnings.append("TD build differs (disk %s vs live %s); round-trip may shift parameter defaults" % (db, lb))
        lv = live_raw.get("version")
        dv = disk_raw.get("version")
        if lv and dv and lv != dv:
            warnings.append("TDN format version differs (disk %s vs live %s)" % (dv, lv))
        return warnings
    ...
    def _diff_normalized(
        ...
        root_changes = TDNExt._diff_own_fields(live, disk, keys=TDNExt._DIFF_ROOT_CONTENT_KEYS)
        if root_changes:
            modified.append({
                "path": comp_path,
                "name": (comp_path.rstrip("/").rsplit("/", 1)[-1] if comp_path else live.get("network_path", "")),
    ...
    def DiffLiveVsDisk(self, comp_path="/", max_changed_ops=200, max_bytes=60000):
        ...
        if not rel:
            return {"error": "%s is not TDN-externalized (no .tdn file is tracked for it)" % comp_path}
        try:
            abs_path = str(self.ownerComp.ext.Embody.buildAbsolutePath(self.ownerComp.ext.Embody.normalizePath(rel)))
        except Exception:
            abs_path = rel
        ...
        if not os.path.isfile(abs_path):
            return {
                "error": "On-disk .tdn not found: %s" % abs_path,
                "comp_path": comp_path,
                "file": abs_path,
                "file_exists": False,
            }
        ...
        if disk_tdn is None:
            return {
                "error": "On-disk .tdn is missing or corrupt: %s" % abs_path,
                "comp_path": comp_path,
                "file": abs_path,
                "file_exists": True,
            }
    ...
    def DiffAllLiveVsDisk(self, max_comps=200, max_changed_ops=50, max_bytes=60000):
        ...
        try:
            ...
            for row in range(1, table.numRows):
                strat = (table[row, "strategy"].val if has_strategy else table[row, "type"].val) or "tox"
                if strat == "tdn":
        ...
        for comp_path in tdn_paths:
            ...
            if examined >= max_comps:
                ...
                break
            examined += 1
            env = self.DiffLiveVsDisk(comp_path, max_changed_ops=max_changed_ops, max_bytes=max_bytes)
    ...
    def ExportNetwork(
        self,
        root_path: str = "/",
        include_dat_content: Optional[bool] = None,
        ...
        try:
            ...
            if options.get("include_storage", True):
                ...
                root_storage = self._exportStorage(root_op)
                control_keys = {
                    k: v for k, v in root_storage.items() if k in ("embed_dats_in_tdn", "embed_storage_in_tdn")
                }
                if control_keys:
            ...
            if output_file:
                ...
                before_tdn = set()
                if not skip_cleanup:
                    before_tdn = TDNExt._collectExistingTDNFiles(scan_folder, root_path)
                ...
                write_result = TDNExt._safe_write_tdn(filepath, content, scan_folder)
                if not write_result.get("success"):
                    return {"error": f"Safe write failed: {write_result.get('error')}"}

                if not skip_cleanup:
                    protected = [filepath]
                    if cleanup_protected:
                        protected.extend(cleanup_protected)
                    stale = TDNExt._cleanupStaleTDNFiles(before_tdn, protected, scan_folder)
                    if stale:
                        self._log(f"Cleaned up {len(stale)} stale .tdn file(s)", "INFO")

                result["file"] = filepath
                self._trackTDNExport(
                    root_path,
                    filepath,
                    build_num=build_num,
                    touch_build=f"{app.version}.{app.build}",
                )
                self._log(f"Exported network to {filepath}", "SUCCESS")
    ...
    def ExportNetworkAsync(
        ...
        if self._export_state is not None and not self._export_state.get("done"):
        ...
        if resolved_path:
            ...
            before_tdn = TDNExt._collectExistingTDNFiles(proj_folder, root_path)
            ...
            protected_files = list(self.ownerComp.ext.Embody._getAllTrackedTDNFiles(exclude_path=root_path))
        ...
        def worker():
            ...
            operators = TDNExt._assembleHierarchy(state["results"], state["root_path"])

            # Attach annotations to assembled hierarchy (pure Python dicts)
            ann_results = state.get("annotation_results", {})
            TDNExt._attachAnnotations(operators, state["root_path"], ann_results)
            ...
            tdn = {
                ...
                "options": {
                    "include_dat_content": state["options"]["include_dat_content"],
                    "include_storage": state["options"].get("include_storage", True),
                },
            }
            ...
            if state["output_file"]:
                ...
                content = TDNExt._compact_json_dumps(tdn)
                write_result = TDNExt._safe_write_tdn(state["output_file"], content, base_folder)
                if not write_result.get("success"):
                    state["result"] = {"error": f"Safe write failed: {write_result.get('error')}"}
                    return

                protected = [state["output_file"]] + state.get("protected_files", [])
                stale = TDNExt._cleanupStaleTDNFiles(before_tdn, protected, base_folder)
        ...
        self._log(f"Exporting {len(op_paths)} operators from {root_path}...", "INFO")
    ...
    def ExportProjectTDNInteractive(self):
        ...
        if choice not in (1, 2):
            return
        self.ExportNetworkAsync(output_file="auto", embed_all=(choice == 1))
        ...
        try:
            ...
            for i in range(idx, batch_end):
                try:
                    ...
                    if target_op:
                        op_data = self._exportSingleOp(target_op, state["options"], depth=0, recurse=False)
                        if op_data:
            ...
            if batch_end >= len(paths):
                ...
                if root_op:
                    ...
                    root_color = tuple(root_op.color)
                    if self._colorsDiffer(root_color, DEFAULT_COLOR):
                        root_meta["color"] = [round(c, 4) for c in root_color]
                    root_tags = list(root_op.tags)
                    if root_tags:
    ...
    def _onExportSuccess(self):
        ...
        if state and state.get("result"):
            ...
            if result.get("cleaned_up"):
                self._log(f"Cleaned up {result['cleaned_up']} stale .tdn file(s)", "INFO")
    ...
    def ReexportAllTDNs(self) -> None:
        ...
        try:
            ...
            self._log(f"Re-exporting {len(tdn_entries)} TDN file(s)...", "INFO")
    ...
    def _validateOpDefs(op_defs, path="operators"):
        ...
        for i, op_def in enumerate(op_defs):
            if not isinstance(op_def, dict):
                return f"{path}[{i}] must be a mapping, got {type(op_def).__name__}"
            children = op_def.get("children")
            if children is not None:
                err = TDNExt._validateOpDefs(children, f"{path}[{i}].children")
                if err:
                    return err
        return None
    ...
    def ImportNetwork(
        self,
        target_path: str,
        tdn: Union[dict[str, Any], list[dict[str, Any]]],
        clear_first: bool = False,
        restore_file_links: bool = False,
    ) -> dict[str, Any]:
        """
        ...
        if isinstance(tdn, dict) and "operators" in tdn:
            ...
            try:
                _file_newer = tuple(int(x) for x in str(tdn_version).split(".")) > tuple(
                    int(x) for x in TDN_VERSION.split(".")
                )
            except Exception:
                _file_newer = str(tdn_version) != TDN_VERSION
            if _file_newer:
                self._log(
                    f"TDN file is v{tdn_version}, newer than this build (v{TDN_VERSION}); some content may not import",
                    "WARNING",
                )
            ...
            if source_td and source_td != current_td:
                self._log(f"TDN exported from TD {source_td} (current: {current_td})", "INFO")
        ...
        if isinstance(tdn, dict):
            ...
            if par_templates and not isinstance(par_templates, dict):
                self._log(
                    f"Ignoring malformed par_templates ({type(par_templates).__name__})",
                    "WARNING",
                )
                par_templates = {}
            if type_defaults and not isinstance(type_defaults, dict):
                self._log(
                    f"Ignoring malformed type_defaults ({type(type_defaults).__name__})",
                    "WARNING",
                )
                type_defaults = {}
            if par_templates:
        ...
        excluded_names = {c.name for c in dest.children if self._hasExcludeTag(c)}
        if excluded_names:
            before = len(op_defs)
            op_defs = [d for d in op_defs if d.get("name") not in excluded_names]
        ...
        if tdn_paths:
            ...
            if tdn_paths:
                skipped = self._stripNestedTDNChildren(op_defs, target_path, tdn_paths)
                for sp in skipped:
                    self._log(
                        f"Skipping children of {sp} -- has its own TDN externalization (source of truth)",
                        "INFO",
                    )
        ...
        if tox_paths:
            skipped = self._stripNestedTOXChildren(op_defs, target_path, tox_paths)
            for sp in skipped:
                self._log(
                    f"Skipping children of {sp} -- has its own TOX externalization (source of truth)",
                    "INFO",
                )
        ...
        try:
            ...
            if clear_first:
                try:
                    ...
                    self._log(f"External capture failed on {target_path}: {e}", "DEBUG")
                if not captured_externals:
                    try:
                        stashed = dest.fetch("_tdn_external_wires", [], search=False)
                        if stashed:
            ...
            if clear_first:
                ...
                excluded_children = {c.path for c in dest.children if self._hasExcludeTag(c)}
                if excluded_children:
                    self._log(
                        f"Preserving {len(excluded_children)} excluded COMP(s) during clear of {dest.path}",
                        "DEBUG",
                    )
                ...
                for child in list(dest.children):
                    try:
                        if child.dock is not None and child.dock.path not in excluded_children:
                            child.dock = None
                    except Exception:
                        pass
                for child in list(dest.children):
                    ...
                    for u_op in dest.findChildren(depth=1, includeUtility=True):
                        if u_op.type == "annotate":
                            try:
                                ...
                                self._log(
                                    f"Failed to destroy annotation {u_op.path}: {e}",
                                    "WARNING",
                                )
                except Exception:
                    pass
            ...
            pre_existing = set() if clear_first else {c.name for c in dest.children}
            ...
            if isinstance(tdn, dict):
                ...
                if top_anns:
                    self._createAnnotationsFromList(dest, top_anns, ann_created)
            ...
            if isinstance(tdn, dict):
                ...
                if tdn_type and dest.OPType != tdn_type:
                    self._log(
                        f"Type mismatch: TDN expects {tdn_type} but destination is {dest.OPType}",
                        "WARNING",
                    )
                ...
                for seq_name, blocks in tdn_sequences.items():
                    try:
                        ...
                        self._log(
                            f"Failed to set sequence {seq_name} on {dest.path}: {e}",
                            "WARNING",
                        )
                    ...
                    if isinstance(tdn_flags, list):
                        for entry in tdn_flags:
                            try:
                                ...
                                self._log(
                                    f"Failed to set flag {entry} on {dest.path}: {e}",
                                    "DEBUG",
                                )
                        ...
                        for flag_name, value in tdn_flags.items():
                            try:
                                ...
                                self._log(
                                    f"Failed to set flag {flag_name} on {dest.path}: {e}",
                                    "DEBUG",
                                )
                ...
                if tdn_color:
                    try:
                        ...
                        self._log(f"Failed to set color on {dest.path}: {e}", "DEBUG")
                ...
                for key, value in tdn_storage.items():
                    try:
                        ...
                        self._log(
                            f'Failed to restore storage key "{key}" on {dest.path}: {e}',
                            "WARNING",
                        )
            ...
            if captured_externals:
                try:
                    ext_restored = self._restoreExternalConnections(dest, captured_externals)
                except Exception as e:
                    self._log(f"External restore failed on {target_path}: {e}", "WARNING")
            try:
                dest.unstore("_tdn_external_wires")
            except Exception:
                pass

            self._log(f"Imported {len(created)} operators into {target_path}", "SUCCESS")
            result = {
                "success": True,
    ...
    def ImportNetworkFromFile(
        self, file_path: str, target_path: str = "/", clear_first: bool = False
    ) -> dict[str, Any]:
        """
    ...
    def _exportChildren(self, parent_op, options, depth):
        ...
        for child in children:
            ...
            if sibling.OPType != child.OPType:
                continue
            if child.dock is not None and sibling.dock is not None and child.dock.path == sibling.dock.path:
                skip.add(name)
                self._log(
                    f'Skipping duplicate companion "{name}" (original: "{base}")',
                    "INFO",
                )
        ...
        for child in children:
            ...
            if child.path in SYSTEM_PATHS or child.path.startswith(_SYSTEM_PATH_PREFIXES):
                continue
            ...
            if op_data is not None:
                ...
                if not (set(op_data.keys()) - _TRIVIAL_KEYS):
                    self._log(
                        f'Skipping default child "{child.name}" (no customizations)',
                        "DEBUG",
                    )
                    continue
    ...
    def _exportSingleOp(self, target, options, depth, recurse=True):
        ...
        if options.get("include_storage", True):
            ...
            # Preserve Embody control keys even when storage is excluded
            storage = self._exportStorage(target)
            control_keys = {k: v for k, v in storage.items() if k in ("embed_dats_in_tdn", "embed_storage_in_tdn")}
            if control_keys:
        ...
        if target.family == "DAT" and (options.get("include_dat_content", True) or self._isInsideAnimationCOMP(target)):
        ...
        if hasattr(target, "children"):
            ...
            if is_palette and handling == "blackbox":
                ...
                if max_depth is None or depth < max_depth:
                    children = self._exportChildren(target, options, depth + 1)
                    if children:
    ...
    def _loadDivergentDefaults(self):
        ...
        try:
            ...
            if os.path.isfile(catalog_path):
                ...
                if catalog:
                    ...
                    self._log(
                        f"Loaded catalog from .embody/ for build {build_str} ({len(catalog)} types)",
                        "DEBUG",
                    )
                    return
        except Exception:
        ...
        if build_str in build_cols:
            ...
            self._log(
                f"Divergent defaults: no column for build {build_str}, using {col_name}",
                "DEBUG",
            )
        else:
            return
        ...
        self._log(
            f"Loaded divergent defaults from tableDAT: {len(self._divergent_defaults)} op types from column {col_name}",
            "DEBUG",
        )
    ...
    def _deserializeDivergentValue(val_str, style):
        ...
        if style in ("Float", "XY", "XYZ", "XYZW", "UV", "UVW", "WH", "RGB", "RGBA"):
            try:
    ...
    def _getCreationValueOnTheFly(self, op_type):
        ...
        try:
            if self._scan_workspace is None:
                self._scan_workspace = self.ownerComp.create(baseCOMP, "_defaults_workspace")
            ...
            self._log(f"On-the-fly default probe failed for {op_type}: {e}", "DEBUG")
    ...
    def _getCreationFlagDefaults(self, target):
        ...
        try:
            ...
            self._log(f"Creation flag-default probe failed for {op_type}: {e}", "DEBUG")
        ...
        for block in seq.blocks:
            ...
            for p in block_pars.get(block.index, []):
                ...
                if value is not None:
                    creation_default = self._getCreationDefault(target.OPType, p.name, p)
    ...
    def _exportStorage(self, target):
        ...
        for key, value in raw_storage.items():
            ...
            try:
                ...
                self._log(
                    f'Skipping non-serializable storage key "{key}" on {target.path}: {type(value).__name__} - {e}',
                    "DEBUG",
                )
        return result
    ...
    def _deserializeStorageValue(self, value):
        ...
        if isinstance(value, dict):
            if "$type" in value and "$value" in value and len(value) == 2:
                ...
                raw = value["$value"]
                if type_name == "tuple":
                    return tuple(self._deserializeStorageValue(v) for v in raw)
                elif type_name == "set":
                    return set(self._deserializeStorageValue(v) for v in raw)
                    ...
                    self._log(
                        f'Unknown $type "{type_name}" in storage, treating as plain dict',
                        "WARNING",
                    )
            return {k: self._deserializeStorageValue(v) for k, v in value.items()}
        return value
    ...
    def _exportAnnotations(self, parent_op):
        ...
        """
        try:
            annotations = parent_op.findChildren(type=annotateCOMP, depth=1, includeUtility=True)
        except Exception:
            return []
                ...
                for i, connector in enumerate(getattr(comp, local_attr, [])):
                    for c in connector.connections:
                        ...
                        conns.append({
                            ...
                            "local_index": i,
                            "remote": _rel(src),
                            "remote_index": _find_remote_index(src, comp, remote_out_attr),
                        })
            except Exception as e:
                self._log(
                    f"External capture ({local_attr}) error on {comp.path}: {e}",
                    "DEBUG",
                )
                ...
                for i, connector in enumerate(getattr(comp, local_attr, [])):
                    for c in connector.connections:
                        ...
                        conns.append({
                            ...
                            "local_index": i,
                            "remote": _rel(dst),
                            "remote_index": _find_remote_index(dst, comp, remote_in_attr),
                        })
            except Exception as e:
                self._log(
                    f"External capture ({local_attr}) error on {comp.path}: {e}",
    ...
    def _restoreExternalConnections(self, comp, conns) -> int:
        ...
        for entry in conns:
            try:
                ...
                if direction == "input":
                    local_attr = "inputConnectors" if kind == "op" else "inputCOMPConnectors"
                    remote_attr = "outputConnectors" if kind == "op" else "outputCOMPConnectors"
                    local_conns = getattr(comp, local_attr, [])
                    remote_conns = getattr(remote, remote_attr, [])
                    if local_idx >= len(local_conns) or remote_idx >= len(remote_conns):
                        self._log(
                            f"External restore: connector index out of "
                            f"range for {remote_ref}[{remote_idx}] -> "
                            f"{comp.name}[{local_idx}] ({kind})",
                            "WARNING",
                        )
                        continue
                    ...
                    local_attr = "outputConnectors" if kind == "op" else "outputCOMPConnectors"
                    remote_attr = "inputConnectors" if kind == "op" else "inputCOMPConnectors"
                    local_conns = getattr(comp, local_attr, [])
                    remote_conns = getattr(remote, remote_attr, [])
                    if local_idx >= len(local_conns) or remote_idx >= len(remote_conns):
                        self._log(
                ...
                self._log(f"External restore error on {comp.path}: {entry}: {e}", "WARNING")

        if restored:
            self._log(f"Restored {restored} external connection(s) on {comp.path}", "INFO")
        return restored
    ...
    def _createOps(self, parent, op_defs, created, pre_existing=None):
        ...
        for op_def in op_defs:
            ...
            if existing is not None and name not in pre_existing:
                ...
                self._log(f'Reusing existing operator "{name}"', "INFO")
                children = op_def.get("children", [])
            ...
            try:
                ...
                if new_op.name != name:
                    try:
                        ...
                        self._log(
                            f'Operator "{name}" auto-named to "{new_op.name}"',
                            "WARNING",
                        )
                created.append(new_op.path)
                op_def["_created_op"] = new_op
            except Exception as e:
                self._log(f'Failed to create {op_type} "{name}": {e}', "WARNING")
                continue
            ...
            if tdn_ref and new_op.isCOMP:
                ...
                self._log(
                    f"Skipping children of {new_op.path} -- managed by {tdn_ref}",
                    "DEBUG",
                )
                ...
                try:
                    ...
                    pass
                self._log(
                    f"Skipping children of {new_op.path} -- managed by {tox_ref}",
                    "DEBUG",
                )
    ...
    def _createCustomParsOnOp(self, target, custom_par_defs):
        ...
        for par_def in custom_par_defs:
            ...
            if belongs_to_seq:
                seq_template_counts[belongs_to_seq] = seq_template_counts.get(belongs_to_seq, 0) + 1
            ...
            if not method_name:
                self._log(f'Unknown par style "{style}" for {par_name}', "WARNING")
                continue
            ...
            if not append_method:
                self._log(f"Method {method_name} not found on Page", "WARNING")
                continue
            ...
            try:
                ...
                if par is None:
                    ...
                    suffixes = STYLE_SUFFIXES.get(style, [])
                    if suffixes:
                        par = getattr(target.par, par_name + suffixes[0], None)
                if par is None:
                    continue
                ...
                if par.isMenu:
                    if "menuSource" in par_def:
                        ...
                        par.menuLabels = par_def.get("menuLabels", par_def["menuNames"])
                ...
                self._log(f'Failed to create custom par "{par_name}": {e}', "WARNING")
        ...
        for seq_name, count in seq_template_counts.items():
            try:
                ...
                self._log(
                    f"Failed to set blockSize={count} on sequence {seq_name} of {target.path}: {e}",
                    "WARNING",
                )
            ...
            if sequences and target:
                for seq_name, blocks in sequences.items():
                    ...
                    try:
                        ...
                        self._log(
                            f"Failed to set numBlocks={len(blocks)} on sequence {seq_name} of {target.path}: {e}",
                            "WARNING",
                        )
                        continue
                    ...
                    for i, block_data in enumerate(blocks):
                        ...
                        block = seq[i]
                        for base_name, value in block_data.items():
                            par = self._resolveSequenceBlockPar(target, seq, block, i, base_name)
                            if par is None:
                                self._log(
                                    f'Sequence param "{base_name}" not found in {seq_name}[{i}] on {target.path}',
                                    "WARNING",
                                )
                                continue
    ...
    def _setParameters(self, parent, op_defs):
        ...
        for op_def in op_defs:
            ...
            flat_defs = TDNExt._flattenCustomPars(op_def.get("custom_pars", {}))
    ...
    def _setCustomParValues(self, target, flat_defs):
        ...
        for par_def in flat_defs:
            ...
            elif "values" not in par_def and "default" in par_def and style not in ("Pulse", "Momentary", "Header"):
                suffixes = STYLE_SUFFIXES.get(style, [])
            ...
            if "values" in par_def:
                ...
                if suffixes:
                    ...
                    for suffix, val in zip(actual_suffixes, values):
                        if val is not None:
                            self._setParValue(target, base_name + suffix, val)
                elif style in ("Float", "Int") and len(values) > 1:
                    # Numeric multi-component: suffix is 1, 2, 3...
                    for i, val in enumerate(values):
                        if val is not None:
                            self._setParValue(target, f"{par_name}{i + 1}", val)
    ...
    def _setParValue(self, target, par_name, value):
        ...
        try:
            ...
            self._log(f"Failed to set {par_name} on {target.path}: {e}", "WARNING")
            ...
            if isinstance(flags_data, list):
                ...
                for flag_name, value in flags_data.items():
                    try:
                        ...
                        self._log(
                            f"Failed to set flag {flag_name} on {target.path}: {e}",
                            "DEBUG",
                        )
    ...
    def _wireConnections(self, parent, op_defs):
        ...
        for op_def in op_defs:
            ...
            self._wireConnectionList(parent, target, op_def.get("inputs", []), comp=False)

            # COMP connections (top/bottom)
            self._wireConnectionList(parent, target, op_def.get("comp_inputs", []), comp=True)
    ...
    def _wireConnectionList(self, parent, target, conn_list, comp=False):
        ...
        for i, entry in enumerate(conn_list):
            ...
            if not source:
                self._log(
                    f"Connection source not found: {source_ref} -> {target.name}[{dest_index}]",
                    "WARNING",
                )
                continue
            ...
            try:
                if comp:
                    if hasattr(source, "outputCOMPConnectors"):
                        source.outputCOMPConnectors[0].connect(target.inputCOMPConnectors[dest_index])
                else:
                    ...
                    if src_conns and dest_index < len(tgt_conns):
                        ...
                        and dest_index < len(target.inputCOMPConnectors)
                    ):
                        ...
                        src_conns[0].connect(target.inputCOMPConnectors[dest_index])
                    else:
                        self._log(
                ...
                self._log(
                    f"Failed to connect {kind}{source_ref} -> {target.name}[{dest_index}]: {e}",
                    "WARNING",
                )
            ...
            if "dat_content" in op_def and target.family == "DAT":
                try:
                    ...
                    if "not editable" in err_str:
                        self._log(
                            f"Skipping read-only DAT {target.path} (auto-generated content)",
                            "DEBUG",
                        )
                    else:
                        self._log(
            ...
            if storage:
                for key, value in storage.items():
                    try:
                        ...
                        self._log(
                            f'Failed to restore storage key "{key}" on {target.path}: {e}',
                            "WARNING",
                        )
            ...
            if startup_storage:
                for key, value in startup_storage.items():
                    try:
                        ...
                        self._log(
                            f'Failed to restore startup storage key "{key}" on {target.path}: {e}',
                            "WARNING",
                        )
        ...
        for op_def in op_defs:
            ...
            if dock_target:
                try:
                    ...
                    self._log(f"Failed to dock {target.path} to {dock_ref}: {e}", "WARNING")
            else:
                self._log(f"Dock target not found: {dock_ref} for {target.path}", "WARNING")
    ...
    def _createAnnotationsFromList(self, parent, annotations_data, created):
        ...
        for ann_def in annotations_data:
            try:
                ...
                self._log(
                    f'Failed to create annotation "{ann_def.get("name", "?")}": {e}',
                    "WARNING",
                )
    ...
    def _restoreFileLinks(self, dest) -> int:
        ...
        for op_path, rel_path in file_map.items():
            ...
            try:
                ...
                self._log(f"Failed to restore file link on {op_path}: {e}", "WARNING")

        if restored:
            self._log(f"Restored file links on {restored} DAT(s) in {dest.path}", "INFO")
    ...
    def _restoreTOXShells(self, dest) -> int:
        ...
        def _walk(comp):
            ...
            for child in list(getattr(comp, "children", ()) or ()):
                try:
                    pending = child.fetch("_pending_tox_restore", None, search=False)
                except Exception:
                    pending = None
                if pending:
                    try:
                        ...
                        self._log(f"Failed to restore TOX shell {child.path}: {e}", "WARNING")
                else:
        ...
        _walk(dest)
        if restored:
            self._log(f"Restored {restored} TOX shell(s) under {dest.path} from .tox", "INFO")
        return restored
    ...
    def _collectAllPaths(self, parent_op, max_depth=None, depth=0, embed_all=False):
        ...
        for child in parent_op.children:
            ...
            if hasattr(child, "children"):
                if self._isPaletteClone(child) and (self._resolvePaletteHandling(child) == "blackbox"):
                    continue
                ...
                if not embed_all and self._hasTOXTag(child):
                    continue
                if max_depth is None or depth < max_depth:
                    paths.extend(self._collectAllPaths(child, max_depth, depth + 1, embed_all))
    ...
    def _resolvePaletteHandling(self, target):
        ...
        """
        try:
            stored = target.fetch(self._PALETTE_HANDLING_KEY, None, search=False)
        except Exception:
            stored = None
    ...
    def _promptPaletteHandling(self, target):
        ...
        try:
            ...
            self._log(
                f"Palette prompt failed on {target.path}: {e} (defaulting to blackbox)",
                "WARNING",
            )
            return "blackbox"
        ...
        try:
            ...
            if clone_par.mode == ParMode.EXPRESSION:
                ...
                if "defaultCOMPs" in expr:
                    return False
                if any(s in expr for s in ("TDBasicWidgets", "TDResources", "TDTox")):
                    return True
        except Exception as e:
            self._log(f"Error checking palette clone for {target.path}: {e}", "DEBUG")
        return False
    ...
    def _stripNestedTDNChildren(self, op_defs: list, parent_path: str, tdn_paths: set) -> list:
        ...
        for op_def in op_defs:
            ...
            if children and child_path in tdn_paths:
                ...
                skipped.extend(self._stripNestedTDNChildren(children, child_path, tdn_paths))
        return skipped

    def _stripNestedTOXChildren(self, op_defs: list, parent_path: str, tox_paths: set) -> list:
        ...
        for op_def in op_defs:
            ...
            if children and child_path in tox_paths:
                ...
                skipped.extend(self._stripNestedTOXChildren(children, child_path, tox_paths))
        return skipped
    ...
    def _resolveTOXRef(self, target) -> "Optional[str]":
        ...
        try:
            ...
            if not table or table.numRows < 2:
                return None
            for i in range(1, table.numRows):
                if table[i, "path"].val == target.path and table[i, "strategy"].val == "tox":
    ...
    def _resolveTDNRef(self, target) -> "Optional[str]":
        ...
        try:
            ...
            if not table or table.numRows < 2:
                return None
            for i in range(1, table.numRows):
                if table[i, "path"].val == target.path and table[i, "strategy"].val == "tdn":
                    return table[i, "rel_file_path"].val
        except Exception:
            pass
        return None
        ...
        for op_def in op_defs:
            ...
            if tdn_ref:
                ...
                if child_path not in tdn_paths:
                    warnings.append(
                        f"tdn_ref for {child_path} points to {tdn_ref} but no matching entry in externalizations table"
                    )

                # Check 2: referenced file exists on disk
                try:
                    abs_path = self.ownerComp.ext.Embody.buildAbsolutePath(tdn_ref)
                    if not abs_path.is_file():
                        warnings.append(f"tdn_ref for {child_path}: file not found: {tdn_ref}")
                except Exception:
                    warnings.append(f"tdn_ref for {child_path}: cannot resolve path: {tdn_ref}")

            # Recurse into children
            children = op_def.get("children", [])
            if children:
                warnings.extend(self._validateTDNRefs(children, child_path))
        ...
        for op_def in op_defs:
            ...
            if tox_ref:
                ...
                if child_path not in tox_paths:
                    warnings.append(
                        f"tox_ref for {child_path} points to {tox_ref} but no matching entry in externalizations table"
                    )

                # Check 2: referenced file exists on disk
                try:
                    abs_path = self.ownerComp.ext.Embody.buildAbsolutePath(tox_ref)
                    if not abs_path.is_file():
                        warnings.append(f"tox_ref for {child_path}: file not found: {tox_ref}")
                except Exception:
                    warnings.append(f"tox_ref for {child_path}: cannot resolve path: {tox_ref}")

            # Recurse into children
            children = op_def.get("children", [])
            if children:
                warnings.extend(self._validateTOXRefs(children, child_path))
    ...
    def _trackTDNExport(self, root_path, file_path, build_num=None, touch_build=None):
        ...
        try:
            ...
            rel_path = self.ownerComp.ext.Embody.normalizePath(str(Path(file_path).relative_to(project.folder)))
            ...
            if root_path != "/":
                try:
                    ...
                    tdn_tag = ""
                if not (target is not None and tdn_tag and tdn_tag in target.tags):
                    self._log(
            ...
            if has_strategy:
                ...
                table.appendRow([root_path, "tdn", rel_path, timestamp, "", build_str, tb_str])
            _stamp_recovery_pointer()
        except Exception as e:
    ...
    def _warnLargeTDN(self, filepath: str, root_path: str) -> None:
        ...
        choice = self.ownerComp.ext.Embody._messageBox("Large TDN File", msg, buttons=["OK", "Don't show again"])
    ...
    def _warnLockedNonDATs(self, root_op, context="export"):
        ...
        if context == "export":
            self._log(
                f"Locked non-DAT operators in {root_op.path}: {summary} -- frozen data will not persist through TDN",
                "WARNING",
            )
            try:
                ui.messageBox(
            ...
            self._log(
                ...
                f"frozen data and should be unlocked to re-cook",
                "WARNING",
            )
    ...
    def CopySelectedToClipboard(self) -> dict:
        ...
        if len(comps) > 1:
            self._log(
                "Copy TDN: %d COMPs selected; copying the first (%s)" % (len(comps), comps[0].name),
                "INFO",
            )
    ...
    def PasteNetworkFromClipboard(self, target: "OP") -> dict:
        ...
        self._log(
            "Pasted TDN into %s (mode=%s, source=%s, verdict=%s)"
            % (target.name, plan["mode"], plan.get("source"), verdict),
            "SUCCESS",
        )
        return {
            "ok": True,
    ...
    def PasteNetworkAsNewComp(self) -> dict:
        ...
        verdict = (plan.get("capability") or {}).get("verdict")
        self._log(
            "Pasted new COMP '%s' from clipboard TDN (mode=%s, source=%s, verdict=%s)"
            % (base.name, plan["mode"], plan.get("source"), verdict),
            "SUCCESS",
        )
        return {
            "ok": True,
    ...
    def _clipboardWatchTick(self, gen: int = 0) -> None:
        ...
        # Pending run() calls can outlive COMP replacement during upgrades.
        run(
            "o = op(%r)\nif o and o.valid: o.ext.TDN._clipboardWatchTick(%d)" % (self.ownerComp.path, gen),
            fromOP=self.ownerComp,
