...
def query_network(
    ...
    result = {"parent": parent_path, "count": len(operators), "operators": operators}
...
def get_connections(ext, op_path: str) -> dict:
    ...
    for i, inp in enumerate(target.inputs):
        inputs.append({"index": i, "connected_to": inp.path if inp else None})

    outputs = []
    for i, connector in enumerate(target.outputConnectors):
        connected = [conn.owner.path for conn in connector.connections]
        outputs.append({"index": i, "connected_to": connected})

    result = {"path": op_path, "inputs": inputs, "outputs": outputs}
    ...
    if hasattr(target, "inputCOMPConnectors"):
        ...
        for i, connector in enumerate(target.inputCOMPConnectors):
            connected = [conn.owner.path for conn in connector.connections]
            comp_inputs.append({"index": i, "connected_to": connected})
        result["comp_inputs"] = comp_inputs

        comp_outputs = []
        for i, connector in enumerate(target.outputCOMPConnectors):
            connected = [conn.owner.path for conn in connector.connections]
            comp_outputs.append({"index": i, "connected_to": connected})
...
def exec_op_method(ext, op_path: str, method: str, args: list = None, kwargs: dict = None) -> dict:
...
def sample_grid_top(ext, target, grid) -> dict:
    try:
        ...
        for out_idx, name in enumerate(("r", "g", "b", "a")):
            stats[name] = _channel_stat(channel_map[out_idx], pad_values[out_idx])
    ...
    try:
        ...
        for child in parent_op.children:
            ...
            try:
                ...
                if (
                    dock_host is not None
                    and dock_host.parent() is not None
    ...
    try:
        if target.type == "annotate":
            ...
            return {
                "path": op_path,
                "is_annotation": True,
                "enclosed_ops": [{"path": o.path, "name": o.name, "type": o.OPType} for o in enclosed],
                "count": len(enclosed),
            }
        else:
            ...
            return {
                ...
                "enclosing_annotations": [
                    {"path": a.path, "name": a.name, "mode": a.par.Mode.eval()} for a in enclosing
                ],
                "count": len(enclosing),
            }
    except Exception as e:
...
def find_children(
    ...
    try:
        ...
        return {"parent": op_path, "count": len(results), "operators": results}
    except Exception as e:
...
def get_externalizations(ext) -> dict:
    ...
    try:
        ...
        for row in range(1, table.numRows):
            ...
            strategy = (table[row, "strategy"].val if has_strategy else table[row, "type"].val) or "tox"
            try:
                abs_path = str(op.Embody.ext.Embody.buildAbsolutePath(op.Embody.ext.Embody.normalizePath(rel)))
            except Exception:
                abs_path = rel
        ...
        return {"count": len(externalizations), "externalizations": externalizations}
    except Exception as e:
...
def get_externalization_status(ext, op_path: str) -> dict:
    ...
    try:
        ...
        return {"path": op_path, "externalized": False}
    except Exception as e:
...
def export_network(
    ...
    protected = ext.ownerComp.ext.Embody._getAllTrackedTDNFiles(exclude_path=root_path) if output_file else None
    ...
    if output_file and isinstance(result, dict) and result.get("file") and isinstance(result.get("tdn"), dict):
        doc = result.pop("tdn")
        result["summary"] = {
        ...
        result["note"] = "Full .tdn written to file; Read the file for operators, params and DAT content."
    return result


def read_tdn(ext, comp_path="/", include_dat_content=None, max_depth=None, embed_all=False):
...
def resolve_diff_target(ext, target):
    ...
    try:
        ...
        table = None
    if not table:
        return None, ("Operator not found and externalizations table unavailable: %s" % target)
    ...
    for row in range(1, table.numRows):
        ...
        try:
            abs_p = str(op.Embody.ext.Embody.buildAbsolutePath(op.Embody.ext.Embody.normalizePath(rel))).replace(
                "\\", "/"
            )
        except Exception:
            abs_p = rel
    ...
    if not matches:
        return None, ("No externalized COMP found for %r (not a COMP path or a tracked .tdn file)" % target)
    if len(matches) > 1:
        comps = ", ".join(m[0] for m in matches)
        return None, (
            "Ambiguous: %r matches multiple externalized files (%s). Pass the COMP path instead." % (target, comps)
        )
    comp_path, strat = matches[0]
    if strat != "tdn":
        return None, (
            "%s is externalized as %s, not tdn -- diff_tdn only applies to TDN-strategy COMPs." % (comp_path, strat)
        )
...
def diff_tdn(ext, target="", max_changed_ops=200, max_bytes=60000):
    ...
    return ext.ownerComp.ext.TDN.DiffLiveVsDisk(
        comp_path=comp_path, max_changed_ops=max_changed_ops, max_bytes=max_bytes
    )
