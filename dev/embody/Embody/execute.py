...
def onStart():
    ...
    run(
        f"op('{parent.Embody}').ext.Embody._restoreSettings(kick_envoy=True)",
        delayFrames=5,
    )
...
def onCreate():
    ...
    run(
        f"op('{parent.Embody}').ext.Embody.CreateExternalizationsTable()",
        delayFrames=15,
    )
...
def onProjectPreSave():
    ...
    try:
        ...
        try:
            parent.Embody.ext.Embody.Log(f"Pre-save externalization failed (save will continue): {e}", "ERROR")
        except Exception:
...
def _runPreSaveExternalization():
    ...
    if mode == "off":
        parent.Embody.ext.Embody.Log("TDN mode=off -- skipping pre-save TDN strip/export", "INFO")
        return
    ...
    for comp_path, rel_tdn_path in tdn_comps:
        ...
        try:
            ...
            result = parent.Embody.ext.TDN.ExportNetwork(root_path=comp_path, output_file=None)
            if not result.get("success"):
                parent.Embody.ext.Embody.Log(
                    f"Pre-save export failed for {comp_path}: {result.get('error')}",
                    "ERROR",
                )
                continue
            ...
            if existing_tdn and parent.Embody.ext.TDN._tdn_content_equal(new_tdn, existing_tdn):
                exported.append((comp_path, rel_tdn_path))
                continue

            # Content changed (or first export) - write to disk
            scan_folder = str(project.folder)
            before_tdn = parent.Embody.ext.TDN._collectExistingTDNFiles(scan_folder, comp_path)
            # Only files Embody tracks are deletion candidates -- never
            # reclaim a stray the user placed themselves.
            before_tdn = parent.Embody.ext.TDN._restrictToTrackedTDN(before_tdn)
            content = parent.Embody.ext.TDN._compact_json_dumps(new_tdn)
            write_result = parent.Embody.ext.TDN._safe_write_tdn(abs_path, content, scan_folder)
            if not write_result.get("success"):
                parent.Embody.ext.Embody.Log(
                    f"Pre-save write failed for {comp_path}: {write_result.get('error')}",
                    "ERROR",
                )
                continue
            ...
            # Stale file cleanup
            protected = [abs_path]
            other_protected = parent.Embody.ext.Embody._getAllTrackedTDNFiles(exclude_path=comp_path)
            if other_protected:
                protected.extend(other_protected)
            parent.Embody.ext.TDN._cleanupStaleTDNFiles(before_tdn, protected, scan_folder)
            ...
            parent.Embody.ext.Embody.Log(f"Pre-save export error for {comp_path}: {e}", "ERROR")
    ...
    if mode != "full":
        parent.Embody.ext.Embody.Log("TDN mode=export -- skipping Phase 2 strip", "DEBUG")
        return
    ...
    exported_by_depth = sorted(exported, key=lambda x: x[0].count("/"), reverse=True)
...
def onProjectPostSave():
    ...
    if stripped:
        ...
        for entry in stripped:
            ...
            try:
                if not rel_path:
                    parent.Embody.ext.Embody.Log(
                        f"Post-save restore: no TDN file path for {comp_path}",
                        "WARNING",
                    )
                    continue
                abs_path = parent.Embody.ext.Embody.buildAbsolutePath(rel_path)
                if not abs_path.is_file():
                    parent.Embody.ext.Embody.Log(f"Post-save restore: .tdn file missing: {rel_path}", "WARNING")
                    continue
                tdn_doc = parent.Embody.ext.TDN.tdn_load(abs_path.read_text(encoding="utf-8"))
                ...
                try:
                    parent.Embody.ext.Embody.Log(f"Post-save restore failed for {comp_path}: {e}", "ERROR")
                except Exception:
                    pass
                # Attempt rollback from backup .tdn
                try:
                    backup_path = parent.Embody.ext.TDN._get_backup_path_instance(str(abs_path))
                    if backup_path.is_file():
                        backup_tdn = parent.Embody.ext.TDN.tdn_load(backup_path.read_text(encoding="utf-8"))
                        parent.Embody.ext.TDN.ImportNetwork(
                            target_path=comp_path,
                            tdn=backup_tdn,
                            clear_first=True,
    ...
    if stripped and parent.Embody.par.Envoyenable.eval():
        ...
        run(
            f"op('{parent.Embody}').ext.Envoy.Start()",
            fromOP=parent.Embody,
            delayFrames=30,
        )
    return
