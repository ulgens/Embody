...
def set_parameter(
    ...
    try:
        ...
        # Set constant value (with type coercion for numeric/toggle pars)
            ...
            # from TDMCP).
            if isinstance(value, str) and par.isMenu and par.style == "Menu" and value not in par.menuNames:
...
def disconnect_op(ext, op_path: str, input_index: int = 0, comp: bool = False) -> dict:
...
def edit_dat_content(
    ...
    if count == 0:
        ...
        hint = (" A case-insensitive search would have matched -- check the casing of old_string.") if ci_match else ""
        return {
            "error": (
    ...
    try:
        ...
        if (x is not None or y is not None) and target.parent():
            ...
            for sibling in target.parent().children:
                ...
                if tx < sx + sw + MARGIN and tx + tw + MARGIN > sx and ty < sy + sh + MARGIN and ty + th + MARGIN > sy:
...
def cook_op(ext, op_path: str, force: bool = True, recurse: bool = False) -> dict:
    """Cook an operator"""
    target = op(op_path)
    if not target:
        return {"error": f"Operator not found: {op_path}"}
...
def externalize_op(ext, op_path: str, tag_type: str = None) -> dict:
    ...
    try:
        ...
        return {"success": True, "path": op_path, "tag": tag_type, "file": file_path}
    except Exception as e:
...
def remove_externalization_tag(ext, op_path: str) -> dict:
    ...
    try:
        ...
        return {"success": True, "path": op_path, "removed_tags": removed}
    except Exception as e:
...
def create_extension(
    ...
    try:
        getattr(comp.par, f"ext{ext_index}object").val = f"op('./{class_name}').module.{class_name}(me)"
