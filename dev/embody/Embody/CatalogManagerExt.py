...
class CatalogManagerExt:
    ...
    def _processChunk(self):
        ...
        # Update status
        self.ownerComp.par.Status = f"Scanning defaults ({self._scan_count}/{self._scan_total})"
    ...
    def _finalizeScan(self):
        ...
        if self._scan_errors:
            ...
            self._log(
                f"Scan skipped {len(self._scan_errors)} non-instantiable types (first 5: {self._scan_errors[:5]})",
                "DEBUG",
            )

        self._log(f"Scan complete: {self._scan_count} types, {sum(len(v) for v in self._scan_results.values())} params")
        ...
        if bootstrap_palette:
            self._log(
                f"Palette bootstrap hit: {len(bootstrap_palette)} entries for build {self._build_str} (skipping scan)"
            )
    ...
    def _startPaletteScan(self, op_catalog):
        ...
        for root, _dirs, files in os.walk(palette_dir):
            for fname in files:
                ...
                if stem in _PALETTE_SCAN_BLOCKLIST or stem.startswith(_PALETTE_SCAN_BLOCKLIST_PREFIXES):
                    skipped.append(stem)
                    continue
        ...
        self._palette_workspace = self.ownerComp.create(baseCOMP, "_palette_workspace")
        ...
        self._log(
            ...
            "so it will not run again for this TD build.",
            "INFO",
        )
        self.ownerComp.par.Status = f"Scanning palette (0/{len(self._palette_queue)})"
    ...
    def _finalizePaletteScan(self):
        ...
        self._log(f"Palette scan complete: {len(self._palette_results)} components")
        self._log(
            "Palette catalog cached. Any palette errors printed above were expected and can be ignored.",
            "INFO",
        )
    ...
    def _patchCrossBuildDefaults(self, current_catalog):
        ...
        for comp_path, rel_tdn_path in tdn_comps:
            ...
            try:
                abs_path = str(self.ownerComp.ext.Embody.buildAbsolutePath(rel_tdn_path))
            ...
            if source_build not in source_catalogs:
                ...
                if os.path.isfile(source_path):
                    source_catalogs[source_build] = self._readCatalog(source_path)
                else:
            ...
            shifted = self._findShiftedDefaults(source_catalog, current_catalog)
            if not shifted:
                continue
        ...
        for child in comp.findChildren(depth=-1, includeUtility=False):
            ...
            for par_name, (old_val, new_val) in shifted[op_type].items():
                ...
                if self._valuesEqual(current_val, new_val):
                    ...
                    patches.append((child.path, par_name, str(new_val), str(old_val)))
    ...
    def _showPatchSummary(self, patches, current_build):
        ...
        msg = f"{count} parameter{'s' if count != 1 else ''} updated to preserve original values:\n\n" + "\n".join(
            lines
        )

        self.ownerComp.ext.Embody._messageBox("Embody \u2014 Cross-Build Parameter Update", msg, buttons=["OK"])
    ...
    def _populateTDNExt(self, catalog):
        ...
        param_catalog = {k: v for k, v in catalog.items() if not k.startswith("_")}
    ...
    def ExportPaletteCatalog(self):
        ...
        table = self.ownerComp.op("palette_catalog")
        if table is None:
            self._log("ExportPaletteCatalog: palette_catalog tableDAT not found", "ERROR")
            return
        ...
        self._log(
            f"ExportPaletteCatalog: wrote {len(palette)} rows for build {build_str}",
            "SUCCESS",
        )
    ...
    def _getPaletteDir():
        ...
        candidates = [
            # macOS app bundle
            os.path.join(app.installFolder, "Contents", "Resources", "tfs", "Samples", "Palette"),
    ...
    def _writeCatalog(self, path, catalog):
        ...
        try:
            ...
            content = json.dumps(catalog, separators=(",", ":"), sort_keys=True)
            with open(path, "w", encoding="utf-8") as f:
                f.write(content)
            self._log(f"Wrote catalog to {os.path.basename(path)} ({len(catalog)} types)")
        except Exception as e:
    ...
    def _timeStateAccessors(self):
        ...
        return [
            ...
            (
                "cookRate",
                lambda: project.cookRate,
                lambda v: setattr(project, "cookRate", v),
            ),
            (
                "realTime",
                lambda: project.realTime,
    ...
    def _log(self, msg, level="INFO"):
        ...
        try:
            self.ownerComp.ext.Embody.Log(f"[CatalogManager] {msg}", level)
        except Exception:
