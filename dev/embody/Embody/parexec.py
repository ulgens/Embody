...
def onValueChange(par, prev):
    ...
    if par.name == "Folder":
        ...
        if parent.Embody.par.Aiprojectroot.eval() == "custom":
            parent.Embody.ext.Embody._migrateRootFiles("custom", "custom", old_custom=prev, new_custom=par.eval())
            if parent.Embody.par.Envoyenable.eval():
                parent.Embody.InitEnvoy()
        ...
        if par.eval():
            ...
            run(
                "parent.Embody.ext.Envoy.Start() if parent.Embody.par.Envoyenable.eval() else None",
                delayFrames=30,
            )
        else:
