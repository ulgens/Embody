...
def extract_ai_config(ext):
    ...
    details = [f"{target_dir}/AGENTS.md"] + [f"{target_dir}/{f}" for f in ext._AI_CONFIG_FILES.get(client, [])]
    ext._guardFileWrite(
        "AI config",
        f"write AI assistant config for {ai_client_label(ext)} in {target_dir}",
        details,
        _write,
    )
...
def write_claude_md(ext, target_dir):
    ...
    if claude_md_path.exists():
        ...
        if "<!-- Generated by Embody/Envoy" in existing:
            ...
            ext.Log(
                f"CLAUDE.md already exists (not generated by Embody). Wrote MCP reference to {fallback} instead.",
                "WARNING",
            )
            return fallback
    else:
...
def manifest_record_appended_file(ext, target_dir, path, marker, kind="block"):
    ...
    try:
        ...
        if not any(e.get("path") == rel for e in m["files_appended"]):
            m["files_appended"].append({"path": rel, "marker": marker, "kind": kind})
            save_install_manifest(ext, target_dir, m)
    except Exception as e:
        ext.Log(f"Install-manifest append record failed for {path}: {e}", "DEBUG")
...
def write_template(ext, target_dir, rel_path, content):
    ...
    if target_path.exists():
        ...
        if stored is not None and content_hash(ext, existing) != stored:
            ext.Log(
                f"Kept your edits to {rel_path} (delete the file to regenerate it from the template).",
                "INFO",
            )
            return False
...
def client_files_missing(ext, target_dir, client):
    ...
    checks = {
        "claudecode": lambda d: (
            (not (d / "CLAUDE.md").exists() and not (d / "ENVOY.md").exists()) or not (d / ".claude" / "rules").exists()
        ),
...
def init_envoy(ext) -> None:
    ...
    def _apply():
        ...
        ext.Log(f"Envoy config regenerated for {client_label} at {target_dir}", "SUCCESS")
...
def init_git(ext) -> None:
    ...
    ext._guardFileWrite(
        "Git config",
        f"update git config in {git_root}",
...
def update_git_status(ext) -> None:
    ...
    if getattr(ext, "_git_check_running", False) and (now - getattr(ext, "_git_check_started", 0)) < 10:
        return
    ...
    clean_env = {
        ...
        if k not in ("GIT_DIR", "GIT_WORK_TREE", "GIT_INDEX_FILE", "GIT_CEILING_DIRECTORIES")
    }
    ...
    def done():
        ...
        if table is not None:
            for r in range(1, table.numRows):
                rows.append((ext._cellVal(r, "path"), ext._cellVal(r, "rel_file_path")))
        ext.my.store("git_status", map_changed_to_ops(state["changed"] or {}, prefix, rows))
