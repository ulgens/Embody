...
try:
    ...
    target_dir = step("compute_target_dir", lambda: project_dir)  # no git in his project, so target_dir = project_dir

    bridge_dir = step("compute_bridge_dir", lambda: target_dir / ".embody")

    step("mkdir_bridge_dir", lambda: bridge_dir.mkdir(parents=True, exist_ok=True))

    bridge_path = step("compute_bridge_path", lambda: bridge_dir / "envoy-bridge.py")
    ...
    if exists:
        existing = step("read_existing_bridge", lambda: bridge_path.read_text(encoding="utf-8"))
        needs_write = step("compare_bridge_content", lambda: existing != bridge_content)
    else:
        needs_write = True
    ...
    old_bridge = step("compute_old_bridge_path", lambda: target_dir / ".claude" / "envoy-bridge.py")
    step(
        "migrate_old_bridge",
    ...
    for _old_name in (".envoy-tools-cache.json", ".envoy.json", ".embody.json"):
        step(
            f"migrate_old_{_old_name}",
            lambda n=_old_name: (target_dir / n).unlink() if (target_dir / n).exists() else "absent",
        )

    # --- Compute venv Python path ---
    if sys.platform == "win32":
        venv_python = step(
            "compute_venv_python_win",
            lambda: project_dir / ".venv" / "Scripts" / "python.exe",
        )
    else:
        venv_python = step(
            "compute_venv_python_posix",
