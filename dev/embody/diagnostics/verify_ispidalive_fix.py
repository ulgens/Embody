...
registry_pids = [
    {"key": k, "td_pid": info.get("td_pid", 0) if isinstance(info, dict) else None} for k, info in instances.items()
]
...
_record("stash_old_implementation", "completed", f"captured: {_OLD!r}")
...
_record("install_patch", "completed", "EnvoyExt._isPidAlive replaced with patched version")
...
try:
    ...
    _record("writeEnvoyConfig_under_patch", "systemerror_BAD", traceback.format_exc())
except Exception:
...
print(
    "[verify_ispidalive_fix] The monkey-patch is now active for this "
