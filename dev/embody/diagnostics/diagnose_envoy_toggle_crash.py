...
_meta = {
    ...
    "envoy_running_flag": _safe(lambda: bool(op.Embody.fetch("envoy_running", False))),
    ...
    "pydantic_core_version": _safe(lambda: getattr(sys.modules.get("pydantic_core"), "__version__", None)),
...
STEPS = [
    ...
    ("findAvailablePort", lambda: envoy_ext._findAvailablePort(9870)),
    (
        "cleanupStaleThreads",
    ...
    ("upgradeEnvoy", lambda: embody_ext._upgradeEnvoy()),
]
...
_record(
    "full_Start",
    "about_to_run",
    "individual steps survived; invoking Envoy.Start() which spawns the worker thread + uvicorn",
)
try:
    envoy_ext.Start()
...
def _alive_check():
    try:
        ...
        _record(
            "alive_after_5s",
            "survived",
            f"envoyenable={op.Embody.par.Envoyenable.eval()} envoy_running={running} envoystatus={status!r}",
        )
    except Exception:
...
print("[diagnose_envoy_toggle_crash] If TD survives, check the file in ~5 seconds for the alive_after_5s entry.")
