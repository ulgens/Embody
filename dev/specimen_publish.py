...
def _publish():
    ...
    for spec in manifest.get("specimens", []):
        ...
        res = TDN.ExportNetwork(root_path=comp.path, include_dat_content=True, embed_all=True)
    ...
    if written or missing:
        emb.Log(
            f"Specimen publish: {len(written)} written, {len(skipped)} unchanged, {len(missing)} missing",
            "INFO",
        )
        if missing:
